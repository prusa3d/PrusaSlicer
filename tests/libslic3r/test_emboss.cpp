...
Vec2d get_intersection(const Vec2d& point, const Vec2d& dir, const std::array<Vec2d, 3>& triangle)
{
    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        std::optional<double> t2 = ray_segment_intersection(point, dir, triangle[i], triangle[i2]);
        if (t2.has_value() && *t2 > *t)
            t = t2;
    }
...
Vec3d calc_hit_point(const igl::Hit& h, const Index3& triangle, const std::vector<Vec3f>& vertices)
{
    double c1 = h.u;
    double c2 = h.v;
    double c0 = 1.0 - c1 - c2;
...
TEST_CASE("Convert glyph % to model", "[Emboss]")
{
    ...
    std::optional<Emboss::Glyph> glyph = Emboss::letter2glyph(*font, font_index, letter, flatness);
...
TEST_CASE("Visualize glyph from font", "[Emboss]")
{
    ...
    Emboss::FontFileWithCache font(Emboss::create_font_file(font_path.c_str()));
...
ExPolygons heal_and_check(const Polygons& polygons)
{
    ...
    bool is_default_shape = shape.size() == 1
...
TEST_CASE("Heal of overlaping contour", "[Emboss]")
{
    ...
    CHECK(Algorithms::HealPolygon::heal_expolygons(shapes));
}
...
TEST_CASE("Heal of points close to line", "[Emboss]")
{
    ...
    CHECK(Algorithms::HealPolygon::divide_segments_for_close_point(expoly, .6));
    //{ SVG svg("C:/data/temp/healed.svg"); svg.draw(expoly);}
    CHECK(
        Algorithms::ExPolygon::to_points(expoly).size()
        >= (Algorithms::Polygon::to_points(polygon).size() + 2)
    );
}
...
TEST_CASE("Test hit point", "[AABBTreeIndirect]")
{
    ...
    its.indices = {Index3{0, 2, 1}};
    auto tree = AABBTreeIndirect::build_aabb_tree_over_indexed_triangle_set(its.vertices, its.indices);

    Vec3d ray_point(8, 1, 0);
    Vec3d ray_dir(0, 0, 1);
    igl::Hit hit;
    AABBTreeIndirect::intersect_ray_first_hit(its.vertices, its.indices, tree, ray_point, ray_dir, hit);
...
TEST_CASE("Italic check", "[Emboss]")
{
    ...
    while (!dir_paths.empty()) {
        ...
        for (const auto& entry : fs::directory_iterator(dir_path)) {
            ...
            if (exist_italic && exist_non_italic)
                break;
...
CGALMesh to_cgal(const indexed_triangle_set& its, const std::string& face_map_name)
{
    CGALMesh result;
    if (its.empty())
        return result;
...
)
{
    ...
    auto edge_shape_map = result
                              .add_property_map<CGALMesh::Edge_index, IntersectingElemnt>(
                                  edge_shape_map_name
                              )
                              .first;
    auto face_shape_map = result
                              .add_property_map<CGALMesh::Face_index, IntersectingElemnt>(
                                  face_shape_map_name
                              )
                              .first;

    std::vector<CGALMesh::Vertex_index> indices;
    auto insert_contour = [&projection,
                           &indices,
                           &result,
                           &contour_indices,
                           &edge_shape_map,
                           &face_shape_map](const Polygon& polygon, int32_t iexpoly, int32_t id) {
        indices.clear();
        ...
        for (const Point& p2 : polygon.points) {
            ...
            auto vi = result.add_vertex(
                typename CGALMesh::Point{p.first.x(), p.first.y(), p.first.z()}
            );
        ...
        for (int32_t i = 0; i < int32_t(indices.size()); i += 2) {
            ...
            auto find_edge = [&result](
                                 CGALMesh::Face_index fi,
            ...
            edge_shape_map[result.edge(find_edge(fi, indices[i], indices[i + 1]))] = IntersectingElemnt{
                vertex_index,
                contour_index,
                IntersectingElemnt::Type::edge_1
            };
            face_shape_map[fi] = IntersectingElemnt{
                vertex_index,
                contour_index,
                IntersectingElemnt::Type::face_1
            };
            edge_shape_map[result.edge(find_edge(fi, indices[i + 1], indices[j]))] = IntersectingElemnt{
                vertex_index,
                contour_index,
                IntersectingElemnt::Type::edge_2
            };
            face_shape_map[result.add_face(indices[j], indices[i + 1], indices[j + 1])] = IntersectingElemnt{
                vertex_index,
                contour_index,
                IntersectingElemnt::Type::face_2
            };
            ++contour_index;
        }
    };
...
indexed_triangle_set cut_shape(
...
TEST_CASE("Emboss extrude cut", "[Emboss-Cut]")
{
    ...
    auto vert_shape_map = cgal_object
                              .add_property_map<MyMesh::Vertex_index, IntersectingElemnt>(
                                  vert_shape_map_name
                              )
                              .first;
    ...
    MyMesh cgal_shape = MeshBoolean::cgal2::to_cgal(
        shape,
        projection,
        0,
        edge_shape_map_name,
    ...
    auto edge_shape_map = cgal_shape
                              .property_map<MyMesh::Edge_index, IntersectingElemnt>(edge_shape_map_name)
                              .first;
    auto face_shape_map = cgal_shape
                              .property_map<MyMesh::Face_index, IntersectingElemnt>(face_shape_map_name)
                              .first;
    ...
    const auto& p = ::CGAL::Polygon_mesh_processing::parameters::throw_on_self_intersection(false)
                        .visitor(visitor)
    ...
    for (auto fi : cgal_object.faces()) {
        ...
        do {
            ...
            if (get(ecm, edge_index)) {
                ...
                const Polygon& contour             = vertex_index.contour == 0 ?
                                expoly.contour :
                                expoly.holes[vertex_index.contour - 1];
    ...
    for (auto fi : cgal_object.faces()) {
        ...
        case SideType::inside:
            color = ::CGAL::Color{255, 0, 0};
            break;
        case SideType::outside:
            color = ::CGAL::Color{255, 0, 255};
            break;
        case SideType::not_constrained:
            color = ::CGAL::Color{0, 255, 0};
            break;
        }
    }
    ...
    for (Visitor::face_descriptor fi : cgal_object.faces()) {
        if (side_type_map[fi] != SideType::not_constrained)
            continue;
    ...
    std::vector<std::pair<int32_t, int32_t>> map_vertices(
        cgal_object.number_of_vertices(),
    ...
    for (auto fi : cgal_object.faces()) {
        ...
        assert(
            state == FaceState::Unmarked
            || state == FaceState::UnmarkedSplit
            || state == FaceState::UnmarkedEmitted
            || state == FaceState::Marked
            || state == FaceState::MarkedSplit
        );
        if (state == FaceState::UnmarkedEmitted)
            continue; // Already emitted.

        if (state == FaceState::Unmarked || state == FaceState::UnmarkedSplit) {
            // Just copy the unsplit source face.
            const Index3 source_vertices = cube.indices[source_face_id];
            Index3 target_vertices;
            for (int i = 0; i < 3; ++i) {
                target_vertices[i] = map_vertices[source_vertices[i]].first;
                if (target_vertices[i] == -1) {
                    map_vertices[source_vertices[i]].first = target_vertices[i] = int(
        ...
        Index3 target_vertices_extruded{-1, -1, -1};
        for (int i = 0; i < 3; ++i) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (target_vertices_extruded[i] == -1) {
                map_vertices[source_vertices[i]].second = target_vertices_extruded[i] = int(
                    its_extruded.vertices.size()
                );
                const auto& p = cgal_object.point(cgal_object.target(hi));
                its_extruded.vertices.emplace_back(
                    Vec3f{float(p.x()), float(p.y()), float(p.z())} + extrude_dir
                );
            }
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (boundary_vertex[i] && boundary_vertex[j]) {
                ...
                its_extruded.indices.emplace_back(
                    Index3{target_vertices[i], target_vertices[j], target_vertices_extruded[i]}
                );
                its_extruded.indices.emplace_back(
                    Index3{target_vertices_extruded[i], target_vertices[j], target_vertices_extruded[j]}
                );
            }
        }
    }
    ...
    for (auto ei : cgal_object.edges())
        if (cgal_object.is_valid(ei)) {
            ...
            edges_its.indices.emplace_back(
                Index3{
                    static_cast<int>(edges_its.vertices.size()),
                    static_cast<int>(edges_its.vertices.size() + 1),
                    static_cast<int>(edges_its.vertices.size() + 2)
                }
            );
