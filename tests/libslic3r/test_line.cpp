...
TEST_CASE("Check if lines are parallel", "[Line]") {
    CHECK(Line{{0, 0}, {100, 0}}.is_parallel_to(Line{{200, 200}, {0, 200}}));
}
...
TEST_CASE("Parallel lines under angles", "[Line]") {
    ...
    CHECK(line.is_parallel_to(clone));
    ...
    CHECK(line.is_parallel_to(line.direction()));
    INFO("Line is parallel to its direction + PI");
    line.is_parallel_to(line.direction() + M_PI);
    INFO("line is parallel to its direction - PI");
    line.is_parallel_to(line.direction() - M_PI);
...
TEST_CASE("Intersection infinite", "[Line]") {
    ...
    Point r;
    Algorithms::Line::intersection_infinite(a, b, r);
    CHECK(r == Point{300, 0});
}
