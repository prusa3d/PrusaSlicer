...
try_initialize(EGLDisplay dpy, bool surfaceless, int width, int height, std::string* err)
{
    ...
    if (eglBindAPI(EGL_OPENGL_API) != EGL_TRUE) {
        if (err)
            *err = "eglBindAPI(EGL_OPENGL_API) failed — desktop GL not supported by this EGL";
        eglTerminate(dpy);
        return nullptr;
    }
    ...
    if (eglChooseConfig(dpy, cfg_attrs, &cfg, 1, &num_cfgs) != EGL_TRUE || num_cfgs == 0) {
        if (err)
            *err = "eglChooseConfig: no matching EGLConfig";
        eglTerminate(dpy);
        return nullptr;
    }
    ...
    if (ctx == EGL_NO_CONTEXT) {
        if (err)
            *err = "eglCreateContext failed for GL 3.2";
        eglTerminate(dpy);
        return nullptr;
    }
    ...
    if (!surfaceless) {
        const EGLint pb_attrs[] =
            {EGL_WIDTH, width > 0 ? width : 1, EGL_HEIGHT, height > 0 ? height : 1, EGL_NONE};
    ...
    if (eglMakeCurrent(dpy, surf, surf, ctx) != EGL_TRUE) {
        if (err)
            *err = "eglMakeCurrent failed on this display/context";
        if (surf != EGL_NO_SURFACE)
    ...
    SPDLOG_INFO(
        "EGL offscreen context ready ({}), EGL {}.{}",
...
OffscreenGLContext::create(int width, int height, std::string* error_out)
{
    ...
    if (error_out) {
        *error_out = "EGL surfaceless: " + err_surfaceless + "; EGL default: " + err_default;
    }
    return nullptr;
}
