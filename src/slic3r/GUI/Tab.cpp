///|/ Copyright (c) Prusa Research 2017 - 2023 Oleksandra Iushchenko @YuSanka, Lukáš Matěna
///@lukasmatena, Lukáš Hejl @hejllukas, Vojtěch Bubník @bubnikv, Pavel Mikuš @Godrak, Tomáš Mészáros
///@tamasmeszaros, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Vojtěch Král @vojtechkral
///|/ Copyright (c) 2023 Nicholas Sica @NickSica
...
Tab::Tab(wxBookCtrlBase *parent, const wxString &title, Preset::Type type)
    ...
    m_compatible_prints.dialog_label = _L(
        "Select the print profiles this profile is compatible with."
    );
...
void Tab::create_preset_tab() {
    ...
    m_tmp_panel =
        new wxPanel(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBK_LEFT | wxTAB_TRAVERSAL);
    ...
    m_question_btn->SetToolTip(
        _(L("Hover the cursor over buttons to find more information \n"
    ...
    m_treectrl = new wxTreeCtrl(
        panel, wxID_ANY, wxDefaultPosition, wxSize(20 * m_em_unit, -1),
    ...
    m_page_view = new wxScrolledWindow(
        page_parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL
    );
    ...
    m_btn_hide_incompatible_presets
        ->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e) { toggle_show_hide_incompatible(); }));
...
void Tab::add_scaled_button(
    ...
    *btn = new ScalableButton(
        parent, wxID_ANY, icon_name, label, wxDefaultSize, wxDefaultPosition, style
    );
...
Slic3r::GUI::PageShp Tab::add_options_page(
    const wxString &title, const std::string &icon, bool is_extruder_pages /*= false*/
) {
...
void Tab::OnActivate() {
    ...
    if (m_presets_choice->IsShown())
        ...
        // From the tab creation whole top sizer is hidden to correct update of preset combobox's
        // size (see https://github.com/prusa3d/PrusaSlicer/issues/10746)
        ...
        m_btn_hide_incompatible_presets->Show(
            m_show_btn_incompatible_presets && m_type != Slic3r::Preset::TYPE_PRINTER
        );
...
void Tab::update_label_colours() {
    ...
    while (cur_item) {
        ...
        for (auto page : m_pages) {
            ...
            const wxColor *clr = !page->m_is_nonsys_values ?
                &m_sys_label_clr :
    ...
    if (m_type == Preset::TYPE_PRINTER) {
        {
            auto check_bed_custom_options = [](std::vector<std::string> &keys) {
                ...
                if (old_keys_size != keys.size() &&
                    std::find(keys.begin(), keys.end(), "bed_shape") == keys.end())
...
void add_correct_opts_to_options_list(
    const std::string &opt_key, std::map<std::string, int> &map, Tab *tab, int value
) {
    ...
    if (respect_vec_values) {
        ...
        case coInts:
            add_correct_opts_to_options_list<
                ConfigOptionInts>(opt_key, m_options_list, this, m_opt_status_value);
            break;
        case coBools:
            add_correct_opts_to_options_list<
                ConfigOptionBools>(opt_key, m_options_list, this, m_opt_status_value);
            break;
        case coFloats:
            add_correct_opts_to_options_list<
                ConfigOptionFloats>(opt_key, m_options_list, this, m_opt_status_value);
            break;
        case coStrings:
            add_correct_opts_to_options_list<
                ConfigOptionStrings>(opt_key, m_options_list, this, m_opt_status_value);
            break;
        case coPercents:
            add_correct_opts_to_options_list<
                ConfigOptionPercents>(opt_key, m_options_list, this, m_opt_status_value);
            break;
        case coPoints:
            add_correct_opts_to_options_list<
                ConfigOptionPoints>(opt_key, m_options_list, this, m_opt_status_value);
            break;
        case coFloatsOrPercents:
            add_correct_opts_to_options_list<
                ConfigOptionFloatsOrPercents>(opt_key, m_options_list, this, m_opt_status_value);
            break;
        case coEnums:
            add_correct_opts_to_options_list<
                ConfigOptionEnumsGeneric>(opt_key, m_options_list, this, m_opt_status_value);
            break;
...
void Tab::update_changed_tree_ui() {
    ...
    while (cur_item) {
        ...
        for (auto page : m_pages) {
            ...
            const wxColor *clr = sys_page ?
                (m_is_default_preset ? &m_default_text_clr : &m_sys_label_clr) :
                modified_page ?
                &m_modified_label_clr :
                &m_default_text_clr;
...
void Tab::update_undo_buttons() {
    m_undo_btn->SetBitmap_(
        m_is_modified_values ? m_bmp_value_revert.name() : m_bmp_white_bullet.name()
    );
    m_undo_to_sys_btn->SetBitmap_(
        m_is_nonsys_values ? m_bmp_non_system->name() : m_bmp_value_lock.name()
    );
...
void Tab::update_tab_ui() { m_presets_choice->update(); }

// Load a provied DynamicConfig into the tab, modifying the active preset.
// This could be used for example by setting a Wipe Tower position by interactive manipulation in
// the 3D view.
...
std::pair<OG_CustomCtrl *, bool *>
Tab::get_custom_ctrl_with_blinking_ptr(const t_config_option_key &opt_key, int opt_index /* = -1*/) {
    if (!m_active_page)
...
Field *
Tab::get_field(const t_config_option_key &opt_key, Page **selected_page, int opt_index /* = -1*/) {
    Field *field = nullptr;
    for (auto page : m_pages) {
...
void Tab::
    load_key_value(const std::string &opt_key, const boost::any &value, bool saved_value /*= false*/) {
    if (!saved_value)
...
static wxString support_combo_value_for_config(const DynamicPrintConfig &config, bool is_fff) {
    ...
    const std::string buildplate_only = is_fff ?
        "support_material_buildplate_only" :
        slatree + "support_buildplate_only";

    return !config.opt_bool(support) ?
        _("None") :
...
void Tab::on_value_change(const std::string &opt_key, const boost::any &value) {
    ...
    if (is_fff ?
            (opt_key == "support_material" || opt_key == "support_material_auto" ||
    ...
    if (opt_key == "wipe_tower" || opt_key == "single_extruder_multi_material" ||
    ...
    bool single_extruder_multi_material =
        dynamic_cast<ConfigOptionBool *>(
            (m_preset_bundle->printers.get_edited_preset().config)
                .option("single_extruder_multi_material")
        )
            ->value;

    auto wiping_dialog_button = wxGetApp().sidebar().get_wiping_dialog_button();
    if (wiping_dialog_button) {
        wiping_dialog_button->Show(
            wipe_tower_enabled && multiple_extruders && single_extruder_multi_material
        );
        wiping_dialog_button->GetParent()->Layout();
    }
}

void Tab::activate_option(
    ...
    // And do it before finding of the cur_item to avoid a case when Tab isn't activated jet and all
    // treeItems are invisible
    ...
    for (const std::string &another_blinking_opt_key : another_blinking_opt_keys) {
        custom_blinking_ctrls.emplace_back(
            get_custom_ctrl_with_blinking_ptr(another_blinking_opt_key)
        );
    }
...
void Tab::cache_config_diff(
    const std::vector<std::string> &selected_options,
    const DynamicPrintConfig *config /* = nullptr*/
) {
    m_cache_config
        .apply_only(config ? *config : m_presets->get_edited_preset().config, selected_options);
}
...
void Tab::on_presets_changed() {
    ...
    for (auto t : m_dependent_tabs) {
        ...
        // If the printer tells us that the print or filament/sla_material preset has been switched
        // or invalidated, refresh the print or filament/sla_material tab page. But if there are
        // options, moved from the previously selected preset, update them to edited preset
...
void Tab::build_preset_description_line(ConfigOptionsGroup *optgroup) {
    ...
    auto detach_preset_btn = [this](wxWindow *parent) {
        m_detach_preset_btn = new ScalableButton(
            parent, wxID_ANY, "lock_open_sys", _L("Detach from system preset"), wxDefaultSize,
        ...
        btn->Bind(wxEVT_BUTTON, [this, parent](wxCommandEvent &) {
            ...
            wxString msg_text = system ?
                _(L("A copy of the current system preset will be created, which will be detached "
            ...
            // wxMessageDialog dialog(parent, msg_text, _(L("Detach preset")), wxICON_WARNING |
            // wxYES_NO | wxCANCEL);
            MessageDialog
                dialog(parent, msg_text, _(L("Detach preset")), wxICON_WARNING | wxYES_NO | wxCANCEL);
            if (dialog.ShowModal() == wxID_YES)
                save_preset(
                    m_presets->get_edited_preset().is_system ?
                        std::string() :
    ...
    if (parent && parent->vendor) {
        ...
        if (m_type == Slic3r::Preset::TYPE_PRINTER) {
            ...
            case ptFFF: {
                ...
                const std::string &default_print_profile = preset.config.opt_string(
            ...
            case ptSLA: {
                ...
                const std::string &default_sla_print_profile = preset.config.opt_string(
...
static void validate_custom_parameters(
    Tab *tab, const t_config_option_key &opt_key, const boost::any &value
) {
    if (!bool(parse_custom_parameters(boost::any_cast<std::string>(value)))) {
        MessageDialog dialog(
            wxGetApp().mainframe, _L("Unable to parse custom parameters."), _L("Error"),
            wxICON_WARNING | wxOK
        );
        dialog.ShowModal();
    }
...
void TabPrint::build() {
    ...
    optgroup
        ->append_single_option_line("extra_perimeters", category_path + "extra-perimeters-if-needed");
    optgroup->append_single_option_line(
        "extra_perimeters_on_overhangs", category_path + "extra-perimeters-on-overhangs"
    );
    optgroup->append_single_option_line(
        "ensure_vertical_shell_thickness", category_path + "ensure-vertical-shell-thickness"
    );
    optgroup->append_single_option_line(
        "avoid_crossing_curled_overhangs", category_path + "avoid-crossing-curled-overhangs"
    );
    optgroup->append_single_option_line(
        "avoid_crossing_perimeters", category_path + "avoid-crossing-perimeters"
    );
    optgroup->append_single_option_line(
    ...
    optgroup
        ->append_single_option_line("staggered_inner_seams", category_path + "staggered-inner-seams");

    const std::string scarf_seam_path{"seam-position_151069#"};
    optgroup->append_single_option_line(
        "scarf_seam_placement", scarf_seam_path + "scarf-joint-placement"
    );
    optgroup->append_single_option_line(
        "scarf_seam_only_on_smooth", scarf_seam_path + "scarf-joint-only-on-smooth-perimeters"
    );
    optgroup->append_single_option_line(
        "scarf_seam_start_height", scarf_seam_path + "scarf-start-height"
    );
    optgroup->append_single_option_line(
        "scarf_seam_entire_loop", scarf_seam_path + "scarf-joint-around-entire-perimeter"
    );
    optgroup->append_single_option_line("scarf_seam_length", scarf_seam_path + "scarf-joint-length");
    optgroup->append_single_option_line(
        "scarf_seam_max_segment_length", scarf_seam_path + "max-scarf-joint-segment-length"
    );
    optgroup->append_single_option_line(
        "scarf_seam_on_inner_perimeters", scarf_seam_path + "scarf-joint-on-inner-perimeters"
    );

    optgroup->append_single_option_line(
        "external_perimeters_first", category_path + "external-perimeters-first"
    );
    ...
    optgroup
        ->append_single_option_line("fuzzy_skin_thickness", category_path + "fuzzy-skin-thickness");
    optgroup->append_single_option_line(
        "fuzzy_skin_point_dist", category_path + "fuzzy-skin-point-distance"
    );

    optgroup = page->new_optgroup(L("Only one perimeter"));
    category_path = "layers-and-perimeters_1748/#";
    optgroup->append_single_option_line(
        "top_one_perimeter_type", category_path + "top-one-perimeter-type"
    );
    optgroup->append_single_option_line(
        "only_one_perimeter_first_layer", category_path + "only-one-perimeter-first-layer"
    );
    ...
    optgroup->append_single_option_line(
        "ironing_spacing", category_path + "spacing-between-ironing-passes"
    );
    ...
    optgroup->append_single_option_line(
        "infill_every_layers", category_path + "combine-infill-every-x-layers"
    );

    optgroup = page->new_optgroup(L("Advanced"));
    optgroup->append_single_option_line(
        "solid_infill_every_layers", category_path + "solid-infill-every-x-layers"
    );
    optgroup->append_single_option_line("fill_angle", category_path + "fill-angle");
    optgroup->append_single_option_line(
        "solid_infill_below_area", category_path + "solid-infill-threshold-area"
    );
    ...
    optgroup
        ->append_single_option_line("support_material", category_path + "generate-support-material");
    optgroup->append_single_option_line(
        "support_material_auto", category_path + "auto-generated-supports"
    );
    optgroup->append_single_option_line(
        "support_material_threshold", category_path + "overhang-threshold"
    );
    optgroup->append_single_option_line(
        "support_material_enforce_layers", category_path + "enforce-support-for-the-first"
    );
    optgroup->append_single_option_line(
        "raft_first_layer_density", category_path + "raft-first-layer-density"
    );
    optgroup->append_single_option_line(
    ...
    optgroup->append_single_option_line(
        "support_material_bottom_contact_distance", category_path + "contact-z-distance"
    );
    optgroup->append_single_option_line("support_material_pattern", category_path + "pattern");
    optgroup->append_single_option_line(
        "support_material_with_sheath", category_path + "with-sheath-around-the-support"
    );
    optgroup->append_single_option_line(
        "support_material_spacing", category_path + "pattern-spacing-0-inf"
    );
    ...
    optgroup->append_single_option_line(
        "support_material_bottom_interface_layers", category_path + "interface-layers"
    );
    optgroup->append_single_option_line(
        "support_material_interface_pattern", category_path + "interface-pattern"
    );
    optgroup->append_single_option_line(
        "support_material_interface_spacing", category_path + "interface-pattern-spacing"
    );
    optgroup->append_single_option_line(
        "support_material_interface_contact_loops", category_path + "interface-loops"
    );
    optgroup->append_single_option_line(
        "support_material_buildplate_only", category_path + "support-on-build-plate-only"
    );
    optgroup->append_single_option_line(
    ...
    optgroup
        ->append_single_option_line("dont_support_bridges", category_path + "dont-support-bridges");
    optgroup->append_single_option_line(
        "support_material_synchronize_layers", category_path + "synchronize-with-object-layers"
    );
    ...
    optgroup->append_single_option_line(
        "max_volumetric_extrusion_rate_slope_positive", "pressure-equlizer_331504"
    );
    optgroup->append_single_option_line(
        "max_volumetric_extrusion_rate_slope_negative", "pressure-equlizer_331504"
    );
    ...
    optgroup
        ->append_single_option_line("elefant_foot_compensation", "elephant-foot-compensation_114487");
    ...
    line.widget = [this](wxWindow *parent) {
        ...
        stat_text->SetText(_L(
            "Note: When using this option, the Arrange function automatically "
            "accounts for the printer geometry to prevent collisions. Extruder geometry is "
            "built-in for most "
    ...
    create_line_with_widget(
        optgroup.get(), "gcode_substitutions", "g-code-substitutions_301694",
        [this](wxWindow *parent) { return create_manage_substitution_widget(parent); }
    );
    line = {"", ""};
    line.full_width = 1;
    line.widget = [this](wxWindow *parent) { return create_substitutions_widget(parent); };
    optgroup->append_line(line);
...
void TabPrint::update() {
    ...
    // Note: This workaround works till "support_material" and "overhangs" is exclusive sets of
    // mutually no-exclusive parameters. But it should be corrected when we will have more such
    // sets. Disable check of the compatibility of the "support_material" and "overhangs" options
    // for saved user profile NOTE: Initialization of the support_material_overhangs_queried value
    // have to be processed just ones
    if (!m_config_manipulation.is_initialized_support_material_overhangs_queried()) {
        ...
        m_config_manipulation.initialize_support_material_overhangs_queried(
            is_user_and_saved_preset && support_material_overhangs_queried
        );
    }
...
bool Tab::validate_custom_gcode(const wxString &title, const std::string &gcode) {
    ...
    if (invalid) {
        ...
        wxString reports = format_wxstr(
            _L_PLURAL(
                "The following line %s contains reserved keywords.\nPlease remove it, as it may "
                "cause problems in G-code visualization and printing time estimation.",
                "The following lines %s contain reserved keywords.\nPlease remove them, as they "
                "may cause problems in G-code visualization and printing time estimation.",
                tags.size()
            ),
            lines
        );
        // wxMessageDialog dialog(wxGetApp().mainframe, reports, _L("Found reserved keywords in") +
        // " " + _(title), wxICON_WARNING | wxOK);
        MessageDialog dialog(
            wxGetApp().mainframe, reports, _L("Found reserved keywords in") + " " + _(title),
            wxICON_WARNING | wxOK
        );
        dialog.ShowModal();
    }
...
static void validate_custom_gcode_cb(
    Tab *tab, const wxString &title, const t_config_option_key &opt_key, const boost::any &value
) {
    tab->validate_custom_gcodes_was_shown =
...
void TabFilament::set_custom_gcode(const t_config_option_key &opt_key, const std::string &value) {
    std::vector<std::string> gcodes =
        static_cast<const ConfigOptionStrings *>(m_config->option(opt_key))->values;
    gcodes[0] = value;
...
void TabFilament::create_line_with_near_label_widget(
    ConfigOptionsGroupShp optgroup, const std::string &opt_key, int opt_index /* = 0*/
) {
    Line line{"", ""};
    ...
    line.near_label_widget =
        [this, optgroup_wk = ConfigOptionsGroupWkp(optgroup), opt_key, opt_index](wxWindow *parent) {
            ...
            check_box->Bind(wxEVT_CHECKBOX, [optgroup_wk, opt_key, opt_index](wxCommandEvent &evt) {
                ...
                if (auto optgroup_sh = optgroup_wk.lock(); optgroup_sh) {
                    if (Field *field = optgroup_sh->get_fieldc(opt_key, opt_index);
                        field != nullptr) {
...
std::vector<std::pair<std::string, std::vector<std::string>>> filament_overrides_option_keys{
    {"Travel lift",
     {"filament_retract_lift", "filament_travel_ramping_lift", "filament_travel_max_lift",
...
void TabFilament::update_filament_overrides_page() {
    ...
    for (const auto &[title, keys] : filament_overrides_option_keys) {
        ...
        for (const std::string &opt_key : keys) {
            bool is_checked{true};
            if (title == "Retraction" && opt_key != "filament_retract_length" &&
                !have_retract_length) {
                is_checked = false;
            }
            ...
            if (title == "Travel lift" && !uses_ramping_lift &&
                opt_key != "filament_travel_ramping_lift" && opt_key != "filament_retract_lift" &&
                opt_key != "filament_retract_lift_above" &&
                opt_key != "filament_retract_lift_below") {
                is_checked = false;
            }
...
void TabFilament::create_extruder_combobox() {
    m_extruders_cb = new BitmapComboBox(
        this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(12 * m_em_unit, -1), 0, nullptr,
        wxCB_READONLY
    );
    m_extruders_cb->Hide();
...
void TabFilament::update_extruder_combobox() {
    ...
    if (extruder_cnt != m_extruders_cb->GetCount()) {
        ...
        for (size_t id = 1; id <= extruder_cnt; id++)
            m_extruders_cb
                ->Append(format_wxstr("%1% %2%", _L("Extruder"), id), *get_bmp_bundle("funnel"));
    }
    ...
    if (m_active_extruder >= int(extruder_cnt)) {
        ...
        const std::string &preset_name =
            m_preset_bundle->extruders_filaments[0].get_selected_preset_name();
...
void TabFilament::build() {
    ...
    optgroup->append_single_option_line(
        "enable_dynamic_fan_speeds", category_path + "dynamic-fan-speeds"
    );
    ...
    optgroup->append_single_option_line(
        "slowdown_below_layer_time", category_path + "cooling-thresholds"
    );
    ...
    optgroup
        ->append_single_option_line("filament_max_volumetric_speed", "max-volumetric-speed_127176");
    ...
    optgroup->append_single_option_line(
        "filament_infill_max_crossing_speed", "max-crossing-infill-speed"
    );
    ...
    create_line_with_widget(optgroup.get(), "filament_ramming_parameters", "", [this](wxWindow *parent) {
        auto ramming_dialog_btn = new wxButton(
            parent, wxID_ANY, _(L("Ramming settings")) + dots, wxDefaultPosition, wxDefaultSize,
            wxBU_EXACTFIT
        );
    ...
    optgroup->on_change =
        [this,
         &optgroup_title =
             optgroup->title](const t_config_option_key &opt_key, const boost::any &value) {
...
void TabFilament::update_description_lines() {
    ...
    if (m_active_page->title() == "Cooling" && m_cooling_description_line)
        m_cooling_description_line->SetText(
            from_u8(PresetHints::cooling_description(m_presets->get_edited_preset()))
        );
...
void TabFilament::toggle_options() {
    ...
    if (m_active_page->title() == "Cooling") {
        ...
        for (auto el :
             {"max_fan_speed", "fan_below_layer_time", "slowdown_below_layer_time",
              "min_print_speed", "cooling_slowdown_logic"})
...
void TabFilament::load_current_preset() {
    ...
    assert(
        m_active_extruder >= 0 &&
        size_t(m_active_extruder) < m_preset_bundle->extruders_filaments.size()
    );
    const std::string &selected_extr_filament_name =
        m_preset_bundle->extruders_filaments[m_active_extruder].get_selected_preset_name();
...
bool TabFilament::save_current_preset(const std::string &new_name, bool detach) {
    ...
    const bool is_saved = Tab::save_current_preset(
        new_name, detach
    ); // Note: is_saved is "true", if preset with new_name was saved as new preset
    ...
    m_preset_bundle->extruders_filaments[m_active_extruder].select_filament(
        m_presets->get_idx_selected()
    );
    return is_saved;
}
...
wxSizer *Tab::description_line_widget(
    wxWindow *parent, ogStaticText **StaticText, wxString text /*= wxEmptyString*/
) {
...
void TabPrinter::build() {
    ...
    std::string def_preset_name = "- default " +
        std::string(m_printer_technology == ptSLA ? "FFF" : "SLA") + " -";
...
void TabPrinter::build_print_host_upload_group(Page *page) {
    ...
    wxString description_line_text = _L(
        ...
        "Printer profiles combo box, "
        "by selecting the \"Add physical printer\" item in the Printer combo box. The Physical "
        "Printer profile editor opens "
        "also when clicking on the \"cog\" icon in the Printer settings tab. The Physical Printer "
        "profiles are being stored "
    ...
    line.widget = [this, description_line_text](wxWindow *parent) {
        return description_line_widget(
            parent,
...
void TabPrinter::build_fff() {
    ...
    create_line_with_widget(
        optgroup.get(), "bed_shape", "custom-svg-and-png-bed-textures_124612",
    ...
    optgroup->on_change = [this, optgroup_wk = ConfigOptionsGroupWkp(optgroup)](
        ...
        size_t extruders_count = size_t(
            boost::any_cast<int>(optgroup_sh->get_value("extruders_count"))
        );
        wxTheApp->CallAfter([this, opt_key, value, extruders_count]() {
            if (opt_key == "extruders_count" || opt_key == "single_extruder_multi_material") {
                ...
                update_dirty();
                if (opt_key ==
                    "single_extruder_multi_material") { // the single_extruder_multimaterial was
                                                        // added to force pages
                    on_value_change(
                        opt_key, value
                    ); // rebuild - let's make sure the on_value_change is not skipped
                    ...
                    if (boost::any_cast<bool>(value) && m_extruders_count > 1) {
                        ...
                        std::vector<unsigned char> high_flow_nozzles =
                            static_cast<const ConfigOptionBools *>(
                                m_config->option("nozzle_high_flow")
                            )
                                ->values;
                        ...
                        for (size_t i = 1; i < nozzle_diameters.size(); ++i) {
                            ...
                            if (fabs(nozzle_diameters[i] - nozzle_diameters[0]) > EPSILON ||
                                ...
                                const wxString msg_text = _(L(
                                    ...
                                    "all extruders must have the same nozzle diameter and "
                                    "'High flow' state.\n"
                                    "Do you want to change these values for all extruders to "
                                    "first extruder values?"
                                ));
                                MessageDialog dialog(
                                    parent(), msg_text, _(L("Extruder settings do not match")),
                                ...
                                if (dialog.ShowModal() == wxID_YES) {
                                    ...
                                    new_conf.set_key_value(
                                        "nozzle_diameter", new ConfigOptionFloats(nozzle_diameters)
                                    );
                                    new_conf.set_key_value(
                                        "nozzle_high_flow", new ConfigOptionBools(high_flow_nozzles)
                                    );
                                } else
                                    new_conf.set_key_value(
                                        "single_extruder_multi_material", new ConfigOptionBool(false)
                                    );
        ...
        optgroup->on_change = [this](t_config_option_key opt_key, boost::any value) {
            ...
            if (opt_key == "thumbnails" && m_config->has("thumbnails_format")) {
                ...
                if (const std::string val = boost::any_cast<std::string>(value); !value.empty()) {
                    auto [thumbnails_list, errors] = GCodeThumbnails::make_and_check_thumbnail_list(
                        val
                    );

                    if (errors != enum_bitmask<ThumbnailError>()) {
                        // TRN: First argument is parameter name, the second one is the value.
                        std::string error_str = format(
                            _u8L("Invalid value provided for parameter %1%: %2%"), "thumbnails", val
                        );
                        error_str += GCodeThumbnails::get_error_string(errors);
                        InfoDialog(parent(), _L("G-code flavor is switched"), from_u8(error_str))
                            .ShowModal();
                    }
...
optgroup->on_change = [this](t_config_option_key opt_key, boost::any value) {
    wxTheApp->CallAfter([this, opt_key, value]() {
        ...
        if (opt_key == "gcode_flavor") {
            ...
            bool supports_travel_acceleration = GCodeWriter::supports_separate_travel_acceleration(
                flavor
            );
            ...
            if ((flavor == gcfKlipper && is_emit_to_gcode) ||
                ...
                if (!m_supports_min_feedrates && m_use_silent_mode) {
                    ...
                    auto silent_mode = static_cast<ConfigOptionBool *>(
                        m_config->option("silent_mode")->clone()
                    );
...
optgroup->on_change =
    [this,
     &optgroup_title = optgroup->title](const t_config_option_key &opt_key, const boost::any &value) {
        validate_custom_gcode_cb(this, optgroup_title, opt_key, value);
    };
optgroup->edit_custom_gcode = [this](const t_config_option_key &opt_key) {
    edit_custom_gcode(opt_key);
...
void TabPrinter::build_sla() {
    ...
    for (auto &axis : {"X", "Y", "Z"}) {
        auto opt = optgroup->get_option(
            std::string("relative_correction_") + char(std::tolower(axis[0]))
        );
        opt.opt.label = axis;
...
void TabPrinter::extruders_count_changed(size_t extruders_count) {
    ...
    if (m_extruders_count != extruders_count) {
        ...
        is_count_changed = is_updated_mm_filament_presets = true;
               ...
               m_preset_bundle->project_config.option<ConfigOptionFloats>("wiping_volumes_matrix")
                       ->values.size() > 1) {
...
static void create_legend(
    Slic3r::GUI::PageShp page,
...
PageShp TabPrinter::build_kinematics_page() {
    ...
    optgroup->on_change = [this](const t_config_option_key &opt_key, boost::any value) {
        if (opt_key == "machine_limits_usage" &&
            static_cast<MachineLimitsUsage>(boost::any_cast<int>(value)) ==
...
void TabPrinter::build_extruder_pages(size_t n_before_extruders) {
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count;
         ++extruder_idx) {
        // # build page
        ...
        optgroup->on_change = [this,
                               extruder_idx](const t_config_option_key &opt_key, boost::any value) {
            const bool is_single_extruder_MM = m_config->opt_bool("single_extruder_multi_material");
            const bool is_nozzle_diameter_changed = opt_key.find("nozzle_diameter") !=
                std::string::npos;
            ...
            if (is_single_extruder_MM && m_extruders_count > 1 && is_nozzle_diameter_changed) {
                ...
                std::vector<double> nozzle_diameters =
                    static_cast<const ConfigOptionFloats *>(m_config->option("nozzle_diameter"))
                        ->values;
                ...
                if (fabs(nozzle_diameters[extruder_idx == 0 ? 1 : 0] - new_nd) > EPSILON) {
                    ...
                    MessageDialog
                        dialog(parent(), msg_text, _L("Nozzle diameter"), wxICON_WARNING | wxYES_NO);
                    ...
                    new_conf
                        .set_key_value("nozzle_diameter", new ConfigOptionFloats(nozzle_diameters));
                    load_config(new_conf);
                }
            }
            ...
            if (is_single_extruder_MM && m_extruders_count > 1 && is_high_flow_changed) {
                ...
                std::vector<unsigned char> nozzle_high_flow =
                    static_cast<const ConfigOptionBools *>(m_config->option("nozzle_high_flow"))
                        ->values;
                ...
                if (nozzle_high_flow[extruder_idx == 0 ? 1 : 0] != new_hf) {
                    ...
                    MessageDialog dialog(
                        parent(), msg_text, _L("Extruder settings do not match"),
                        wxICON_WARNING | wxYES_NO
                    );
                    ...
                    new_conf
                        .set_key_value("nozzle_high_flow", new ConfigOptionBools(nozzle_high_flow));
                    load_config(new_conf);
                }
            }
            ...
            if (is_nozzle_diameter_changed || is_high_flow_changed) {
                if (extruder_idx == 0)
                    // Mark the print & filament enabled if they are compatible with the currently
                    // selected preset. If saving the preset changes compatibility with other
                    // presets, keep the now incompatible dependent presets selected, however with a
                    // "red flag" icon showing that they are no more compatible.
                    m_preset_bundle->update_compatible(PresetSelectCompatibleType::Never);
                else
                    m_preset_bundle->update_filaments_compatible(
                        PresetSelectCompatibleType::Never, extruder_idx
                    );
            }
        ...
        auto reset_to_filament_color = [this, extruder_idx](wxWindow *parent) {
            ScalableButton *btn = new ScalableButton(
                parent, wxID_ANY, "undo", _L("Reset to Filament Color"), wxDefaultSize,
                wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT
            );
            ...
            btn->Bind(wxEVT_BUTTON, [this, extruder_idx](wxCommandEvent &e) {
                std::vector<std::string> colors =
                    static_cast<const ConfigOptionStrings *>(m_config->option("extruder_colour"))
                        ->values;
                colors[extruder_idx] = "";
        ...
        line.widget = [this, extruder_idx](wxWindow *parent) {
            ScalableButton *btn = new ScalableButton(
                parent, wxID_ANY, "copy", _L("Apply below setting to other extruders"),
            ...
            auto has_changes = [this]() {
                ...
                dirty_options.erase(
                    std::remove_if(
                        dirty_options.begin(), dirty_options.end(),
                ...
                for (const std::string &opt : extruder_options)
                    if (std::find(
                            dirty_options.begin(), dirty_options.end(),
                            opt + "#" + std::to_string(extruder_idx)
                        ) != dirty_options.end())
                        return true;
                return false;
#endif
            ...
            parent->Bind(
                wxEVT_UPDATE_UI,
                [this, has_changes](wxUpdateUIEvent &evt) {
    ...
    if (m_extruders_count < m_extruders_count_old)
        m_pages.erase(
            m_pages.begin() + n_before_extruders + m_extruders_count,
...
void TabPrinter::build_unregular_pages(bool from_initial_build /* = false*/) {
    ...
    bool show_mach_limits =
        (flavor == gcfMarlinLegacy || flavor == gcfMarlinFirmware || flavor == gcfRepRapFirmware ||
         flavor == gcfKlipper);
    ...
    for (size_t i = n_before_extruders; i < m_pages.size();
         ++i) // first make sure it's not there already
    ...
    if (show_mach_limits)
        n_before_extruders++;
    size_t n_after_single_extruder_MM =
        2; //	Count of pages after single_extruder_multi_material page
    ...
    if (from_initial_build ||
        (m_extruders_count > 1 && m_config->opt_bool("single_extruder_multi_material") &&
...
void TabPrinter::update_pages() {
    ...
    const PrinterTechnology new_printer_technology =
        m_presets->get_edited_preset().printer_technology();
...
void TabPrinter::toggle_options() {
    ...
    long val;
    if (m_active_page->title().StartsWith("Extruder ", &extruder_number) &&
        ...
        vec.resize(0);
        vec =
            {"retract_speed", "deretract_speed", "retract_before_wipe", "retract_restart_extra",
             "wipe"};
        for (auto el : vec)
        ...
        toggle_option(
            "retract_restart_extra_toolchange", have_multiple_extruders && toolchange_retraction, i
        );
    }
    ...
    if (m_active_page->title() == "Machine limits" && m_machine_limits_description_line) {
        assert(
            ...
            flavor == gcfRepRapFirmware || flavor == gcfKlipper
        );
...

    {
        ...
        if (m_type == Slic3r::Preset::TYPE_PRINTER) {
            ...
            if (printer_technology != static_cast<TabPrinter *>(this)->m_printer_technology) {
                ...
                // But we don't need it. So, to avoid activation of the page, set m_active_page to
                // NULL till unusable Tabs will be deleted
                ...
                for (auto tab : wxGetApp().tabs_list) {
                    ...
                    if (tab->supports_printer_technology(printer_technology)) {
                        dynamic_cast<TopBar *>(wxGetApp().tab_panel())
                            ->InsertNewPage(
                                wxGetApp().tab_panel()->FindPage(this), tab, tab->title(), ""
                            );
                    } else {
            ...
            if (printer_technology == ptFFF) {
                ...
                static_cast<TabPrinter *>(this)->m_sys_extruders_count = parent_preset == nullptr ?
                    0 :
                    static_cast<const ConfigOptionFloats *>(
                        parent_preset->config.option("nozzle_diameter")
                    )
                        ->values.size();
            }
        } else {
        ...
        m_opt_status_value = (m_presets->get_selected_preset_parent() ? osSystemValue : 0) |
            osInitValue;
        init_options_list();
        update_visibility();
        update_sla_prusa_specific_visibility();
...
void Tab::rebuild_page_tree() {
    ...
    for (auto p : m_pages) {
        if (!p->get_show())
            continue;
        auto itemId =
            m_treectrl->AppendItem(rootItem, translate_category(p->title(), m_type), p->iconID());
...
void Tab::update_btns_enabling() {
    ...
    m_btn_delete_preset->Show(
        is_printer_and_selected_physical || (!preset.is_default && !preset.is_system)
    );

    m_btn_rename_preset->Show(
        !is_printer_and_selected_physical && !preset.is_default && !preset.is_system &&
        !preset.is_external
    );

    if (m_btn_edit_ph_printer)
        m_btn_edit_ph_printer->SetToolTip(
            m_preset_bundle->physical_printers.has_selection() ?
...
bool Tab::select_preset(
        ...
        if (delete_current) {
            ...
            if (idx_new < presets.size())
                for (; idx_new < presets.size() && !presets[idx_new].is_visible; ++idx_new)
                    ;
            if (idx_new == presets.size())
                for (idx_new = idx_current - 1; idx_new > 0 && !presets[idx_new].is_visible;
                     --idx_new)
                    ;
    ...
    assert(
        !delete_current ||
        (m_presets->get_edited_preset().name != preset_name &&
         m_presets->get_edited_preset().is_user())
    );
    bool current_dirty = !delete_current && m_presets->current_is_dirty();
    bool print_tab = m_presets->type() == Preset::TYPE_PRINT ||
        m_presets->type() == Preset::TYPE_SLA_PRINT;
    ...
    if (current_dirty && !may_discard_current_dirty_preset(nullptr, preset_name)) {
        ...
        // Before switching the print profile to a new one, verify, whether the currently active
        // filament or SLA material are compatible with the new print. If it is not compatible and
        // the current filament or SLA material are dirty, let user decide whether to discard the
        // changes or keep the current print selection.
        PresetWithVendorProfile printer_profile =
            m_preset_bundle->printers.get_edited_preset_with_vendor_profile();
        PrinterTechnology printer_technology = printer_profile.preset.printer_technology();
        PresetCollection &dependent = (printer_technology == ptFFF) ?
            m_preset_bundle->filaments :
            m_preset_bundle->sla_materials;
        bool old_preset_dirty = dependent.current_is_dirty();
        bool new_preset_compatible = is_compatible_with_print(
            dependent.get_edited_preset_with_vendor_profile(),
            m_presets->get_preset_with_vendor_profile(*m_presets->find_preset(preset_name, true)),
            printer_profile
        );
        if (!canceled)
        ...
        if (!canceled) {
            ...
            m_dependent_tabs.emplace_back(
                (printer_technology == ptFFF) ?
                    Preset::Type::TYPE_FILAMENT :
        ...
        // Before switching the printer to a new one, verify, whether the currently active print and
        // filament are compatible with the new printer. If they are not compatible and the current
        // print or filament are dirty, let user decide whether to discard the changes or keep the
        ...
        PrinterTechnology old_printer_technology =
            m_presets->get_edited_preset().printer_technology();
        ...
        if (new_printer_technology == ptSLA && old_printer_technology == ptFFF &&
            ...
            for (PresetUpdate &pu : updates) {
                pu.old_preset_dirty = (old_printer_technology == pu.technology) &&
                    pu.presets->current_is_dirty();
                ...
                if (pu.tab_type == Preset::TYPE_FILAMENT && pu.new_preset_compatible) {
                    ...
                    const int active_extruder =
                        dynamic_cast<const TabFilament *>(wxGetApp().get_tab(Preset::TYPE_FILAMENT))
                            ->get_active_extruder();
                    const int extruder_count_new = int(
                        dynamic_cast<const ConfigOptionFloats *>(
                            new_printer_preset.config.option("nozzle_diameter")
                        )
                            ->size()
                    );
                    // if active_extruder is bigger than extruders_count,
                    // then it means that edited filament preset will be changed and we have to
                    // check this changes
                    force_update_edited_preset = active_extruder >= extruder_count_new;
                }
                if (!canceled)
                    canceled = pu.old_preset_dirty &&
                        (!pu.new_preset_compatible || force_update_edited_preset) &&
    ...
    if (!canceled && delete_current) {
        ...
        // It does not matter which preset will be made active as the preset will be re-selected
        // from the preset_name variable. The 'external' presets will only be removed from the
        // preset list, their files will not be deleted.
        try {
    ...
    if (canceled) {
        if (m_type == Preset::TYPE_PRINTER) {
            if (!last_selected_ph_printer_name.empty() &&
                m_presets->get_edited_preset().name ==
                    PhysicalPrinter::get_preset_name(last_selected_ph_printer_name)) {
                // If preset selection was canceled and previously was selected physical printer, we
                // should select it back
        ...
        // Trigger the on_presets_changed event so that we also restore the previous value in the
        // plater selector, if this action was initiated from the plater.
        on_presets_changed();
    } else {
        if (current_dirty)
        ...
        // Mark the print & filament enabled if they are compatible with the currently selected
        // preset. The following method should not discard changes of current print or filament
        // presets on change of a printer profile, if they are compatible with the current printer.
        ...
        if (current_dirty || delete_current || print_tab || printer_tab)
            m_preset_bundle->update_compatible(
                update_compatible_type(
                    ...
                    (print_tab ? this : wxGetApp().get_tab(Preset::TYPE_PRINT))
        ...
        if (!is_selected && printer_tab) {
            ...
            if (printer_technology == ptFFF && m_dependent_tabs.front() != Preset::Type::TYPE_PRINT)
                m_dependent_tabs = {Preset::Type::TYPE_PRINT, Preset::Type::TYPE_FILAMENT};
                     ...
                     m_dependent_tabs.front() != Preset::Type::TYPE_SLA_PRINT)
...
bool Tab::may_discard_current_dirty_preset(
    PresetCollection *presets /*= nullptr*/, const std::string &new_printer_name /*= ""*/
) {
    if (presets == nullptr)
        presets = m_presets;

    UnsavedChangesDialog dlg(m_type, presets, new_printer_name);
    if (wxGetApp().app_config->get("default_action_on_select_preset") == "none" &&
        dlg.ShowModal() == wxID_CANCEL)
        return false;

    if (dlg.save_preset()) // save selected changes
    {
        const std::vector<std::string> &unselected_options = dlg.get_unselected_options(
            presets->type()
        );
        ...
        if (m_type == presets->type()) // save changes for the current preset from this tab
        {
            ...
            presets->get_edited_preset()
                .config.apply_only(presets->get_selected_preset().config, unselected_options);
            save_preset(name);
        } else {
    ...
    } else if (dlg.transfer_changes()) // move selected changes
    {
        ...
        if (m_type == presets->type()) // move changes for the current preset from this tab
        {
            if (m_type == Preset::TYPE_PRINTER) {
                auto it =
                    std::find(selected_options.begin(), selected_options.end(), "extruders_count");
...
bool Tab::tree_sel_change_delayed() {
    // There is a bug related to Ubuntu overlay scrollbars, see
    // https://github.com/prusa3d/PrusaSlicer/issues/898 and
    // https://github.com/prusa3d/PrusaSlicer/issues/952. The issue apparently manifests when
    // Show()ing a window with overlay scrollbars while the UI is frozen. For this reason, we will
    // Thaw the UI prematurely on Linux. This means destroing the no_updates object prematurely.
#ifdef __linux__
    std::unique_ptr<wxWindowUpdateLocker> no_updates(new wxWindowUpdateLocker(this));
#else
    /* On Windows we use DoubleBuffering during rendering,
     * so on Window is no needed to call a Freeze/Thaw functions.
     * But under OSX (builds compiled with MacOSX10.14.sdk) wxStaticBitmap rendering is broken
     * without Freeze/Thaw call.
     */
...
void Tab::OnKeyDown(wxKeyEvent &event) {
    if (event.GetKeyCode() == WXK_TAB)
        m_treectrl->Navigate(
            event.ShiftDown() ? wxNavigationKeyEvent::IsBackward : wxNavigationKeyEvent::IsForward
        );
    else
        event.Skip();
}
...
void Tab::transfer_options(
    const std::string &name_from, const std::string &name_to, std::vector<std::string> options
) {
    if (options.empty())
        return;
...
void Tab::save_preset(std::string name /*= ""*/, bool detach) {
    ...
    if (m_type == Preset::TYPE_FILAMENT && edited_preset.vendor &&
        edited_preset.vendor->templates_profile) {
        edited_printer = wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_string(
            "printer_model"
        );
    ...
    // Mark the print & filament enabled if they are compatible with the currently selected preset.
    // If saving the preset changes compatibility with other presets, keep the now incompatible
    // dependent presets selected, however with a "red flag" icon showing that they are no more
    // compatible.
    m_preset_bundle->update_compatible(PresetSelectCompatibleType::Never);
    ...
    if (m_type == Preset::TYPE_PRINTER)
        static_cast<TabPrinter *>(this)->m_initial_extruders_count =
            static_cast<TabPrinter *>(this)->m_extruders_count;

    // Parent preset is "default" after detaching, so we should to update UI values, related on
    // parent preset
    if (detach)
        update_ui_items_related_on_parent_preset(m_presets->get_selected_preset_parent());
...

    {
        ...
        // Update profile selection combo boxes at the depending tabs to reflect modifications in
...
void Tab::rename_preset() {
    ...
    if (m_type == Preset::TYPE_PRINTER && !m_preset_bundle->physical_printers.empty()) {
        ...
        std::vector<std::string> ph_printers =
            m_preset_bundle->physical_printers.get_printers_with_preset(
        ...
        if (!ph_printers.empty()) {
            ...
            msg += "\n" +
                _L_PLURAL(
                       "Note, that the selected preset will be renamed in this printer too.",
...
void Tab::delete_preset() {
    ...
    if (m_presets_choice->is_selected_physical_printer()) {
        ...
        if (printer.preset_names.size() == 1) {
            if (m_presets_choice->del_physical_printer(
                    _L("It's a last preset for this physical printer.")
                )) {
        ...
        msg = format_wxstr(
            _L("Are you sure you want to delete \"%1%\" preset from the physical printer \"%2%\"?"),
        ...
        if (m_type == Preset::TYPE_PRINTER && !physical_printers.empty()) {
            ...
            // Ask a customer about next action, if there is a printer with just one preset and this
            // preset is equal to delete
            std::vector<std::string> ph_printers =
                physical_printers.get_printers_with_preset(current_preset.name, false);
            std::vector<std::string> ph_printers_only =
                physical_printers.get_printers_with_only_preset(current_preset.name);
        ...
        // TRN "remove/delete"
        msg += from_u8(
            (boost::format(_u8L("Are you sure you want to %1% the selected preset?")) % action).str()
        );
    }
    ...
    if (current_preset.is_default ||
        // wxID_YES != wxMessageDialog(parent(), msg, title, wxYES_NO | wxNO_DEFAULT |
        // wxICON_QUESTION).ShowModal())
        wxID_YES !=
            MessageDialog(parent(), msg, title, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION).ShowModal())
        return;
    ...
    // Select will handle of the preset dependencies, of saving & closing the depending profiles,
    // and finally of deleting the preset.
...
void Tab::update_compatibility_ui() {
    ...
    m_btn_hide_incompatible_presets->SetToolTip(
        m_show_incompatible_presets ?
            "Both compatible an incompatible presets are shown. Click to hide presets not "
            "compatible with the current printer." :
            "Only compatible presets are shown. Click to show both the presets compatible and not "
            "compatible with the current printer."
    );
...
void Tab::create_line_with_widget(
    ConfigOptionsGroup *optgroup, const std::string &opt_key, const std::string &path, widget_t widget
) {
...
// Return a callback to create a Tab widget to mark the preferences as compatible / incompatible to
// the current printer.
wxSizer *Tab::compatible_widget_create(wxWindow *parent, PresetDependencies &deps) {
    deps.checkbox = CheckBox::GetNewWin(parent, _L("All"));
    deps.checkbox->SetFont(Slic3r::GUI::wxGetApp().normal_font());
    wxGetApp().UpdateDarkUI(deps.checkbox);
    deps.btn = new ScalableButton(
        parent, wxID_ANY, "printer", format_wxstr(" %s %s", _L("Set"), dots), wxDefaultSize,
        wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT
    );
...
void SubstitutionManager::init(
    DynamicPrintConfig *config, wxWindow *parent, wxFlexGridSizer *grid_sizer
) {
    m_config = config;
    m_parent = parent;
...
void SubstitutionManager::validate_length() {
    if ((m_substitutions.size() % 4) != 0) {
        WarningDialog(
            m_parent, "Value of gcode_substitutions parameter will be cut to valid length",
...
void SubstitutionManager::delete_substitution(int substitution_id) {
    ...
    std::vector<std::string> &substitutions =
        m_config->option<ConfigOptionStrings>("gcode_substitutions")->values;
    substitutions.erase(
        std::next(substitutions.begin(), substitution_id * 4),
...
void SubstitutionManager::add_substitution(
    int substitution_id,
    ...
    auto add_text_editor =
        [substitution_id, top_sizer, this](const wxString &value, int opt_pos, int proportion) {
            auto editor = new ::TextInput(
                m_parent, value, "", "", wxDefaultPosition, wxSize(15 * m_em, wxDefaultCoord),
                wxTE_PROCESS_ENTER
            );
    ...
    bool case_insensitive = strchr(params.c_str(), 'i') != nullptr ||
        strchr(params.c_str(), 'I') != nullptr;
    bool whole_word = strchr(params.c_str(), 'w') != nullptr ||
        strchr(params.c_str(), 'W') != nullptr;
    bool match_single_line = strchr(params.c_str(), 's') != nullptr ||
        strchr(params.c_str(), 'S') != nullptr;
    ...
    for (wxWindow *chb : std::initializer_list<
             wxWindow *>{chb_regexp, chb_case_insensitive, chb_whole_word, chb_match_single_line}) {
...
void SubstitutionManager::update_from_config() {
    ...
    if (m_substitutions == subst && m_grid_sizer->IsShown(1)) {
        ...
        for (size_t i = 0; i < subst.size(); i += 4) {
            ...
            const bool regexp = strchr(params.c_str(), 'r') != nullptr ||
                strchr(params.c_str(), 'R') != nullptr;
            m_chb_match_single_lines[subst_id++]->Show(regexp);
        }

        // "gcode_substitutions" values didn't changed in config. There is no need to
        // update/recreate controls
        return;
    }
...
void SubstitutionManager::edit_substitution(
    int substitution_id, int opt_pos, const std::string &value
) {
    validate_length();
...
wxSizer *TabPrint::create_manage_substitution_widget(wxWindow *parent) {
    auto create_btn =
        [parent](ScalableButton **btn, const wxString &label, const std::string &icon_name) {
            *btn = new ScalableButton(
    ...
    m_del_all_substitutions_btn->Bind(wxEVT_BUTTON, [this, parent](wxCommandEvent e) {
        if (MessageDialog(
                parent, _L("Are you sure you want to delete all substitutions?"), SLIC3R_APP_NAME,
    ...
    sizer->Add(
        m_del_all_substitutions_btn, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxLEFT, em_unit(parent)
    );
...
wxSizer *TabPrint::create_substitutions_widget(wxWindow *parent) {
    wxFlexGridSizer *grid_sizer = new wxFlexGridSizer(
        2, 5, wxGetApp().em_unit()
    ); // delete_button,  edit column contains "Find", "Replace", "Notes"
...
wxSizer *TabPrinter::create_bed_shape_widget(wxWindow *parent) {
    ScalableButton *btn = new ScalableButton(
        parent, wxID_ANY, "printer", " " + _(L("Set")) + " " + dots, wxDefaultSize,
        wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT
    );
    btn->SetFont(wxGetApp().normal_font());
    btn->SetSize(btn->GetBestSize());
    ...
    btn->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e) {
                  ...
                  dlg.build_dialog(
                      *m_config->option<ConfigOptionPoints>("bed_shape"),
    ...
    // add information about Category/Grope for "bed_custom_texture" and "bed_custom_model" as a
    // copy from "bed_shape" option
    {
...
void TabPrinter::cache_extruder_cnt(const DynamicPrintConfig *config /* = nullptr*/) {
    const DynamicPrintConfig &cached_config = config ?
        *config :
        m_presets->get_edited_preset().config;
    if (Preset::printer_technology(cached_config) == ptSLA)
        return;

    // get extruders count
    auto *nozzle_diameter = dynamic_cast<const ConfigOptionFloats *>(
        cached_config.option("nozzle_diameter")
    );
...
bool Tab::validate_custom_gcodes() {
    if (m_type != Preset::TYPE_FILAMENT &&
        (m_type != Preset::TYPE_PRINTER ||
         static_cast<TabPrinter *>(this)->m_printer_technology != ptFFF))
        return true;
    if (m_active_page->title() != L("Custom G-code"))
        return true;

    // When we switch Settings tab after editing of the custom g-code, then warning message could ba
    // already shown after KillFocus event and then it's no need to show it again
    ...
    for (auto opt_group : m_active_page->m_optgroups) {
        ...
        valid &= validate_custom_gcode(
            opt_group->title, boost::any_cast<std::string>(opt_group->get_value(key))
        );
        if (!valid)
            break;
    }
...
void TabPrinter::update_machine_limits_description(const MachineLimitsUsage usage) {
    ...
    case MachineLimitsUsage::TimeEstimateOnly:
        text = _L(
            ...
            "estimate print time, "
            "which may therefore not be accurate as the printer may apply a different set of "
            "machine limits."
        );
        break;
...
void Tab::fill_icon_descriptions() {
    m_icon_descriptions.emplace_back(
        ...
        L("indicates that the settings are the same as the system (or default) values for the "
    ...
    m_icon_descriptions.emplace_back(
        ...
        L("indicates that some settings were changed and are not equal to the system (or default) "
          "values for "
    ...
    m_icon_descriptions.emplace_back(
        &m_bmp_white_bullet, L("WHITE BULLET"),
    ...
    m_icon_descriptions.emplace_back(
        ...
        // TRN Description for "EDIT VALUE" in the Help dialog (the icon is currently used only to
        // edit custom gcodes).
...
void Tab::set_tooltips_text() {
    ...
    m_ttg_value_lock = _(L(
        "LOCKED LOCK icon indicates that the settings are the same as the "
        "system (or default) values "
        "for the current option group"
    ));
    m_ttg_value_unlock = _(L(
    ...
    m_ttg_white_bullet = _(
        L("WHITE BULLET icon indicates that the settings are the same as in the last saved "
          "preset for the current option group.")
    );
    m_ttg_value_revert = _(L(
    ...
    // Text to be shown on the "Revert to system" aka "Lock to system" button next to each input field.
    m_tt_value_lock = _(
        L("LOCKED LOCK icon indicates that the value is the same as the system (or default) value.")
    );
    m_tt_value_unlock = _(L(
    ...
    // Text to be shown on the "Undo user changes" button next to each input field.
    m_tt_white_bullet = _(
        L("WHITE BULLET icon indicates that the value is the same as in the last saved preset.")
    );
    m_tt_value_revert = _(L(
        "BACK ARROW icon indicates that the value was changed and is not equal "
...
bool Tab::delete_current_preset() { return m_presets->delete_current_preset(); }

Page::Page(wxWindow *parent, const wxString &title, int iconID)
    : m_parent(parent), m_title(title), m_iconID(iconID) {
...
void Page::activate(ConfigOptionMode mode, std::function<void()> throw_if_canceled) {
    for (auto group : m_optgroups) {
        if (!group->activate(throw_if_canceled))
            continue;
        m_vsizer->Add(
            group->sizer, 0, wxEXPAND | (group->is_legend_line() ? (wxLEFT | wxTOP) : wxALL), 10
        );
...
ConfigOptionsGroupShp Page::new_optgroup(const wxString &title, int noncommon_label_width /*= -1*/) {
    ...
    optgroup->get_initial_config = [tab]() { return tab->m_presets->get_selected_preset().config; };
...
void TabSLAMaterial::build() {
    ...
    optgroup->on_change = [this](t_config_option_key opt_key, boost::any value) {
        ...
        if (opt_key == "material_density") {
            double new_bottle_volume = new_conf.option("bottle_weight")->getFloat() /
                boost::any_cast<double>(value) * 1000;
            new_conf.set_key_value("bottle_volume", new ConfigOptionFloat(new_bottle_volume));
        }
    ...
    for (auto &axis : {"X", "Y", "Z"}) {
        auto opt = optgroup->get_option(
            std::string("material_correction_") + char(std::tolower(axis[0]))
        );
        opt.opt.label = axis;
        line.append_option(opt);
    }
...
static void create_tilt_legend(ConfigOptionsGroupShp optgroup) {
    ...
    std::vector<std::pair<std::string, std::string>> columns = {
        // TRN: This is a label of a column of parameters in settings to be used when the area is
    ...
    auto legend = [columns](wxWindow *parent) {
        ...
        legend_sizer->Add(
            new wxStaticText(parent, wxID_ANY, "", wxDefaultPosition, wxSize(25 * em_unit(parent), -1))
        );
        for (auto &[name, tooltip] : columns) {
            auto legend_item = new wxStaticText(
                parent, wxID_ANY, _(name), wxDefaultPosition, wxSize(20 * em_unit(parent), -1)
            );
...
std::vector<std::string> disable_tilt_options =
    {"tilt_down_initial_speed", "tilt_down_offset_steps", "tilt_down_offset_delay",
     "tilt_down_finish_speed",  "tilt_down_cycles",       "tilt_down_delay",
     "tilt_up_initial_speed",   "tilt_up_offset_steps",   "tilt_up_offset_delay",
     "tilt_up_finish_speed",    "tilt_up_cycles",         "tilt_up_delay"};
...
void TabSLAMaterial::toggle_tilt_options(bool is_above) {
    if (m_active_page && m_active_page->title() == "Material printing profile") {
        ...
        bool use_tilt = boost::any_cast<bool>(
            optgroup->get_config_value(*m_config, "use_tilt", column_id)
        );
...
bool Tab::is_prusa_printer() const { return SLAPrint::is_prusa_print(printer_model()); }
...
static void add_options_into_line(
    ConfigOptionsGroupShp &optgroup,
...
void TabSLAPrint::build_sla_support_params(
    const std::vector<SamePair<std::string>> &prefixes, const Slic3r::GUI::PageShp &page
) {
...
static std::vector<std::string> get_override_opt_kyes_for_line(
...
void TabSLAMaterial::create_line_with_near_label_widget(
    ConfigOptionsGroupShp optgroup, const std::string &key
) {
    if (optgroup->title == "Support head" || optgroup->title == "Support pillar")
        add_options_into_line(
            optgroup, {{"", L("Default")}, {"branching", L("Branching")}}, key, "material_ow_"
        );
    else {
    ...
    line->near_label_widget =
        [this, optgroup_wk = ConfigOptionsGroupWkp(optgroup), key](wxWindow *parent) {
            wxWindow *check_box = CheckBox::GetNewWin(parent);
            wxGetApp().UpdateDarkUI(check_box);
            ...
            check_box->Bind(wxEVT_CHECKBOX, [this, optgroup_wk, key](wxCommandEvent &evt) {
                ...
                if (auto optgroup_sh = optgroup_wk.lock(); optgroup_sh) {
                    auto opt_keys =
                        get_override_opt_kyes_for_line(optgroup_sh->title.ToStdString(), key);
...
void TabSLAMaterial::update_line_with_near_label_widget(
    ConfigOptionsGroupShp optgroup, const std::string &key, bool is_checked /* = true*/
) {
...
void TabSLAPrint::update_description_lines() {
    ...
    if (m_active_page && m_active_page->title() == "Supports") {
        ...
        if (m_support_object_elevation_description_line) {
            ...
            if (is_visible) {
                bool elev = !m_config->opt_bool("pad_enable") ||
                    !m_config->opt_bool("pad_around_object");
                m_support_object_elevation_description_line->SetText(
                    elev ?
                        "" :
                        format_wxstr(
                            _L("\"%1%\" is disabled because \"%2%\" is on in \"%3%\" category.\n"
