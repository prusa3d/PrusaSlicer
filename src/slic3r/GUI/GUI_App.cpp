///|/ Copyright (c) Prusa Research 2018 - 2023 Vojtěch Bubník @bubnikv, Oleksandra Iushchenko
///@YuSanka, Lukáš Matěna @lukasmatena, Tomáš Mészáros @tamasmeszaros, David Kocík @kocikdav, Enrico
///Turri @enricoturri1966, Lukáš Hejl @hejllukas, Filip Sykala @Jony01, Vojtěch Král @vojtechkral
...
public:
    SplashScreen(
        const wxBitmap &bitmap, long splashStyle, int milliseconds, wxPoint pos = wxDefaultPosition
    )
        : wxSplashScreen(
              bitmap,
              splashStyle,
    ...
    void Decorate(wxBitmap &bmp) {
        ...
        m_action_line_y_position = logo_and_header_height +
            0.5 *
                (bmp.GetHeight() - margin - credits_height - logo_and_header_height - text_height);
    }
...
private:
    ...
    void init_constant_text() {
        ...
        int text_banner_width = lround(0.4 * m_main_bitmap.GetWidth()) -
            roundl(m_scale * 50); // banner_width - margins

        float title_font_scale = (float) text_banner_width /
            GetTextExtent(m_constant_text.title).GetX();
        ...
        float version_font_scale = (float) text_banner_width /
            GetTextExtent(m_constant_text.version).GetX();
        scale_font(m_constant_text.version_font, version_font_scale > 2.f ? 2.f : version_font_scale);

        // The width of the credits information string doesn't respect to the banner width some
        // times. So, scale credits_font in the respect to the longest string width
        ...
        if (fs::is_directory(old_path)) {
            wxString msg = from_u8(
                (boost::format(_u8L("Starting with %1% 2.3, configuration "
                                    "directory on Linux has changed (according to XDG Base "
                                    "Directory Specification) to \n%2%.\n\n"
                                    "This directory did not exist yet (maybe you run the new "
                                    "version for the first time).\nHowever, "
                                    "an old %1% configuration directory was detected in \n%3%.\n\n"
                                    "Consider moving the contents of the old directory to the new "
                                    "location in order to access "
                                    "your profiles, etc.\nNote that if you decide to downgrade %1% "
                                    "in future, it will use the old "
                                    "location again.\n\n"
                                    "What do you want to do now?")) %
                 SLIC3R_APP_NAME % new_path % old_path)
                    .str()
            );
            wxString caption = from_u8(
                (boost::format(_u8L("%s - BREAKING CHANGE")) % SLIC3R_APP_NAME).str()
            );
...
static const FileWildcards file_wildcards_by_type[FT_SIZE] = {
    ...
    /* FT_MODEL */
    {"Known files"sv,
     {".stl"sv, ".obj"sv, ".3mf"sv, ".amf"sv, ".zip.amf"sv, ".xml"sv, ".step"sv, ".stp"sv, ".svg"sv}
    },
    ...
    {"Masked SLA files"sv, {".sl1"sv, ".sl1s"sv, ".pwmx"sv, ".ctb"sv}},

    /* FT_ZIP */ {"Zip files"sv, {".zip"sv}},
};

// This function produces a Win32 file dialog file template mask to be consumed by wxWidgets on all
// platforms. The function accepts a custom extension parameter. If the parameter is provided, the
// custom extension will be added as a fist to the list. This is important for a "file save" dialog
// on OSX, which strips an extension from the provided initial file name and substitutes it with the
// default extension (the first one in the template).
...
static GUID GUID_DEVINTERFACE_HID = {0x4D1E55B2, 0xF16F, 0x11CF, 0x88, 0xCB, 0x00,
...
static void register_win32_device_notification_event() {
    wxWindow::MSWRegisterMessageHandler(
        WM_DEVICECHANGE,
        [](wxWindow *win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
            // Some messages are sent to top level windows by default, some messages are sent to
            // only registered windows, and we explictely register on MainFrame only.
            ...
            case DBT_DEVICEARRIVAL:
                if (lpdb->dbch_devicetype == DBT_DEVTYP_VOLUME)
                    plater->GetEventHandler()->AddPendingEvent(
                        VolumeAttachedEvent(EVT_VOLUME_ATTACHED)
                    );
            ...
            case DBT_DEVICEREMOVECOMPLETE:
                if (lpdb->dbch_devicetype == DBT_DEVTYP_VOLUME)
                    plater->GetEventHandler()->AddPendingEvent(
                        VolumeDetachedEvent(EVT_VOLUME_DETACHED)
                    );
                else if (lpdb->dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE) {
                    PDEV_BROADCAST_DEVICEINTERFACE lpdbi = (PDEV_BROADCAST_DEVICEINTERFACE) lpdb;
                    //				if (lpdbi->dbcc_classguid == GUID_DEVINTERFACE_VOLUME)
        ...
        [](wxWindow *win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
            ...
            if (wParam == RIM_INPUT && plater != nullptr && main_frame->IsActive()) {
                ...
                if (raw.header.dwType == RIM_TYPEHID &&
                    plater->get_mouse3d_controller()
                        .handle_raw_input_win32(raw.data.hid.bRawData, raw.data.hid.dwSizeHid))
                    return true;
            }
            return false;
        }
    ...
    wxWindow::MSWRegisterMessageHandler(
        ...
        [](wxWindow *win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
            ...
            if (copy_data_structure->dwData == 1) {
                ...
                Slic3r::GUI::wxGetApp().other_instance_message_handler()->handle_message(
                    into_u8(arguments)
                );
            }
            return true;
...
static void generic_exception_handle() {
    ...
    // This is why we use wxLogError() here as well instead of a custom dialog, because it
    // accumulates errors if multiple have been collected and displays just one error message for
    // all of them. Otherwise we would get multiple error messages for one missing png, for example.
    //
    // If a custom error message window (or some other solution) were to be used, it would be
    // necessary to turn off wxLogError() usage in wx APIs, most notably in wxImage
    // - see https://docs.wxwidgets.org/trunk/classwx_image.html#aa32e5d3507cc0f8c3330135bc0befc6a

    try {
        throw;
    } catch (const std::bad_alloc &ex) {
        // bad_alloc in main thread is most likely fatal. Report immediately to the user (wxLogError
        // would be delayed) and terminate the app so it is at least certain to happen now.
        wxString errmsg = wxString::Format(
            _L("%s has encountered an error. It was likely caused by running out of memory. "
               "If you are sure you have enough RAM on your system, this may also be a bug and we "
               "would "
        ...
        wxString errmsg = _L(
            "PrusaSlicer has encountered a localization error. "
...
void GUI_App::post_init() {
    ...
    if (this->is_gcode_viewer()) {
        ...
        if (!this->init_params->load_configs.empty())
            ...
            // FIXME this is not strictly correct, as one may pass a print/filament/printer profile
            // here instead of a full config.
...
#if 0 // This code was moved to EVT_CONFIG_UPDATER_SYNC_DONE bind - after preset_updater finishes
      // synchronization.
...
#endif
        CallAfter([this] {
            // preset_updater->sync downloads profile updates and than via event checks updates and
            // incompatible presets. We need to run it on startup. start before cw so it is canceled
            // by cw if needed?
            this->preset_updater->sync(
                preset_bundle, this,
                std::move(
...
#else
    return m_opengl_mgr.init_glcontext(
        canvas, init_params != nullptr ? init_params->opengl_version : std::make_pair(0, 0),
...
void GUI_App::init_app_config() {
    ...
    if (!app_config)
        app_config = new AppConfig(
            is_editor() ? AppConfig::EAppMode::Editor : AppConfig::EAppMode::GCodeViewer
        );
    ...
    if (m_app_conf_exists) {
        ...
        if (!error.empty()) {
            // Error while parsing config file. We'll customize the error message and rethrow to be
            // displayed.
            if (is_editor()) {
                throw Slic3r::RuntimeError(format(
                    "Error parsing PrusaSlicer config file, it is probably corrupted. "
                    "Try to manually delete the file to recover from the error. Your user profiles "
                ...
                throw Slic3r::RuntimeError(format(
                    "Error parsing PrusaGCodeViewer config file, it is probably corrupted. "
...
void copy_vendor_ini(const std::vector<std::string> &vendors) {
    for (const std::string &vendor : vendors) {
        boost::system::error_code ec;
        const boost::filesystem::path ini_in_resources = boost::filesystem::path(
                                                             Slic3r::resources_dir()
                                                         ) /
            "profiles" / (vendor + ".ini");
        assert(boost::filesystem::exists(ini_in_resources));
        const boost::filesystem::path ini_in_vendors = boost::filesystem::path(Slic3r::data_dir()) /
            "vendor" / (vendor + ".ini");
        ...
        std::string message;
        CopyFileResult cfr =
            copy_file(ini_in_resources.string(), ini_in_vendors.string(), message, false);
        if (cfr != SUCCESS) {
            BOOST_LOG_TRIVIAL(error) << "Failed to copy file " << ini_in_resources << " to "
                                     << ini_in_vendors << ": " << message;
        }
    }
}
} // namespace
...
void GUI_App::legacy_app_config_vendor_check() {
    ...
    // We expect this is detected and fixed on the first run, when PrusaResearchSLA is not installed
    // yet. Steps: Move the printers in appconfig to PrusaResearchSLA Moving the printers is not
    // enough. The new ini PrusaResearchSLA needs to be installed. But we cannot install bundles
    // without preset updater. So we just move it to the vendor folder. Since all profiles are named
    // the same, it should not be a problem. Preset updater should be doing blocking update over
    // PrusaResearch.ini. Then all should be ok.
    ...
    const std::map<std::string, std::map<std::string, std::set<std::string>>> &vendor_map =
        app_config->vendors();
    for (const auto &moved_models_of_vendor : moved_models) {
        if (const auto &vendor_it = vendor_map.find(moved_models_of_vendor.first);
            vendor_it != vendor_map.end()) {
    ...
    if (vendors_to_create.empty()) {
        // If there are no printers to move, also do check if "new" vendors really has ini file in
        // vendor folder. In case of running older and current slicer back and forth, there might be
        // vendors in appconfig without ini.
    ...
    BOOST_LOG_TRIVIAL(warning) << "PrusaSlicer has found legacy SLA printers. The printers will be "
                                  "moved to new vendor and its ini file will be installed. "
                                  "Configuration snapshot will be taken.";

    // Take snapshot now, since creation of new vendors in appconfig, snapshots wont be compatible
    // in older slicers.
    // If any of the new vendors already is in appconfig, there is no reason to do a snapshot, it
    // will fail or wont be compatible in previous version.
    ...
    if (do_snapshot) {
        GUI::Config::take_config_snapshot_report_error(
            *app_config, Config::Snapshot::SNAPSHOT_UPGRADE, ""
        );
    }
    ...
    for (const auto &vendor : vendor_map) {
        for (const auto &model : vendor.second) {
            if (vendors_from_to.find(vendor.first) != vendors_from_to.end() &&
                std::find(
                    moved_models[vendor.first].begin(), moved_models[vendor.first].end(), model.first
                ) != moved_models[vendor.first].end()) {
...
std::string GUI_App::check_older_app_config(Semver current_version, bool backup) {
        ...
        msg(nullptr,
            ...
            backup ? format_wxstr(
                         _L("The active configuration was created by <b>%1% %2%</b>,"
                            ...
                            "\nIf so, your active configuration will be backed up before importing "
                            "the new configuration."),
                         SLIC3R_APP_NAME, current_version.to_string(), older_data_dir_path,
                         last_semver.to_string()
                     ) :
                     format_wxstr(
                         _L("An existing configuration was found in <b>%3%</b>"
                            "\ncreated by <b>%1% %2%</b>."
    ...
    if (msg.ShowModal() == wxID_YES) {
        ...
        if (backup) {
            ...
            if (!GUI::Config::take_config_snapshot_cancel_on_error(
                    *app_config, Config::Snapshot::SNAPSHOT_USER, "",
                    _u8L("Continue and import newer configuration?"), &snapshot
                ))
                return {};
            if (snapshot) {
                // Save snapshot ID before loading the alternate AppConfig, as loading the alternate
                // AppConfig may fail.
        ...
        std::string error = app_config->load(
            (boost::filesystem::path(older_data_dir_path) / filename).string()
        );
        if (!error.empty()) {
            // Error while parsing config file. We'll customize the error message and rethrow to be
            // displayed.
...
void GUI_App::init_single_instance_checker(const std::string &name, const std::string &path) {
    ...
    m_single_instance_checker = std::make_unique<
        wxSingleInstanceChecker>(boost::nowide::widen(name), boost::nowide::widen(path));
}
...
void GUI_App::jump_to_option(
    const std::string &opt_key, Preset::Type type, const std::wstring &category
) {
...
void GUI_App::update_search_lines() { mainframe->update_search_lines(m_searcher->search_string()); }
...
bool GUI_App::on_init_inner() {
        ...
        RichMessageDialog
            dlg(nullptr,
                _L("You are running a 32 bit build of PrusaSlicer on 64-bit Windows."
                   ...
                   "\nPlease download and install a 64 bit build of PrusaSlicer from "
    ...
    wxCHECK_MSG(
        wxDirExists(resources_dir), false,
    ...
    // Enable this to disable Windows Vista themes for all wxNotebooks. The themes seem to lead to
    // terrible performance when working on high resolution multi-display setups.
    //    wxSystemOptions::SetOption("msw.notebook.themed-background", 0);

    //     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;

    // !!! Initialization of UI settings as a language, application color mode, fonts... have to be
    // done before first UI action. Like here, before the show InfoDialog in check_older_app_config()
    ...
    std::string older_data_dir_path;
    if (m_app_conf_exists) {
        if (app_config->orig_version().valid() &&
            app_config->orig_version() < *Semver::parse(SLIC3R_VERSION)) {
...
#ifdef _MSW_DARK_MODE
    // app_config can be updated in check_older_app_config(), so check if dark_color_mode and
    // sys_menu_enabled was changed
    ...
    if (is_editor()) {
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr, wxString::Format(_L("%s\nDo you want to continue?"), msg),
                    "PrusaSlicer", wxICON_QUESTION | wxYES_NO);
            ...
            app_config->set(
                "tls_accepted_cert_store_location",
    ...
    if (app_config->get_bool("show_splash_screen")) {
        ...
        wxBitmap bmp = SplashScreen::MakeBitmap(wxBitmap(
            from_u8(var(is_editor() ? "splashscreen.jpg" : "splashscreen-gcodepreview.jpg")),
            wxBITMAP_TYPE_JPEG
        ));
        ...
        scrn = new SplashScreen(
            bmp.IsOk() ? bmp :
                         get_bmp_bundle("PrusaSlicer", 400)->GetPreferredBitmapSizeAtScale(1.0),
    ...
    if (is_editor()) {
        ...
        Bind(EVT_SLIC3R_EXPERIMENTAL_VERSION_ONLINE, [this](const wxCommandEvent &evt) {
            if (this->plater_ != nullptr &&
                (m_app_updater->get_triggered_by_user() ||
                 app_config->get("notify_release") == "all")) {
                std::string evt_string = into_u8(evt.GetString());
                if (*Semver::parse(SLIC3R_VERSION) < *Semver::parse(evt_string)) {
                    auto notif_type =
                        (evt_string.find("beta") != std::string::npos ?
    ...
    try {
        // Enable all substitutions (in both user and system profiles), but log the substitutions in
        // user profiles only. If there are substitutions in system profiles, then a "reconfigure"
        // event shall be triggered, which will force installation of a compatible system preset,
        // thus nullifying the system preset substitutions.
        init_params->preset_substitutions = preset_bundle->load_presets(
            *app_config, ForwardCompatibilitySubstitutionRule::EnableSystemSilent
        );
    } catch (const std::exception &ex) { delayed_error_load_presets = ex.what(); }
    ...
    // Call this check only after appconfig was loaded to mainframe, otherwise there will be
    // duplicity error.
    legacy_app_config_vendor_check();

    sidebar().obj_list()->init_objects(); // propagate model objects to object list
    update_mode(
    ); // mode sizer doesn't exist anymore, so we came update mode here, before load_current_presets
    ...
    Bind(wxEVT_IDLE, [this](wxIdleEvent &event) {
        ...
        // An ugly solution to GH #5537 in which GUI_App::init_opengl (normally called from events
        // wxEVT_PAINT and wxEVT_SET_FOCUS before GUI_App::post_init is called) wasn't called before
        // GUI_App::post_init and OpenGL wasn't initialized. Since issue #9774 Where same problem
        // occured on MacOS Ventura, we decided to have this check on MacOS as well.
    ...
    if (const std::string &crash_reason = app_config->get("restore_win_position");
        ...
        InfoDialog dialog(
            nullptr, _L("PrusaSlicer started after a crash"),
            format_wxstr(
                _L("PrusaSlicer crashed last time when attempting to set window position.\n"
...
void GUI_App::init_ui_colours() {
    ...
    m_color_highlight_label_default = is_dark_mode ?
...
void GUI_App::UpdateDarkUI(wxWindow *window, bool highlited /* = false*/, bool just_font /* = false*/) {
    ...
    if (wxButton *btn = dynamic_cast<wxButton *>(window)) {
        ...
        // button marking
        if (!dynamic_cast<TopBarItemsCtrl *>(window->GetParent()
            )) // don't marking the button if it is from TopBar
        {
            auto mark_button = [this, btn, highlited](const bool mark) {
                if (btn->GetLabel().IsEmpty())
                    btn->SetBackgroundColour(
                        mark          ? m_color_selected_btn_bg :
...
#ifdef __APPLE__
    // This is a limit imposed by OSX. The way the native button widget is drawn only allows it to
    // be stretched horizontally, and the vertical size is fixed. (see
    // https://stackoverflow.com/questions/29083891/wxpython-button-size-being-ignored-on-osx) But
    // standard height is possible to change using SetWindowVariant method (see
...
void GUI_App::set_mode_palette(const std::vector<wxColour> &palette) {
    ...
    for (size_t mode = 0; mode < palette.size(); ++mode) {
        ...
        std::string color_str = clr == wxTransparentColour ?
            std::string("") :
...
void GUI_App::check_printer_presets() {
    ...
    wxString msg_text =
        _L("You have the following presets with saved options for \"Print Host upload\"") + ":";
    for (const std::string &preset_name : preset_names)
        msg_text += "\n    \"" + from_u8(preset_name) + "\",";
    msg_text.RemoveLast();
    msg_text += "\n\n" +
        _L("But since this version of PrusaSlicer we don't show this information in Printer "
           "Settings anymore.\n"
...
static void update_scrolls(wxWindow *window) {
    ...
    while (node) {
        ...
        if (dynamic_cast<wxScrollHelper *>(win) || dynamic_cast<wxTreeCtrl *>(win) ||
...
void GUI_App::load_project(wxWindow *parent, wxString &input_file) const {
    input_file.Clear();
    wxFileDialog dialog(
        parent ? parent : GetTopWindow(), _L("Choose one file (3MF/AMF):"),
        app_config->get_last_dir(), "", file_wildcards(FT_PROJECT), wxFD_OPEN | wxFD_FILE_MUST_EXIST
    );
...
void GUI_App::import_zip(wxWindow *parent, wxString &input_file) const {
    wxFileDialog dialog(
        parent ? parent : GetTopWindow(), _L("Choose ZIP file") + ":",
...
void GUI_App::load_gcode(wxWindow *parent, wxString &input_file) const {
    input_file.Clear();
    wxFileDialog dialog(
        parent ? parent : GetTopWindow(), _L("Choose one file (GCODE/GCO/G/BGCODE/BGC/NGC):"),
...
static const wxLanguageInfo *linux_get_existing_locale_language(
    ...
    if (system_language) {
        ...
        std::string system_country = "_" +
            into_u8(system_language->CanonicalName.AfterFirst('_')).substr(0, 2);
...
int GUI_App::GetSingleChoiceIndex(
...
bool GUI_App::select_language() {
    ...
    std::sort(
        language_infos.begin(), language_infos.end(),
        [](const wxLanguageInfo *l, const wxLanguageInfo *r) {
    ...
    const long index = GetSingleChoiceIndex(
        _L("Select the language"), _L("Language"), names, init_selection_default
    );
    ...
    if (index != -1 && (init_selection == -1 || init_selection != index)) {
        ...
        if (this->load_language(new_language_info->CanonicalName, false)) {
            ...
            // 1) Hopefully the language set to wxTranslations by this->load_language(), but that API
            // is weird and we don't want to rely on its
...
bool GUI_App::load_language(wxString language, bool initial) {
    if (initial) {
        ...
        // Get the system language.
        {
            ...
            if (lang_system != wxLANGUAGE_UNKNOWN) {
                ...
                BOOST_LOG_TRIVIAL(trace)
                    << boost::format("System language detected (user locales and such): %1%") %
                        m_language_info_system->CanonicalName.ToUTF8().data();
            }
        }
        {
            // Allocating a temporary locale will switch the default wxTranslations to its internal
            // wxTranslations instance.
            wxLocale temp_locale;
#ifdef __WXOSX__
            // ysFIXME - temporary workaround till it isn't fixed in wxWidgets:
            // Use English as an initial language, because of under OSX it try to load "inappropriate"
            // language for wxLANGUAGE_DEFAULT. For example in our case it's trying to load "en_CZ"
            // and as a result PrusaSlicer catch warning message. But wxWidgets guys work on it.
            temp_locale.Init(wxLANGUAGE_ENGLISH);
#else
            temp_locale.Init();
#endif // __WXOSX__
       // Set the current translation's language to default, otherwise GetBestTranslation() may not
       // work (see the wxWidgets source code).
            wxTranslations::Get()->SetLanguage(wxLANGUAGE_DEFAULT);
            // Let the wxFileTranslationsLoader enumerate all translation dictionaries for PrusaSlicer
            // and try to match them with the system specific "preferred languages".
            // There seems to be a support for that on Windows and OSX, while on Linuxes the code just
            // returns wxLocale::GetSystemLanguage(). The last parameter gets added to the list of
            // detected dictionaries. This is a workaround for not having the English dictionary. Let's
            // hope wxWidgets of various versions process this call the same way.
            wxString best_language = wxTranslations::Get()
                                         ->GetBestTranslation(SLIC3R_APP_KEY, wxLANGUAGE_ENGLISH);
            if (!best_language.IsEmpty()) {
                m_language_info_best = wxLocale::FindLanguageInfo(best_language);
                BOOST_LOG_TRIVIAL(trace) << boost::format("Best translation language detected (may "
                                                          "be different from user locales): %1%") %
    ...
    const wxLanguageInfo *language_info = language.empty() ? nullptr :
                                                             wxLocale::FindLanguageInfo(language);
    if (!language.empty() && (language_info == nullptr || language_info->CanonicalName.empty())) {
        // Fix for wxWidgets issue, where the FindLanguageInfo() returns locales with undefined
        // ANSII code (wxLANGUAGE_KONKANI or wxLANGUAGE_MANIPURI).
        language_info = nullptr;
        BOOST_LOG_TRIVIAL(error) << boost::format("Language code \"%1%\" is not supported") %
                language.ToUTF8().data();
    }

    if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
        BOOST_LOG_TRIVIAL(trace
        ) << boost::format("The following language code requires right to left layout, which is "
                           "not supported by PrusaSlicer: %1%") %
    ...
    if (language_info == nullptr) {
        ...
        if (m_language_info_system != nullptr &&
            m_language_info_system->LayoutDirection != wxLayout_RightToLeft)
            language_info = m_language_info_system;
        if (m_language_info_best != nullptr &&
            m_language_info_best->LayoutDirection != wxLayout_RightToLeft)
    ...
    if (language_info == m_language_info_best || language_info == m_language_info_system) {
        // The current language matches user's default profile exactly. That's great.
               ...
               language_info->CanonicalName.BeforeFirst('_') ==
                   m_language_info_best->CanonicalName.BeforeFirst('_')) {
        // Use whatever the operating system recommends, if it the language code of the dictionary
        // matches the recommended language. This allows a Swiss guy to use a German dictionary
        // without forcing him to German locales.
        language_info = m_language_info_best;
    ...
    if (!wxLocale::IsAvailable(language_info->Language)) {
        ...
        BOOST_LOG_TRIVIAL(trace
        ) << boost::format("Can't switch language to %1% (missing locales). Using %2% instead.") %
    ...
    if (!wxLocale::IsAvailable(language_info->Language)) {
        ...
        wxString message = "Switching PrusaSlicer to language " + language_info->CanonicalName +
            " failed.";
    ...
    // FIXME wxWidgets cause havoc if the current locale is deleted. We just forget it causing
    ...
    // FIXME This is a temporary workaround, the correct solution is to switch to "C" locale during
    // file import / export only. wxSetlocale(LC_NUMERIC, "C");
...
wxMenu *GUI_App::get_config_menu(MainFrame *main_frame) {
    ...
    const wxString config_wizard_tooltip = from_u8(
        (boost::format(_u8L("Run %s")) % config_wizard_name).str()
    );
    // Cmd+, is standard on OS X - what about other operating systems?
    if (is_editor()) {
        local_menu->Append(
            config_id_base + ConfigMenuWizard, config_wizard_name + dots, config_wizard_tooltip
        );
        local_menu->Append(
            config_id_base + ConfigMenuSnapshots, _L("&Configuration Snapshots") + dots,
            _L("Inspect / activate configuration snapshots")
        );
        local_menu->Append(
            config_id_base + ConfigMenuTakeSnapshot, _L("Take Configuration &Snapshot"),
            _L("Capture a configuration snapshot")
        );
        local_menu->Append(
            config_id_base + ConfigMenuUpdateConf, _L("Check for Configuration Updates"),
            _L("Check for configuration updates")
        );
        local_menu->Append(
            config_id_base + ConfigMenuUpdateApp, _L("Check for Application Updates"),
            _L("Check for new version of application")
        );
#if defined(__linux__) && defined(SLIC3R_DESKTOP_INTEGRATION)
        // if (DesktopIntegrationDialog::integration_possible())
        local_menu->Append(
            config_id_base + ConfigMenuDesktopIntegration, _L("Desktop Integration"),
...
#ifdef __APPLE__
    local_menu->Append(
        config_id_base + ConfigMenuPreferences, _L("&Preferences") + dots + "\tCtrl+,",
        _L("Application preferences")
    );
#else
    append_menu_item(
        local_menu, config_id_base + ConfigMenuPreferences, _L("&Preferences") + "\tCtrl+P",
        _L("Application preferences"), [](wxCommandEvent &) { wxGetApp().open_preferences(); }, "",
        nullptr, []() { return true; }, main_frame
    );
#endif
    ...
    if (is_editor()) {
        local_menu->AppendSeparator();
        local_menu->Append(
            config_id_base + ConfigMenuFlashFirmware, _L("Flash Printer &Firmware"),
            _L("Upload a firmware image into an Arduino based printer")
        );
        // TODO: for when we're able to flash dictionaries
        // local_menu->Append(config_id_base + FirmwareMenuDict,  _L("Flash Language File"),
        // _L("Upload a language dictionary file into a Prusa printer"));
    }
    local_menu->Append(
        config_id_base + ConfigMenuWifiConfigFile, _L("Wi-Fi Configuration File"),
    ...
    local_menu->Bind(wxEVT_MENU, [this, config_id_base](wxEvent &event) {
        ...
        case ConfigMenuTakeSnapshot:
            ...
            if (wxString action_name = _L("Taking a configuration snapshot");
                check_and_save_current_preset_changes(
                    action_name,
                    _L("Some presets are modified and the unsaved changes will not be captured by "
        ...
        case ConfigMenuSnapshots:
            if (check_and_save_current_preset_changes(
                    _L("Loading a configuration snapshot"), "", false
                )) {
                ...
                if (!dlg.snapshot_to_activate().empty()) {
                    if (!Config::SnapshotDB::singleton().is_on_snapshot(*app_config) &&
                        !Config::take_config_snapshot_cancel_on_error(
                            *app_config, Config::Snapshot::SNAPSHOT_BEFORE_ROLLBACK, "",
                            GUI::format(
                                _L("Continue to activate a configuration snapshot %1%?"),
                    ...
                    try {
                        ...
                        // Enable substitutions, log both user and system substitutions. There
                        // should not be any substitutions performed when loading system presets
                        // because compatibility of profiles shall be verified using the
                        // min_slic3r_version keys in config index, but users are known to be
                        ...
                        // Load the currently selected preset into the GUI, update the preset
                        // selection box.
                        load_current_presets();
                    } catch (std::exception &ex) {
                        GUI::show_error(
                            nullptr,
                            _L("Failed to activate configuration snapshot.") + "\n" +
                                into_u8(ex.what())
                        );
                    }
                }
...
bool GUI_App::has_unsaved_preset_changes() const {
    PrinterTechnology printer_technology = preset_bundle->printers.get_edited_preset()
                                               .printer_technology();
...
// - Taking snapshot
bool GUI_App::check_and_save_current_preset_changes(
    const wxString &caption,
    ...
    if (has_current_preset_changes()) {
        ...
        std::string act = app_config_key.empty() ? "none" :
                                                   wxGetApp().app_config->get(app_config_key);
        ...
        if (dlg.save_preset()) // save selected changes
        {
            for (const std::pair<std::string, Preset::Type> &nt : dlg.get_names_and_types())
                preset_bundle->save_changes_for_preset(
                    nt.first, nt.second, dlg.get_unselected_options(nt.second)
                );
            ...
            MessageDialog(
                nullptr, dlg.msg_success_saved_modifications(dlg.get_names_and_types().size())
            )
                .ShowModal();
        }
    }
...
// New Project          => Current project is saved    => UnsavedChangesDialog: "Keep / Discard /
// Cancel"
//                      => Current project isn't saved => UnsavedChangesDialog: "Keep / Discard /
//                      Save / Cancel"
// Close ConfigWizard   => Current project is saved    => UnsavedChangesDialog: "Keep / Discard /
// Save / Cancel" Note: no_nullptr postponed_apply_of_keeped_changes indicates that thie function is
// called after ConfigWizard is closed
bool GUI_App::check_and_keep_current_preset_changes(
    const wxString &caption,
    const wxString &header,
    int action_buttons,
    ...
    if (has_current_preset_changes()) {
        ...
        const std::string app_config_key = is_called_from_configwizard ?
            "" :
        ...
        else // save selected changes
        {
            ...
            if (dlg.save_preset()) {
                ...
                reset_modifications();
                       ...
                       (dlg.has_unselected_options() || is_called_from_configwizard)) {
                // execute this part of code only if not all modifications are keeping to the new
                // project OR this function is called when ConfigWizard is closed and "Keep
                ...
                for (const std::pair<std::string, Preset::Type> &nt : preset_names_and_types) {
                    ...
                    if (type == Preset::TYPE_PRINTER) {
                        auto it = std::find(
                            selected_options.begin(), selected_options.end(), "extruders_count"
                        );
...
bool GUI_App::can_load_project() {
    ...
    if (saved_project == wxID_CANCEL ||
        (plater()->is_project_dirty() && saved_project == wxID_NO &&
         !check_and_save_current_preset_changes(
             _L("Project is loading"), _L("Opening new project while some presets are unsaved.")
         )))
        return false;
    return true;
}
...
bool GUI_App::check_print_host_queue() {
    ...
    MessageDialog dialog(
        mainframe, message, wxString(SLIC3R_APP_NAME) + " - " + _(L("Ongoing uploads")),
    ...
    for (Tab *tab : tabs_list)
        if (tab->supports_printer_technology(printer_technology)) {
            if (tab->type() == Preset::TYPE_PRINTER) {
                ...
                // Mark the plater to update print bed by tab->load_current_preset() from
                // Plater::on_config_change().
                this->plater()->force_print_bed_update();
            } else if (tab->type() == Preset::TYPE_FILAMENT)
                // active extruder can be changed in a respect to the new loaded configurations, if
                // some filament preset will be modified
    ...
    if (boost::starts_with(narrow_url, "prusaslicer://open?file=")) {
        ...
        if (app_config && !app_config->get_bool("downloader_url_registered")) {
            ...
            BOOST_LOG_TRIVIAL(error)
                << "Recieved command to open URL, but it is not allowed in app configuration. URL: "
                << url;
            return;
        }
...
void GUI_App::open_web_page_localized(const std::string &http_address) {
    open_browser_with_warning_dialog(
        http_address + "&lng=" + this->current_language_code_safe(), nullptr, false
    );
}

// If we are switching from the FFF-preset to the SLA, we should to control the printed objects if
// they have a part(s). Because of we can't to print the multi-part objects with SLA technology.
bool GUI_App::may_switch_to_SLA_preset(const wxString &caption) {
    if (model_has_parameter_modifiers_in_objects(model())) {
        show_info(
            nullptr,
            _L("It's impossible to print object(s) which contains parameter modifiers with SLA "
               "technology.") +
...
bool GUI_App::run_wizard(ConfigWizard::RunReason reason, ConfigWizard::StartPage start_page) {
    ...
    auto cw_loading_dlg =
        new ConfigWizardLoadingDialog(mainframe, _L("Loading Configuration Wizard..."));
    ...
    if (reason == ConfigWizard::RunReason::RR_USER) {
        ...
        const SharedArchiveRepositoryVector &repos =
            plater()->get_preset_archive_database()->get_selected_archive_repositories();
        preset_updater->sync_blocking(preset_bundle, this, repos);
        preset_updater->update_index_db();
        // Offer update installation.
        preset_updater->config_update(
            app_config->orig_version(), PresetUpdater::UpdateParams::SHOW_TEXT_BOX, repos
        );
    }
    ...
    if (res) {
        ...
        // #ysFIXME - delete after testing: This part of code looks redundant. All checks are inside
        // ConfigWizard::priv::apply_config()
...
void GUI_App::show_downloader_registration_dialog() {
    InfoDialog
        msg(nullptr,
            ...
            format_wxstr(
                _L("Do you wish to register downloads from <b>Printables.com</b>"
...
void GUI_App::gcode_thumbnails_debug() {
    ...
    wxFileDialog dialog(
        GetTopWindow(), _L("Select a gcode file:"), "", "",
        "G-code files (*.gcode)|*.gcode;*.GCODE;", wxFD_OPEN | wxFD_FILE_MUST_EXIST
    );
    if (dialog.ShowModal() != wxID_OK)
        return;
    ...
    std::string out_path =
        boost::filesystem::path(in_filename).remove_filename().append(L"thumbnail").string();
        ...
        while (std::getline(in_file, gcode_line)) {
            if (in_file.good()) {
                if (boost::starts_with(gcode_line, BEGIN_MASK)) {
                    ...
                    std::string out_filename = out_path + std::to_string(width) + "x" +
                    ...
                    if (out_file.good()) {
                        ...
                        decoded.resize(boost::beast::detail::base64::decode(
                                           (void *) &decoded[0], row.data(), row.size()
                        )
                                           .first);
...
void GUI_App::window_pos_restore(
    wxTopLevelWindow *window, const std::string &name, bool default_maximized
) {
    if (name.empty()) {
        return;
    }
...
#ifndef __APPLE__
    else if (is_editor() && m_last_app_conf_lower_version &&
...
bool GUI_App::check_updates(const bool invoked_by_user) {
    if (invoked_by_user) {
        // do preset_updater sync so if user runs slicer for a long time, check for updates actually
        // delivers updates. for preset_updater sync we need to sync archive database first
        plater()->get_preset_archive_database()->sync_blocking();
...
bool GUI_App::open_browser_with_warning_dialog(
    const wxString &url,
    ...
    if (force_remember_choice || app_config->get(option_key).empty()) {
        if (app_config->get(option_key).empty()) {
            RichMessageDialog dialog(
                parent, _L("Open hyperlink in default browser?"), _L("PrusaSlicer: Open hyperlink"),
            ...
            if (dialog.IsCheckBoxChecked()) {
                ...
                wxString msg =
                    ...
                    format_wxstr(
                        _L("Visit \"Preferences\" and check \"%1%\"\nto changes your choice."),
                        preferences_item
                    );

                MessageDialog msg_dlg(
                    parent, msg, _L("PrusaSlicer: Don't ask me again"),
...
bool GUI_App::open_login_browser_with_dialog(
    const wxString &url, wxWindow *parent /* = nullptr*/, int flags /* = 0*/
) {
    bool auth_login_dialog_confirmed = app_config->get_bool("auth_login_dialog_confirmed");
    if (!auth_login_dialog_confirmed) {
        RichMessageDialog dialog(
            parent,
            _L("Open default browser with Prusa Account Log in page?\n(If you select 'Yes', you "
               "will not be asked again.)"),
            _L("PrusaSlicer: Open Log in page"), wxICON_QUESTION | wxYES_NO
        );
        if (dialog.ShowModal() != wxID_YES)
            return false;
...
//     if (!frame->IsActive())
//         frame->RequestUserAttention(defined(__WXOSX__/*&Wx::wxMAC */)? wxUSER_ATTENTION_ERROR :
...
void GUI_App::on_version_read(wxCommandEvent &evt) {
    ...
    if (this->plater_ == nullptr ||
        (!m_app_updater->get_triggered_by_user() && opt != "all" && opt != "release")) {
        BOOST_LOG_TRIVIAL(info) << "Version online: " << evt.GetString()
    ...
    if (*Semver::parse(SLIC3R_VERSION) >= *Semver::parse(into_u8(evt.GetString()))) {
        if (m_app_updater->get_triggered_by_user()) {
            std::string text = (*Semver::parse(into_u8(evt.GetString())) == Semver()) ?
                ...
                Slic3r::format(
                    _u8L("You are currently running the latest released version %1%."),
                    evt.GetString()
                );

            if (*Semver::parse(SLIC3R_VERSION) > *Semver::parse(into_u8(evt.GetString())))
                text = Slic3r::format(
                    _u8L("There are no new released versions online. The latest release version is "
                         "%1%."),
                    evt.GetString()
                );
    ...
    this->plater_->get_notification_manager()->push_notification(NotificationType::NewAppAvailable
        ...
        , [](wxEvtHandler* evnthndlr)
    {wxGetApp().open_web_page_localized("https://www.prusa3d.com/slicerweb"); return true; }
    );
    */
    // updater
    ...
    if (from_user) {
        if (m_app_updater->get_download_ongoing()) {
            MessageDialog msgdlg(
                nullptr,
                _L("Downloading of the new version is in progress. Do you want to continue?"),
                _L("Notice"), wxYES_NO
            );
...
void GUI_App::start_download(std::string url) {
    ...
    if (app_config && !app_config->get_bool("downloader_url_registered")) {
        notification_manager()->push_notification(NotificationType::URLNotRegistered);
        BOOST_LOG_TRIVIAL(error
        ) << "Received command to open URL, but it is not allowed in app configuration. URL: "
          << url;
        return;
    }
    ...
    if (dest_folder.empty() || !boost::filesystem::is_directory(dest_folder)) {
        std::string msg = _u8L("Could not start URL download. Destination folder is not set. "
                               "Please choose destination folder in Configuration Wizard.");
        BOOST_LOG_TRIVIAL(error) << msg;
...
void GUI_App::open_wifi_config_dialog(bool forced, const wxString &drive_path /* = {}*/) {
    ...
    if (!forced && dialog_was_declined) {
        ...
        notification_manager()->push_notification(
            ...
            NotificationManager::NotificationLevel::ImportantNotificationLevel
            // TRN Text of notification when Slicer starts and usb stick with printer settings ini
            // file is present
            ,
            _u8L("Printer configuration file detected on removable media.")
            // TRN Text of hypertext of notification when Slicer starts and usb stick with printer
            // settings ini file is present
            ,
...
bool GUI_App::select_printer_preset(const Preset *preset) {
    ...
    // TabPresetComboBox::update() looks at physical_printers and if some has selected = true, it
    // overrides the selection. This might be, because OnSelect event callback is not triggered
...
bool GUI_App::select_printer_from_connect(const std::string &msg) {
    ...
    std::string model_name =
        plater()->get_user_account()->get_keyword_from_json(msg, "printer_model");
    std::string uuid = plater()->get_user_account()->get_keyword_from_json(msg, "uuid");
    if (model_name.empty()) {
        std::vector<std::string> compatible_printers;
        plater()
            ->get_user_account()
            ->fill_supported_printer_models_from_json(msg, compatible_printers);
    ...
    if (model_name.empty()) {
        BOOST_LOG_TRIVIAL(error
        ) << "Failed to select printer from Connect. Printer_model is empty.";
        return false;
    }
    ...
    BOOST_LOG_TRIVIAL(info) << "Select printer from Connect. Model: " << model_name
                            << "nozzle: " << nozzle;
    // select printer
    const Preset *printer_preset = preset_bundle->printers
                                       .find_system_preset_by_model_and_variant(model_name, nozzle);
    bool is_installed = printer_preset && select_printer_preset(printer_preset);
    // notification
    std::string out = printer_preset ?
        (is_installed ?
             GUI::format(_L("Installed and selected printer:\n%1%"), printer_preset->name) :
             GUI::format(_L("Selected printer:\n%1%"), printer_preset->name)) :
        GUI::format(_L("Printer not found:\n%1%"), model_name);
    this->plater()->get_notification_manager()->close_notification_of_type(
        NotificationType::SelectPrinterFromConnect
    );
    this->plater()->get_notification_manager()->push_notification(
        NotificationType::SelectPrinterFromConnect,
        printer_preset ? NotificationManager::NotificationLevel::ImportantNotificationLevel :
...
void GUI_App::search_and_select_filaments(
    const std::string &material, size_t extruder_index, std::string &out_message
) {
    ...
    for (const auto &filament : preset_bundle->extruders_filaments[extruder_index]) {
     ...
     : */ GUI::format(
                    _L("Extruder %1%: Selected filament %2%"), extruder_index + 1,
    ...
    for (const auto &filament : preset_bundle->extruders_filaments[extruder_index]) {
        if (filament.is_compatible && !filament.preset->is_default &&
            ...
            filament.preset->name.compare(0, 9, "Prusament") == 0 &&
            select_filament_preset(filament.preset, extruder_index)) {
            out_message += GUI::format(
                               _L("Extruder %1%: Installed and selected filament %2%"),
    ...
    out_message += GUI::format(
                       _L("Extruder %2%: Failed to find and select filament type: %1%"), material,
                       extruder_index + 1
                   ) +
        "\n";
}
...
void GUI_App::select_filament_from_connect(const std::string &msg) {
    ...
    if (extruder_count != materials.size()) {
        BOOST_LOG_TRIVIAL(error) << format(
            "Failed to select filament from Connect. Selected printer has %1% extruders while data "
            "from Connect contains %2% materials.",
            extruder_count, materials.size()
        );
        return;
    }
    ...
    tab->select_preset(
        preset_bundle->extruders_filaments[tab->get_active_extruder()].get_selected_preset_name()
    );

    if (!notification_text.empty()) {
        plater()->get_notification_manager()->close_notification_of_type(
            NotificationType::SelectFilamentFromConnect
        );
...
void GUI_App::handle_connect_request_printer_select(const std::string &msg) {
    ...
    // Lets queue it and wait on result. The result is send via event to plater, where it is send to
    // handle_connect_request_printer_select_inner
    std::string uuid = plater()->get_user_account()->get_keyword_from_json(msg, "uuid");
...
void GUI_App::show_printer_webview_tab() {
    mainframe->show_printer_webview_tab(
