...
#include "slic3r/GUI/Gizmos/GLGizmoTextureSkin.hpp"
...
bool GLGizmosManager::init()
{
    ...
    m_gizmos.emplace_back(new GLGizmoTextureSkin(m_parent, "texture_skin_painting.svg", 10));
...
bool GLGizmosManager::gizmo_event(SLAGizmoEventType action, const Vec2d& mouse_position, bool shift_down, bool alt_down, bool control_down)
{
    ...
    else if (m_current == TextureSkin)
        return dynamic_cast<GLGizmoTextureSkin*>(m_gizmos[TextureSkin].get())->gizmo_event(action, mouse_position, shift_down, alt_down, control_down);
    else
        return false;
}
...
bool GLGizmosManager::on_mouse_wheel(const wxMouseEvent &evt)
{
    ...
    if (m_current == SlaSupports || m_current == Hollow || m_current == FdmSupports || m_current == Seam || m_current == MmSegmentation || m_current == FuzzySkin || m_current == TextureSkin) {
...
bool GLGizmosManager::on_char(wxKeyEvent& evt)
{
    ...
    if ((evt.GetModifiers() & ctrlMask) != 0) {
        ...
        case 'R' :
        {
            if ((m_current == SlaSupports || m_current == Hollow || m_current == FdmSupports || m_current == Seam || m_current == MmSegmentation || m_current == FuzzySkin || m_current == TextureSkin) && gizmo_event(SLAGizmoEventType::ResetClippingPlane))
                processed = true;
