...
void Plater::clear_before_change_volume(ModelVolume &mv, const std::string &notification_msg) {
    ...
    if (const bool paint_removed = !mv.supported_facets.empty() || !mv.seam_facets.empty() || !mv.mm_segmentation_facets.empty() || !mv.fuzzy_skin_facets.empty() || !mv.texture_skin_facets.empty() || !mv.surface_texture_facets.empty(); paint_removed) {
        mv.supported_facets.reset();
        mv.seam_facets.reset();
        mv.mm_segmentation_facets.reset();
        mv.fuzzy_skin_facets.reset();
        mv.texture_skin_facets.reset();
        mv.surface_texture_facets.reset();
...
void Plater::clear_before_change_mesh(int obj_idx, const std::string &notification_msg)
{
    ...
    for (ModelVolume *mv : mo->volumes) {
        paint_removed |= !mv->supported_facets.empty() || !mv->seam_facets.empty() || !mv->mm_segmentation_facets.empty() || !mv->fuzzy_skin_facets.empty() || !mv->texture_skin_facets.empty() || !mv->surface_texture_facets.empty();
        mv->supported_facets.reset();
        mv->seam_facets.reset();
        mv->mm_segmentation_facets.reset();
        mv->fuzzy_skin_facets.reset();
