...
public:
    UploadOptionsDialog(
        ...
        singleRadio = new wxRadioButton(
            this, wxID_ANY, "Single file", wxDefaultPosition, wxDefaultSize, wxRB_GROUP
        );
        ...
        projectsComboBox = new wxComboBox(
            this, wxID_ANY, wxString(""), wxDefaultPosition, wxDefaultSize, 0, nullptr,
            DD_NO_CHECK_ICON
        );
        printerTypeComboBox = new wxComboBox(
            this, wxID_ANY, wxString(""), wxDefaultPosition, wxDefaultSize, 0, nullptr,
            DD_NO_CHECK_ICON | wxTE_READONLY
        );

        printerWarningLabel =
            new wxStaticText(this, wxID_ANY, "Printer type not found, please select manually.");
        ...
        if (cloud_printer_types.size() > 0) {
            for (int i = 0; i < cloud_printer_types.size(); i++) {
                ...
                if (cloud_printer_types[i].Find(preset_name) != wxNOT_FOUND &&
                    printerTypeComboBox->GetSelection() == -1) {
    ...
    void SendRequest() {
        ...
        http.on_error([this](std::string, std::string error, unsigned status) {
                if (!m_cancelled) {
                    m_resp.put("result", false);
                    m_resp.put(
                        "message", (status != 200) ? "HTTP error: " + std::to_string(status) : error
                    );
                    wxQueueEvent(this, new wxThreadEvent(wxEVT_THREAD, wxID_ABORT));
                }
            }
            ...
            if (!m_cancelled) {
                ...
                if (m_resp.get<bool>("result", false) &&
                    m_resp.get_optional<std::string>("message.session").has_value()) {
...
static const int API_SESSION_WAIT_TIMEOUT = 60;
    ...
    
{
    m_api_session_file_path = (boost::filesystem::path(Slic3r::data_dir()) / API_CREDENTIALS_PATH)
                                  ...
                                  .string();
    load_api_session();
    m_preset_name = extract_model_name(std::string(config->opt_string("preset_name")));
}
...
wxString C3DPrinterOS::get_test_ok_msg() const {
    return _("Connection to 3DPrinterOS cloud works correctly.") +
        (!m_username.empty() ? "" + _(" Logined as user: ") + m_username : "");
}
...
bool C3DPrinterOS::upload(
    ...
    UploadOptionsDialog dlg(nullptr, cloud_projects_list, cloud_printer_types_list, wxString((m_preset_name).c_str()));
    ...
    http.form_add("session", m_apikey)
        ...
        .form_add("upload_soft_name", SLIC3R_BUILD_ID)
        .form_add("zip", "false")
    ...
    http.on_complete([&](std::string body, unsigned status) {
        ...
        .on_error([&](std::string body, std::string error, unsigned status) {
            BOOST_LOG_TRIVIAL(error)
                << boost::format("%1%: Error uploading file: %2%, HTTP %3%, body: `%4%`") % name %
                    error % status % body;
        ...
        .on_progress([&](Http::Progress progress, bool &cancel) {
            ...
            if (cancel) {
                BOOST_LOG_TRIVIAL(error) << "3DPrinterOS: Upload canceled";
                res = false;
            }
        })
        .perform_sync();
    ...
    if (res) {
        ...
        update_file(update_file_response, file_id, printer_type_id, SLIC3R_BUILD_ID);
        try {
            if (!update_file_response.get<bool>("result")) {
                const std::string msg =
                    update_file_response.get<std::string>("message", "Unknown update error");
...
void C3DPrinterOS::set_auth(Http &http) const {
...
std::string C3DPrinterOS::get_api_auth_token(wxString &err) const {
    std::string result;
    pt::ptree resp;
    std::string postBody = "app_type=plugin&app_name=" + Http::url_encode(SLIC3R_BUILD_ID);
...
std::string C3DPrinterOS::extract_model_name(const std::string &input) const {
    std::smatch matches;
    // search for prusa
    std::vector<std::string> patterns =
        {R"(Original Prusa i3 MK2)", R"(Original Prusa XL)", R"(Original Prusa MINI)",
         R"(Original Prusa MK4)", R"(Original Prusa MK4S)", R"(Prusa CORE One)"};
    ...
    std::regex pattern(
        R"(^(.*?)\s*(?:\(|\||\b)\s*\d+(\.\d+)?\s*mm|\d+(\.\d+)?\s*Nozzle)", std::regex::icase
    );
...
bool C3DPrinterOS::save_api_session(const std::string &session, const std::string &email) const {
    ...
    j.put("email", email);
    try {
        pt::write_json(m_api_session_file_path, j);
...
void C3DPrinterOS::get_cloud_printer_types(boost::property_tree::ptree &response, const std::string &query) const {
    std::string postBody = std::string("session=" + m_apikey);
    if (!query.empty()) {
        postBody += "&description=" + Http::url_encode(query) + "&software_version=" + Http::url_encode(SLIC3R_BUILD_ID);
    }
...
void C3DPrinterOS::log_out() const { boost::filesystem::remove(m_api_session_file_path.c_str()); }
