...
///@lukasmatena, Tomáš Mészáros @tamasmeszaros, Lukáš Hejl @hejllukas, Vojtěch Bubník @bubnikv,
/// Pavel Mikuš @Godrak, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Vojtěch Král
/// @vojtechkral
...
ConfigFileType guess_config_file_type(const ptree &tree) {
    ...
    for (const ptree::value_type &v : tree) {
        if (v.second.empty()) {
            if (v.first == "background_processing" || v.first == "last_output_path" ||
                v.first == "no_controller" || v.first == "no_defaults")
                ++app_config;
    ...
    return (app_config > bundle && app_config > config) ?
...
VendorProfile VendorProfile::from_ini(
    const ptree &tree, const boost::filesystem::path &path, bool load_all
) {
    ...
    if (!boost::filesystem::exists(path)) {
        throw Slic3r::RuntimeError(
            (boost::format("Cannot load Vendor Config Bundle `%1%`: File not found: `%2%`.") % id %
             path)
                .str()
        );
    }
    ...
    auto get_or_throw =
        [&](const ptree &tree, const std::string &key) -> ptree::const_assoc_iterator {
        auto res = tree.find(key);
        if (res == tree.not_found()) {
            throw Slic3r::RuntimeError(
                (boost::format(
                     "Vendor Config Bundle `%1%` is not valid: Missing secion or key: `%2%`."
                 ) %
                 id % key)
                    .str()
            );
    ...
    if (!config_version) {
        throw Slic3r::RuntimeError(
            (boost::format(
                 "Vendor Config Bundle `%1%` is not valid: Cannot parse config_version: `%2%`."
             ) %
             id % config_version_str)
                .str()
        );
    } else {
    ...
    for (auto &section : tree) {
        if (boost::starts_with(section.first, printer_model_key)) {
            ...
            const char *technology_fallback = boost::algorithm::starts_with(model.id, "SL") ?
                "SLA" :
                "FFF";

            auto technology_field =
                section.second.get<std::string>("technology", technology_fallback);
            if (!ConfigOptionEnum<PrinterTechnology>::from_string(technology_field, model.technology)) {
                BOOST_LOG_TRIVIAL(
                    error
                ) << boost::format("Vendor bundle: `%1%`: Invalid printer technology field: `%2%`") %
                        id % technology_field;
            ...
            if (Slic3r::unescape_strings_cstyle(variants_field, variants)) {
                ...
                BOOST_LOG_TRIVIAL(error)
                    << boost::format("Vendor bundle: `%1%`: Malformed variants field: `%2%`") % id %
                        variants_field;
            }
            ...
            if (Slic3r::unescape_strings_cstyle(default_materials_field, model.default_materials)) {
                ...
                BOOST_LOG_TRIVIAL(
                    error
                ) << boost::format("Vendor bundle: `%1%`: Malformed default_materials field: `%2%`") %
...
void Preset::normalize(DynamicPrintConfig &config) {
    ...
    if (config.option("filament_diameter") != nullptr) {
        ...
        // The following keys are mandatory for the UI, but they are not part of FullPrintConfig,
    ...
    if (const auto *gap_fill_speed = config.option<ConfigOptionFloat>("gap_fill_speed", false);
        ...
        if (auto *gap_fill_enabled = config.option<ConfigOptionBool>("gap_fill_enabled", false);
            gap_fill_enabled)
            gap_fill_enabled->value = false;
    }
    if (auto *first_layer_height =
            config.option<ConfigOptionFloatOrPercent>("first_layer_height", false);
        first_layer_height && first_layer_height->percent)
        if (const auto *layer_height = config.option<ConfigOptionFloat>("layer_height", false);
            layer_height) {
            // Legacy conversion - first_layer_height moved from PrintObject setting to a Print
            // setting, thus we are getting rid of the dependency of first_layer_height on
            // PrintObject specific layer_height. Covert the first layer heigth to an absolute
            // value.
...
std::string Preset::remove_invalid_keys(
    DynamicPrintConfig &config, const DynamicPrintConfig &default_config
) {
...
void Preset::save() { this->config.save(this->file); }

// Return a label of this preset, consisting of a name and a "(modified)" suffix, if this preset is dirty.
std::string Preset::label() const { return this->name + (this->is_dirty ? g_suffix_modified : ""); }
...
bool is_compatible_with_print(
    ...
    if (preset.vendor != nullptr && preset.vendor != active_printer.vendor &&
        !preset.vendor->templates_profile)
    ...
    if (!has_compatible_prints && !condition.empty()) {
        try {
            return PlaceholderParser::evaluate_boolean_expression(
                condition, active_print.preset.config
            );
        } catch (const std::runtime_error &err) {
            // FIXME in case of an error, return "compatible with everything".
            printf(
                "Preset::is_compatible_with_print - parsing error of compatible_prints_condition "
                "%s:\n%s\n",
    ...
    return preset.preset.is_default || active_print.preset.name.empty() || !has_compatible_prints ||
        std::find(
            compatible_prints->values.begin(), compatible_prints->values.end(),
            active_print.preset.name
        ) != compatible_prints->values.end();
}

bool is_compatible_with_printer(
    const PresetWithVendorProfile &preset,
    const PresetWithVendorProfile &active_printer,
    ...
    if (!has_compatible_printers && !condition.empty()) {
        try {
            return PlaceholderParser::evaluate_boolean_expression(
                condition, active_printer.preset.config, extra_config
            );
        } catch (const std::runtime_error &err) {
            // FIXME in case of an error, return "compatible with everything".
            printf(
                "Preset::is_compatible_with_printer - parsing error of "
                "compatible_printers_condition %s:\n%s\n",
                active_printer.preset.name.c_str(), err.what()
            );
            return true;
        }
    }
    return preset.preset.is_default || active_printer.preset.name.empty() ||
        !has_compatible_printers ||
        std::find(
            compatible_printers->values.begin(), compatible_printers->values.end(),
            active_printer.preset.name
        ) != compatible_printers->values.end();
}

bool is_compatible_with_printer(
    const PresetWithVendorProfile &preset, const PresetWithVendorProfile &active_printer
) {
...
void Preset::set_visible_from_appconfig(const AppConfig &app_config) {
    ...
    if (type == TYPE_PRINTER) {
        ...
        const std::string &section_name = (type == TYPE_FILAMENT) ?
            AppConfig::SECTION_FILAMENTS :
            AppConfig::SECTION_MATERIALS;
        if (type == TYPE_FILAMENT && app_config.get_bool("no_templates") && vendor &&
            vendor->templates_profile)
            is_visible = false;
        else if (app_config.has_section(section_name)) {
            // Check whether this profile is marked as "installed" in PrusaSlicer.ini,
            // or whether a profile is marked as "installed", which this profile may have been
            // renamed from.
            const std::map<std::string, std::string> &installed = app_config.get_section(
                section_name
            );
            ...
            for (auto it = this->renamed_from.begin();
                 !is_visible && it != this->renamed_from.end(); ++it)
                is_visible = has(*it);
        } else
            is_visible = false;
    }
...
std::string Preset::trim_vendor_repo_prefix(
    const std::string &id, const VendorProfile *vendor_profile
) const {
    if (!vendor_profile) {
        return id;
    }
...
static std::vector<std::string> s_Preset_filament_options{
    "filament_colour", "filament_diameter", "filament_type", "filament_soluble",
    "filament_abrasive", "filament_notes", "filament_max_volumetric_speed",
    "filament_infill_max_speed", "filament_infill_max_crossing_speed", "extrusion_multiplier",
    "filament_density", "filament_cost", "filament_spool_weight", "filament_loading_speed",
    "filament_loading_speed_start", "filament_load_time", "filament_unloading_speed",
    "filament_unloading_speed_start", "filament_unload_time", "filament_toolchange_delay",
    ...
    "filament_multitool_ramming_flow", "temperature", "idle_temperature", "first_layer_temperature",
    "bed_temperature", "first_layer_bed_temperature", "fan_always_on", "cooling",
    "cooling_slowdown_logic", "cooling_perimeter_transition_distance", "min_fan_speed",
    "max_fan_speed", "bridge_fan_speed", "disable_fan_first_layers", "full_fan_speed_layer",
    "fan_below_layer_time", "slowdown_below_layer_time", "min_print_speed",
    "custom_parameters_filament", "start_filament_gcode", "end_filament_gcode",
    "enable_dynamic_fan_speeds", "chamber_temperature", "chamber_minimal_temperature",
    ...
    "filament_retract_lift_below", "filament_retract_speed", "filament_deretract_speed",
    ...
    "filament_travel_ramping_lift", "filament_travel_slope", "filament_travel_max_lift",
...
static std::vector<std::string> s_Preset_printer_options{
    "printer_technology", "autoemit_temperature_commands", "bed_shape", "bed_custom_texture",
    "bed_custom_model", "binary_gcode", "z_offset", "gcode_flavor", "use_relative_e_distances",
    "use_firmware_retraction", "use_volumetric_e", "variable_layer_height",
    "prefer_clockwise_movements",
    // FIXME the print host keys are left here just for conversion from the Printer preset to
    // Physical Printer preset.
    "host_type", "print_host", "printhost_apikey", "printhost_cafile",
    "single_extruder_multi_material", "custom_parameters_printer", "start_gcode", "end_gcode",
    "before_layer_gcode", "layer_gcode", "toolchange_gcode", "color_change_gcode",
    "pause_print_gcode", "template_custom_gcode", "between_objects_gcode", "printer_vendor",
    "printer_model", "printer_variant", "printer_notes", "cooling_tube_retraction",
    "cooling_tube_length", "high_current_on_filament_swap", "parking_pos_retraction",
    "extra_loading_move", "multimaterial_purging", "max_print_height", "default_print_profile",
    "inherits", "remaining_times", "silent_mode", "machine_limits_usage", "thumbnails",
    "thumbnails_format", "nozzle_high_flow", "extruder_clearance_radius", "extruder_clearance_height"
};
...
static std::vector<std::string> s_Preset_printer_options{
    ...
    // Physical Printer preset.
    "host_type", "print_host", "printhost_apikey", "printhost_cafile",
    "single_extruder_multi_material", "start_gcode", "end_gcode", "before_layer_gcode",
    "layer_gcode", "toolchange_gcode", "color_change_gcode", "pause_print_gcode",
    "template_custom_gcode", "between_objects_gcode", "printer_vendor", "printer_model",
    "printer_variant", "printer_notes", "cooling_tube_retraction", "cooling_tube_length",
    "high_current_on_filament_swap", "parking_pos_retraction", "extra_loading_move",
    "multimaterial_purging", "max_print_height", "default_print_profile", "inherits",
    "remaining_times", "silent_mode", "machine_limits_usage", "thumbnails", "thumbnails_format"
};
...
static std::vector<std::string> s_Preset_sla_print_options{
    ...
    "support_max_bridges_on_pillar", "support_max_weight_on_model",
    ...
    "support_base_safety_distance", "support_critical_angle", "support_max_bridge_length",
    ...
    "branchingsupport_small_pillar_diameter_percent", "branchingsupport_max_bridges_on_pillar",
    ...
    "branchingsupport_base_diameter", "branchingsupport_base_height",
    "branchingsupport_base_safety_distance", "branchingsupport_critical_angle",
    "branchingsupport_max_bridge_length", "branchingsupport_max_pillar_link_distance",
    "branchingsupport_object_elevation",

    "support_points_density_relative", "support_points_minimal_distance", "slice_closing_radius",
    "slicing_mode", "pad_enable", "pad_wall_thickness", "pad_wall_height", "pad_brim_size",
    "pad_max_merge_distance",
    // "pad_edge_radius",
    "pad_wall_slope", "pad_object_gap", "pad_around_object", "pad_around_object_everywhere",
    "pad_object_connector_stride", "pad_object_connector_width", "pad_object_connector_penetration",
    "hollowing_enable", "hollowing_min_thickness", "hollowing_quality",
    "hollowing_closing_distance", "output_filename_format", "default_sla_print_profile",
    "compatible_printers", "compatible_printers_condition", "inherits"
};

static std::vector<std::string> s_Preset_sla_material_options{
    "material_colour", "material_type", "initial_layer_height", "bottle_cost", "bottle_volume",
    "bottle_weight", "material_density", "exposure_time", "initial_exposure_time",
    "bot_light_off_time", "light_off_time", "bot_lift_distance", "lift_distance", "bot_lift_speed",
    "lift_speed", "bot_retract_height", "retract_height", "sla_bot_retract_speed",
    "sla_retract_speed", "bot_light_intensity", "light_intensity", "rest_time_after_lift",
    "rest_time_after_retract", "tsmc_bot_enable", "tsmc_enable", "tsmc_bot_lift_distance",
    "tsmc_lift_distance", "tsmc_bot_lift_speed", "tsmc_lift_speed", "tsmc_bot_retract_height",
    "tsmc_retract_height", "tsmc_sla_retract_speed", "tsmc_sla_bot_retract_speed",
    "tsmc_rest_time_after_lift", "material_correction", "material_correction_x",
    "material_correction_y", "material_correction_z", "material_notes", "material_vendor",
    "material_print_speed", "area_fill", "default_sla_material_profile", "zcorrection_layers",
    "compatible_prints", "compatible_prints_condition", "compatible_printers",
    "compatible_printers_condition", "inherits",

    // overriden options
    "material_ow_support_head_front_diameter", "material_ow_support_head_penetration",
    "material_ow_support_head_width", "material_ow_support_pillar_diameter",

    "material_ow_branchingsupport_head_front_diameter",
    "material_ow_branchingsupport_head_penetration", "material_ow_branchingsupport_head_width",
...
static std::vector<std::string> s_Preset_sla_material_options_all =
...
static std::vector<std::string> s_Preset_sla_printer_options{
    "printer_technology", "bed_shape", "bed_custom_texture", "bed_custom_model", "max_print_height",
    "display_width", "display_height", "display_pixels_x", "display_pixels_y", "display_mirror_x",
    "display_mirror_y", "display_orientation", "fast_tilt_time", "slow_tilt_time",
    "high_viscosity_tilt_time", //"area_fill",
    ...
    "max_initial_exposure_time", "sla_archive_format", "sla_output_precision",
    // FIXME the print host keys are left here just for conversion from the Printer preset to
    // Physical Printer preset.
...
void PresetCollection::load_presets(
    ...
    // Don't use boost::filesystem::canonical() on Windows, it is broken in regard to reparse
    // points, see https://github.com/prusa3d/PrusaSlicer/issues/732
    boost::filesystem::path dir =
        boost::filesystem::absolute(boost::filesystem::path(dir_path) / subdir).make_preferred();
    m_dir_path = dir.string();
    std::string errors_cummulative;
    // Store the loaded presets into a new vector, otherwise the binary search for already existing
    // presets would be broken. (see the "Preset already present, not loading" message).
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(dir))
        if (Slic3r::is_ini_file(dir_entry)) {
            ...
            if (this->find_preset(name, false)) {
                // This happens when there's is a preset (most likely legacy one) with the same name
                // as a system preset that's already been loaded from a bundle.
            ...
            try {
                ...
                try {
                    ...
                    ConfigSubstitutions config_substitutions =
                        config.load_from_ini(preset.file, substitution_rule);
                    if (!config_substitutions.empty())
                        substitutions.push_back(
                            {preset.name, m_type, PresetConfigSubstitutions::Source::UserFile,
                             preset.file, std::move(config_substitutions)}
                        );
                    // Find a default preset for the config. The PrintPresetCollection provides
                    // different default preset based on the "printer_technology" field.
                    ...
                    std::string incorrect_keys =
                        Preset::remove_invalid_keys(preset.config, default_preset.config);
                    if (!incorrect_keys.empty())
                        BOOST_LOG_TRIVIAL(error)
    ...
    m_presets.insert(
        m_presets.end(), std::make_move_iterator(presets_loaded.begin()),
...
Preset &PresetCollection::load_preset(
    const std::string &path, const std::string &name, const DynamicPrintConfig &config, bool select
) {
...
static bool profile_print_params_same(
    const DynamicPrintConfig &cfg_old, const DynamicPrintConfig &cfg_new
) {
    ...
    for (const char *key :
         {"compatible_prints", "compatible_prints_condition", "compatible_printers",
          "compatible_printers_condition", "inherits", "print_settings_id", "filament_settings_id",
          "sla_print_settings_id", "sla_material_settings_id", "printer_settings_id",
          "filament_vendor", "printer_model", "printer_variant", "default_print_profile",
...
// Load a preset from an already parsed config file, insert it into the sorted sequence of presets
// and select it, losing previous modifications.
// Only a single profile could be edited at at the same time, which introduces complexity when
// loading filament profiles for multi-extruder printers.
ExternalPreset PresetCollection::load_external_preset(
    // Path to the profile source file (a G-code, an AMF or 3MF file, a config file)
    const std::string &path,
    // Name of the profile, derived from the source file name.
    const std::string &name,
    // Original name of the profile, extracted from the loaded config. Empty, if the name has not
    // been stored.
    const std::string &original_name,
    // Config to initialize the preset from. It may contain configs of all presets merged in a
    // single dictionary!
    const DynamicPrintConfig &combined_config,
    // Select the preset after loading?
    LoadAndSelect select
) {
    // Load the preset over a default preset, so that the missing fields are filled in from the
    // default preset.
    ...
    if (select == LoadAndSelect::Never) {
        ...
        if ((edited.name == original_name || edited.name == inherits) &&
            profile_print_params_same(edited.config, cfg))
    ...
    if (found) {
        if (select != LoadAndSelect::Never) {
            ...
            if (!profile_print_params_same(it->config, cfg)) {
                // The source config may contain keys from many possible preset types. Just copy
                // those that relate to this preset.

                // Following keys are not used neither by the UI nor by the slicing core, therefore
                // they are not important Erase them from config apply to avoid redundant "dirty"
                // parameter in loaded preset.
                for (const char *key :
                     {"print_settings_id", "filament_settings_id", "sla_print_settings_id",
                      "sla_material_settings_id", "printer_settings_id", "filament_vendor",
                      "printer_model", "printer_variant", "default_print_profile",
                      "default_filament_profile", "default_sla_print_profile",
    ...
    Preset &preset =
        this->load_preset(path, new_name, std::move(cfg), select == LoadAndSelect::Always);
...
Preset &PresetCollection::load_preset(
    const std::string &path, const std::string &name, DynamicPrintConfig &&config, bool select
) {
...
Preset &PresetCollection::get_preset_with_name(
    const std::string &new_name, const Preset *initial_preset
) {
    // 1) Find the preset with a new_name or create a new one,
...
bool PresetCollection::delete_current_preset() {
    ...
    if (new_selected_idx < m_presets.size())
        for (; new_selected_idx < m_presets.size() && !m_presets[new_selected_idx].is_visible;
             ++new_selected_idx)
            ;
    if (new_selected_idx == m_presets.size())
        for (--new_selected_idx; new_selected_idx > 0 && !m_presets[new_selected_idx].is_visible;
             --new_selected_idx)
            ;
...
// Return vendor of the first parent profile, for which the vendor is defined, or null if such
...
const std::string &PresetCollection::get_preset_name_by_alias(const std::string &alias) const {
    for (
        ...
        if (auto it_preset = this->find_preset_internal(it->second); it_preset != m_presets.end() &&
            it_preset->name == it->second && it_preset->is_visible &&
            (it_preset->is_compatible || size_t(it_preset - m_presets.begin()) == m_idx_selected))
            return it_preset->name;
    return alias;
}

const std::string &PresetCollection::get_preset_name_by_alias_invisible(
    ...
    for (
        ...
        it != m_map_alias_to_profile_name.end() && it->first == alias; ++it)
        if (auto it_preset = this->find_preset_internal(it->second); it_preset != m_presets.end() &&
            it_preset->name == it->second && it_preset->is_compatible)
            return it_preset->name;
    return alias;
}
...
Preset *PresetCollection::find_preset(
    const std::string &name, bool first_visible_if_not_found, bool respect_active_preset /*= true*/
) {
...
size_t PresetCollection::update_compatible_internal(
    ...
    for (size_t idx_preset = m_num_default_presets; idx_preset < m_presets.size(); ++idx_preset) {
        ...
        const PresetWithVendorProfile this_preset_with_vendor_profile =
            this->get_preset_with_vendor_profile(preset_edited);
        bool was_compatible = preset_edited.is_compatible;
        preset_edited.is_compatible =
            is_compatible_with_printer(this_preset_with_vendor_profile, active_printer, &config);
        some_compatible |= preset_edited.is_compatible;
        if (active_print != nullptr)
            preset_edited.is_compatible &= is_compatible_with_print(
                this_preset_with_vendor_profile, *active_print, active_printer
            );
    ...
    // Update visibility of the default profiles here if the defaults are suppressed, the current
    // profile is not compatible and we don't want to select another compatible profile.
...
void add_correct_opts_to_diff(
    ...
    t_config_option_keys &vec,
    const ConfigBase &other,
...
static const std::set<std::string> independent_from_extruder_number_options = {
    "bed_shape",           "compatible_printers",
    "compatible_prints",   "filament_ramming_parameters",
    "gcode_substitutions", "post_process",
};

bool PresetCollection::is_independent_from_extruder_number_option(const std::string &opt_key) {
    return independent_from_extruder_number_options.find(opt_key) !=
        independent_from_extruder_number_options.end();
}
    ...
    for (const t_config_option_key &opt_key : config_this.keys()) {
        ...
        if (this_opt != nullptr && other_opt != nullptr && *this_opt != *other_opt) {
            if (PresetCollection::is_independent_from_extruder_number_option(opt_key)) {
                // Scalar variable, or a vector variable, which is independent from number of
                // extruders, thus the vector is presented to the user as a single input.
                diff.emplace_back(opt_key);
            } else if (opt_key == "default_filament_profile") {
                // Ignore this field, it is not presented to the user, therefore showing a
                // "modified" flag for this parameter does not help. Also the length of this field
                // may differ, which may lead to a crash if the block below is used.
            } else if (opt_key == "thumbnails") {
                // "thumbnails" can not contain extensions in old config but they are valid and use
                // PNG extension by default So, check if "thumbnails" is really changed We will
                // compare full thumbnails instead of exactly config values
                auto [thumbnails, er] = GCodeThumbnails::make_and_check_thumbnail_list(config_this);
                auto [thumbnails_new, er_new] = GCodeThumbnails::make_and_check_thumbnail_list(
                    config_other
                );
                ...
                case coInts:
                    add_correct_opts_to_diff<
                        ConfigOptionInts>(opt_key, diff, config_other, config_this);
                    break;
                case coBools:
                    add_correct_opts_to_diff<
                        ConfigOptionBools>(opt_key, diff, config_other, config_this);
                    break;
                case coFloats:
                    add_correct_opts_to_diff<
                        ConfigOptionFloats>(opt_key, diff, config_other, config_this);
                    break;
                case coStrings:
                    add_correct_opts_to_diff<
                        ConfigOptionStrings>(opt_key, diff, config_other, config_this);
                    break;
                case coPercents:
                    add_correct_opts_to_diff<
                        ConfigOptionPercents>(opt_key, diff, config_other, config_this);
                    break;
                case coPoints:
                    add_correct_opts_to_diff<
                        ConfigOptionPoints>(opt_key, diff, config_other, config_this);
                    break;
                case coFloatsOrPercents:
                    add_correct_opts_to_diff<
                        ConfigOptionFloatsOrPercents>(opt_key, diff, config_other, config_this);
                    break;
                case coEnums:
                    add_correct_opts_to_diff<
                        ConfigOptionEnumsGeneric>(opt_key, diff, config_other, config_this);
                    break;
                default: diff.emplace_back(opt_key); break;
                }
                // "nozzle_diameter" is a vector option which contain info about diameter for each
                // nozzle But in the same time size of this vector indicates about count of
                // extruders, So, we need to add it to the diff if its size is changed.
                if (opt_key == "nozzle_diameter" &&
                    static_cast<const ConfigOptionFloats *>(this_opt)->size() !=
...
static constexpr const std::initializer_list<const char *>
    optional_keys{"compatible_prints", "compatible_printers"};
...
std::vector<std::string> PresetCollection::
    dirty_options(const Preset *edited, const Preset *reference, const bool deep_compare /*= false*/) {
...
bool PresetCollection::select_preset_by_name(
    const std::string &name_w_suffix, bool force, bool force_invisible /*= false*/
) {
...
std::vector<std::string> PresetCollection::merge_presets(
    PresetCollection &&other, const VendorMap &new_vendors
) {
...
void PresetCollection::update_map_alias_to_profile_name() {
    ...
    std::sort(
        m_map_alias_to_profile_name.begin(), m_map_alias_to_profile_name.end(),
    ...
    for (Preset &preset : m_presets)
        for (const std::string &renamed_from : preset.renamed_from) {
            ...
            if (!success)
                BOOST_LOG_TRIVIAL(error)
                    << boost::format(
                           "Preset name \"%1%\" was marked as renamed from \"%2%\", though "
                           "preset name \"%3%\" was marked as renamed from \"%2%\" as well."
                       ) %
...
const Preset &PrinterPresetCollection::default_preset_for(const DynamicPrintConfig &config) const {
    const ConfigOptionEnumGeneric *opt_printer_technology = config.opt<ConfigOptionEnumGeneric>(
        "printer_technology"
    );
    return this->default_preset(
        (opt_printer_technology == nullptr || opt_printer_technology->value == ptFFF) ? 0 : 1
    );
}

const Preset *PrinterPresetCollection::find_system_preset_by_model_and_variant(
...
static std::vector<std::string> s_PhysicalPrinter_opts{
    ...
    "preset_names", "printer_technology", "host_type", "print_host", "printhost_apikey",
...
bool PhysicalPrinter::has_empty_config() const {
    return config.opt_string("print_host").empty() &&
        ...
        config.opt_string("printhost_port").empty() && config.opt_string("printhost_user").empty() &&
        config.opt_string("printhost_password").empty();
}

// temporary workaround for compatibility with older Slicer
static void update_preset_name_option(
    const std::set<std::string> &preset_names, DynamicPrintConfig &config
) {
    std::string name;
...
void PhysicalPrinter::update_from_config(const DynamicPrintConfig &new_config) {
    ...
    const std::vector<std::string> &values =
        config.option<ConfigOptionStrings>("preset_names")->values;
...
PhysicalPrinter::PhysicalPrinter(
    const std::string &name, const DynamicPrintConfig &default_config, const Preset &preset
)
    : name(name), config(default_config) {
...
PhysicalPrinterCollection::PhysicalPrinterCollection(
    const std::vector<std::string> &keys, PresetBundle *preset_bundle
)
    : m_preset_bundle_owner(preset_bundle) {
    // Default config for a physical printer containing all key/value pairs of
    // PhysicalPrinter::printer_options().
...
static void update_preset_names_if_were_renamed(
    std::set<std::string> &preset_names, const PrinterPresetCollection &printer_presets
) {
        ...
        if (preset)
            ...
            if (const std::string *new_name = printer_presets.get_preset_name_renamed(preset_name)) {
                BOOST_LOG_TRIVIAL(warning)
                    << "Printer preset present " << preset_name << "was renamed to: " << *new_name
                    << std::endl;
...
void PhysicalPrinterCollection::load_printers(
    ...
    m_dir_path = dir.string();
    std::string errors_cummulative;
    // Store the loaded printers into a new vector, otherwise the binary search for already existing
    // presets would be broken.
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(dir))
        if (Slic3r::is_ini_file(dir_entry)) {
            ...
            if (PhysicalPrinter *found_printer = this->find_printer(name, false)) {
                ...
                update_preset_names_if_were_renamed(
                    found_printer->preset_names, m_preset_bundle_owner->printers
                );
            ...
            try {
                ...
                try {
                    DynamicPrintConfig config;
                    ConfigSubstitutions config_substitutions =
                        config.load_from_ini(printer.file, substitution_rule);
                    if (!config_substitutions.empty())
                        substitutions.push_back(
                            {name, Preset::TYPE_PHYSICAL_PRINTER,
                    ...
                    // Check it and replace with new name(s) if it's needed
                    update_preset_names_if_were_renamed(
                        printer.preset_names, m_preset_bundle_owner->printers
                    );
                } catch (const std::ifstream::failure &err) {
                    throw Slic3r::RuntimeError(
                    ...
                    throw Slic3r::RuntimeError(
                        std::string("Failed loading the preset file: ") + printer.file +
                        "\n\tReason: " + err.what()
                    );
                }
    ...
    m_printers.insert(
        m_printers.end(), std::make_move_iterator(printers_loaded.begin()),
...
PhysicalPrinter *PhysicalPrinterCollection::find_printer(
    const std::string &name, bool case_sensitive_search
) {
...
std::deque<PhysicalPrinter>::iterator PhysicalPrinterCollection::
    find_printer_internal(const std::string &name, bool case_sensitive_search /* = true*/) {
    if (case_sensitive_search)
        return Slic3r::lower_bound_by_predicate(
            m_printers.begin(), m_printers.end(), [&name](const auto &l) { return l.name < name; }
        );
...
PhysicalPrinter *PhysicalPrinterCollection::find_printer_with_same_config(
...
void PhysicalPrinterCollection::
    save_printer(PhysicalPrinter &edited_printer, const std::string &renamed_from /* = ""*/) {
...
void PhysicalPrinterCollection::rename_preset_in_printers(
    const std::string &old_preset_name, const std::string &new_preset_name
) {
    for (PhysicalPrinter &printer : m_printers)
...
std::vector<std::string> PhysicalPrinterCollection::
    get_printers_with_preset(const std::string &preset_name, bool respect_only_preset /*= true*/) {
...
std::vector<std::string> PhysicalPrinterCollection::get_printers_with_only_preset(
    const std::string &preset_name
) {
    std::vector<std::string> printers;
...
void PhysicalPrinterCollection::select_printer(const std::string &full_name) {
    ...
    if (const std::string *new_name = m_preset_bundle_owner->printers.get_preset_name_renamed(
            m_selected_preset
        ))
        m_selected_preset = *new_name;
}

void PhysicalPrinterCollection::select_printer(
    const std::string &printer_name, const std::string &preset_name
) {
...
bool PhysicalPrinterCollection::is_selected(
    PhysicalPrinterCollection::ConstIterator it, const std::string &preset_name
) const {
    return m_idx_selected == size_t(it - m_printers.begin()) && m_selected_preset == preset_name;
}

ExtruderFilaments::ExtruderFilaments(
    PresetCollection *filaments_collection,
    size_t extruder_id,
...
const std::string &ExtruderFilaments::get_preset_name_by_alias(const std::string &alias) const {
    ...
    for (
        ...
        if (auto it_filament = find_filament_internal(it->second);
            ...
            (it_filament->is_compatible ||
             size_t(it_filament - m_extr_filaments.begin()) == m_idx_selected))
...
size_t ExtruderFilaments::update_compatible_internal(
    const PresetWithVendorProfile &active_printer,
    const PresetWithVendorProfile *active_print,
    PresetSelectCompatibleType unselect_if_incompatible
) {
    ...
    if (m_extruder_id > 0 &&
        !printer_preset_adjusted.config.opt_bool("single_extruder_multi_material")) {
        DynamicPrintConfig &active_printer_config = printer_preset_adjusted.config;
        for (const std::string &key : print_config_def.extruder_option_keys()) {
            if (key == "default_filament_profile")
                continue; // Ignore this field, because this parameter is not related to the
                          // extruder but to whole printer.
    ...
    for (size_t idx_preset = num_default_presets; idx_preset < m_extr_filaments.size();
         ++idx_preset) {
        ...
        const PresetWithVendorProfile this_preset_with_vendor_profile =
            m_filaments->get_preset_with_vendor_profile(*preset);
        bool was_compatible = extr_filament.is_compatible;
        extr_filament.is_compatible = is_compatible_with_printer(
            this_preset_with_vendor_profile, active_printer_adjusted, &config
        );
        if (active_print != nullptr)
            extr_filament.is_compatible &= is_compatible_with_print(
                this_preset_with_vendor_profile, *active_print, active_printer_adjusted
            );
...
const VendorProfile::PrinterModel *system_printer_model(const Preset &preset) {
    ...
    if (preset.vendor != nullptr) {
        ...
        if (printer_model != nullptr && !printer_model->value.empty()) {
            auto it = std::find_if(
                preset.vendor->models.begin(), preset.vendor->models.end(),
...
bool compare_vendor_profile_printers(
    const VendorProfile &vp_old, const VendorProfile &vp_new, std::vector<std::string> &new_printers
) {
    for (const VendorProfile::PrinterModel &model : vp_new.models) {
        if (std::find_if(
                vp_old.models.begin(), vp_old.models.end(),
                [model](const VendorProfile::PrinterModel &pm) { return pm.id == model.id; }
            ) == vp_old.models.end())
