...
Polyline JPSPathFinder::find_path(const Point &p0, const Point &p1)
{
    ...
    std::vector<Pixel, Domain::PointsAllocator<Pixel>> out_path;
    ...
    ::Slic3r::Biz::Algorithms::SVG::SVG svg(debug_out_path(("path_jps" + std::to_string(print_z) + "_" + std::to_string(rand() % 1000)).c_str()).c_str(),
    ...
    std::vector<Pixel, Domain::PointsAllocator<Pixel>> tmp_path;
