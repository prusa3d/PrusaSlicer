...
{
    Domain::Vec2big v21 = (p2 - p1).cast<int64_t>();
...
static std::optional<Circle> try_create_circle(const Points::const_iterator begin, const Points::const_iterator end, const double max_radius, const double tolerance)
{
    ...
    if (size == 3) {
        ...
        if (! circle) {
            ...
            Domain::Vec2big first_point = begin->cast<int64_t>();
            Domain::Vec2big last_point  = std::prev(end)->cast<int64_t>();
            Domain::Vec2big v = last_point - first_point;
            Vec2d   vd = v.cast<double>();
            double  ld = v.squaredNorm();
            if (ld > sqr(scaled<double>(0.0015))) {
                Domain::Vec2big c = (first_point.cast<int64_t>() + last_point.cast<int64_t>()) / 2;
                Domain::Vec2big prev_point = first_point;
    ...
    const Points::const_iterator end)
{
    ...
    Domain::Vec2big vstart  = (*begin - center).cast<int64_t>();
    Domain::Vec2big vprev   = vstart;
    int     arc_dir = 0;
    for (auto it = std::next(begin); it != end; ++ it) {
        Domain::Vec2big v = (*it - center).cast<int64_t>();
    ...
    const double                 path_tolerance_percent)
{
    ...
    Domain::Vec2big vstart = (*begin - circle.center).cast<int64_t>();
    Domain::Vec2big vend   = (*std::prev(end) - circle.center).cast<int64_t>();
            ...
            while (end != src.end()) {
                ...
                if (std::optional<Arc> this_arc = try_create_arc(
                    ...
                    while (std::next(end) != src.end()) {
                        assert(end == next_end);
                        {
                            Domain::Vec2big v1 = arc->start_point.cast<int64_t>() - arc->center.cast<int64_t>();
                            Domain::Vec2big v2 = arc->end_point.cast<int64_t>() - arc->center.cast<int64_t>();
                            do {
    ...
    if (path.size() < 2 || path.front().point == point) {
        ...
        for (auto it = std::next(path.cbegin()); it != path.cend(); ++ it) {
            if (it->linear()) {
                ...
                // Circular arc
                Domain::Vec2big center = arc_center(prev.cast<double>(), it->point.cast<double>(), double(it->radius), it->ccw()).cast<int64_t>();
                // Test whether point is inside the wedge.
                Domain::Vec2big v1 = prev.cast<int64_t>() - center;
                Domain::Vec2big v2 = it->point.cast<int64_t>() - center;
                Domain::Vec2big vp = point.cast<int64_t>() - center;
