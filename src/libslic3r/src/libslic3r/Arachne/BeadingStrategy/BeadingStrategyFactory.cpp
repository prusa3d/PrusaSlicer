                                                        ...
                                                        const double  minimum_variable_line_ratio)
{
    ...
    SPDLOG_TRACE("Applying the Redistribute meta-strategy with outer-wall width = {}, inner-wall width = {}.", preferred_bead_width_outer, preferred_bead_width_inner);
    ret = std::make_unique<RedistributeBeadingStrategy>(preferred_bead_width_outer, minimum_variable_line_ratio, std::move(ret));

    if (print_thin_walls) {
        SPDLOG_TRACE("Applying the Widening Beading meta-strategy with minimum input width {} and minimum output width {}.", min_feature_size, min_bead_width);
        ret = std::make_unique<WideningBeadingStrategy>(std::move(ret), min_feature_size, min_bead_width);
    }

    if (outer_wall_offset > 0) {
        SPDLOG_TRACE("Applying the OuterWallOffset meta-strategy with offset = {}.", outer_wall_offset);
        ret = std::make_unique<OuterWallInsetBeadingStrategy>(outer_wall_offset, std::move(ret));
    }

    // Apply the LimitedBeadingStrategy last, since that adds a 0-width marker wall which other beading strategies shouldn't touch.
    SPDLOG_TRACE("Applying the Limited Beading meta-strategy with maximum bead count = {}.", max_bead_count);
