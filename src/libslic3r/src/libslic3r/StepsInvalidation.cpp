...
const std::map<std::string, std::vector<Step>> invalidated_by{
    ...
    {"orca_toolchange_timing", steps({propagate(psGCodeExport)})},
    {"orca_filament_load_time", steps({propagate(psGCodeExport)})},
    {"orca_filament_unload_time", steps({propagate(psGCodeExport)})},
    ...
    {"small_perimeter_threshold", steps({propagate(psGCodeExport)})},
    ...
    {"wipe_speed", steps({propagate(psGCodeExport)})},
    {"wipe_distance", steps({propagate(psGCodeExport)})},
    {"retract_after_wipe", steps({propagate(psGCodeExport)})},
