...
SmoothingParams get_smoothing_params(
    ...
    const double max_xy_velocity =
        Vec2d{
            config.machine_max_feedrate_x.at(machine_mode_idx),
            config.machine_max_feedrate_y.at(machine_mode_idx)}
            .norm();

    const double xy_acceleration = config.machine_max_acceleration_travel.at(machine_mode_idx);
    ...
    const double deceleration_time = max_z_velocity /
        config.machine_max_acceleration_z.at(machine_mode_idx);
    ...
    const unsigned points_count = blend_width > 0 ?
        std::ceil(max_z_velocity / config.machine_max_jerk_z.at(machine_mode_idx)) :
        1;
...
ElevatedTravelParams get_elevated_traval_params(
    ...
    if (!config.travel_ramping_lift.at(extruder_id)) {
        elevation_params.slope_end = 0;
        elevation_params.lift_height = config.retract_lift.at(extruder_id);
        elevation_params.blend_width = 0;
        return elevation_params;
    }
    elevation_params.lift_height = config.travel_max_lift.at(extruder_id);

    const double slope_deg = config.travel_slope.at(extruder_id);
