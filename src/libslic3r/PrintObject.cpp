...
void PrintObject::_slice(const std::vector<coordf_t> &layer_height_profile)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Slicing objects - make_slices in parallel - begin";
    {
	    ...
	    tbb::parallel_for(
	        tbb::blocked_range<size_t>(0, m_layers.size()),
			[this, upscaled, clipped, xy_compensation_scaled, elephant_foot_compensation_scaled]
				(const tbb::blocked_range<size_t>& range) {
	            for (size_t layer_id = range.begin(); layer_id < range.end(); ++ layer_id) {
	                m_print->throw_if_canceled();
	                Layer *layer = m_layers[layer_id];
	                ...
	                if (layer->m_regions.size() == 1) {
							...
							layerm->slices.set(
								union_ex(
									Slic3r::elephant_foot_compensation(
										(delta == 0.f) ? expolygons : offset_ex(expolygons, delta), 
	                    ...
	                    if (xy_compensation_scaled < 0.f || elfoot > 0.f) {
	                        ...
	                        if (elfoot > 0.f) {
								trimming = to_polygons(Slic3r::elephant_foot_compensation(offset_ex(layer->merged(eps), std::min(xy_compensation_scaled, 0.f) - eps),
