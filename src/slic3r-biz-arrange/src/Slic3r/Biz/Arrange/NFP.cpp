...
namespace {
int64_t dotperp(const Vec2big& a, const Vec2big& b)
{
...
static bool line_cmp(const Line& e1, const Line& e2)
{
    ...
    const Vec2big ax(1, 0); // Unit vector for the X axis

    Vec2big p1 = (e1.b - e1.a).cast<int64_t>();
    Vec2big p2 = (e2.b - e2.a).cast<int64_t>();
...
Polygon ifp_convex_convex(const Polygon& fixed, const Polygon& movable)
{
    ...
    { // Older mac compilers generate warnging if line_range is called in-place
