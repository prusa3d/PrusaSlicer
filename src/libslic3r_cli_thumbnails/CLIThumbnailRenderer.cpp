...
Images render_thumbnails(const Project& project,
                         const Biz::Slicing::ThumbnailImageRequest& request,
                         const std::string& resources_dir)
{
    const auto& params = request.params;
    if (params.pixel_format != PixelFormat::RGBA8) return {};
    ...
    if (ge != GLEW_OK && glCreateShader == nullptr) {
        SPDLOG_WARN("CLI thumbnails skipped: glewInit failed and core GL not usable: {}", reinterpret_cast<const char*>(glewGetErrorString(ge)));
        return {};
    }
    if (ge != GLEW_OK) {
        SPDLOG_DEBUG("glewInit returned non-OK ({}) but core GL function pointers loaded — proceeding", reinterpret_cast<const char*>(glewGetErrorString(ge)));
    }
    ...
    for (const auto& config : project.config_containers()) {
        ...
        for (const auto& bed : config->bed_instances()) {
            if (request.type != Biz::ThumbnailType::Scene
                && bed->id().id != params.bed_instance_id) continue;
            for (const ModelInstance* inst : bed->model_instances) {
                if (!inst || !inst->is_printable()) continue;
                for (const ModelVolume* vol : inst->get_object()->volumes) {
                    ...
                    mb.color = App::Scene::color_from_extruder_slot(slot_colors, *vol, *config)
    ...
    for (size_t i = 0; i < lights.size(); ++i) {
        ...
        auto uniform = [&](const char* name) { return glGetUniformLocation(prog, (prefix + name).c_str()); };
