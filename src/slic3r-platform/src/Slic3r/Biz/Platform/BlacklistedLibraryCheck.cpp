...
const std::vector<std::wstring> BlacklistedLibraryCheck::blacklist(
    {L"NahimicOSD.dll", L"SS2OSD.dll", L"amhook.dll", L"AMHook.dll"}
);
...
bool BlacklistedLibraryCheck::is_blacklisted(const std::wstring& dllpath)
{
    ...
    if (std::find(
            BlacklistedLibraryCheck::blacklist.begin(),
