...
Domain::ExPolygonsWithIds create_shape_with_ids(const NSVGimage& image, const NSVGLineParams& param)
{
    ...
    Algorithms::ExPolygonsWithId::center(result);
    return result;
}
...
//
// // tl .. top left
// Domain::Vec2f tl(std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
// // br .. bottom right
// Domain::Vec2f br(std::numeric_limits<float>::min(), std::numeric_limits<float>::min());
// bounds(image, tl, br);
//
// tl.x() = std::floor(tl.x());
// tl.y() = std::floor(tl.y());
...
// br.x() = std::ceil(br.x());
// br.y() = std::ceil(br.y());
// Domain::Vec2f s = br - tl;
// Domain::Point size = s.cast<Domain::coord_t>();
//
...
Domain::coord_t to_coor(float val, double scale)
{
    return static_cast<Domain::coord_t>(std::round(val * scale));
}

bool need_flattening(
    float tessTol,
    const Domain::Vec2f& p1,
    const Domain::Vec2f& p2,
    const Domain::Vec2f& p3,
    const Domain::Vec2f& p4
)
{
    // f .. first
    // s .. second
    auto det = [](const Domain::Vec2f& f, const Domain::Vec2f& s) {
        return std::fabs(f.x() * s.y() - f.y() * s.x());
    };

    Domain::Vec2f pd  = (p4 - p1);
    Domain::Vec2f pd2 = (p2 - p4);
    float d2          = det(pd2, pd);
    Domain::Vec2f pd3 = (p3 - p4);
    float d3          = det(pd3, pd);
    float d23         = d2 + d3;
...
{
    // Domain::Vec2f p1(p[0], p[1]);
    // Domain::Vec2f p2(p[2], p[3]);
    // Domain::Vec2f p3(p[4], p[5]);
    // Domain::Vec2f p4(p[6], p[7]);
    float dx_3 = (p[6] - p[0]) / 3.f;
    float dy_3 = (p[7] - p[1]) / 3.f;

    return Domain::is_approx(p[2], p[0] + dx_3, precision)
        && Domain::is_approx(p[4], p[6] - dx_3, precision)
        && Domain::is_approx(p[3], p[1] + dy_3, precision)
...
)
{
    if (!need_flattening(tessTol, p1, p2, p3, p4)) {
        Domain::coord_t x = static_cast<Domain::coord_t>(std::round(p4.x()));
        Domain::coord_t y = static_cast<Domain::coord_t>(std::round(p4.y()));
    ...
    Domain::Vec2f p12   = (p1 + p2) * 0.5f;
    Domain::Vec2f p23   = (p2 + p3) * 0.5f;
    Domain::Vec2f p34   = (p3 + p4) * 0.5f;
    Domain::Vec2f p123  = (p12 + p23) * 0.5f;
    Domain::Vec2f p234  = (p23 + p34) * 0.5f;
    Domain::Vec2f p1234 = (p123 + p234) * 0.5f;
...
LinesPath linearize_path(NSVGpath* first_path, const Emboss::NSVGLineParams& param)
{
    ...
    Domain::Polylines& polylines = result.polylines;

    // multiple use of allocated memmory for points between paths
    Domain::Points points;
    for (NSVGpath* path = first_path; path != NULL; path = path->next) {
        // Flatten path
        Domain::coord_t x = to_coor(path->pts[0], param.scale);
        Domain::coord_t y = to_coor(path->pts[1], param.scale);
        points.emplace_back(x, y);
        ...
        for (size_t i = 0; i < path_size; i += 3) {
            ...
            if (is_line(p)) {
                // point p4
                Domain::coord_t xx = to_coor(p[6], param.scale);
                Domain::coord_t yy = to_coor(p[7], param.scale);
...
)
{
    ...
    return Algorithms::HealPolygon::heal_polygons(fill, is_non_zero, param.max_heal_iteration);
}
...
Domain::Polylines to_dashes(const Domain::Polyline& polyline, const DashesParam& param)
{
    Domain::Polylines dashes;
    Domain::Polyline dash; // cache for one dash in dashed line
    Domain::Point prev_point;
    ...
    for (const Domain::Point& point : polyline.points) {
        ...
        Domain::Point diff        = point - prev_point;
        ...
        while (dash_length < line_segment_length) {
            ...
            Domain::Point move_point   = diff * d;
            Domain::Point intermediate = prev_point + move_point;

            // add Dash in stroke
            if (is_line) {
                if (dash.empty()) {
                    dashes.emplace_back(Domain::Points{prev_point, intermediate});
                } else {
...
)
{
    ...
    case NSVGlineCap::NSVG_CAP_BUTT:
        end_type = ClipperLib::EndType::etOpenButt;
        break;
    case NSVGlineCap::NSVG_CAP_ROUND:
        end_type = ClipperLib::EndType::etOpenRound;
        break;
    case NSVGlineCap::NSVG_CAP_SQUARE:
        end_type = ClipperLib::EndType::etOpenSquare;
        break;
    }
    ...
    if (shape.strokeDashCount > 0) {
        ...
        Domain::Polylines dashes;
        for (const Domain::Polyline& polyline : lines_path.polylines)
            Slic3r::append(dashes, to_dashes(polyline, params));
        for (const Domain::Polygon& polygon : lines_path.polygons)
            Slic3r::append(dashes, to_dashes(Algorithms::Polygon::to_polyline(polygon), params));
        result = Algorithms::ClipperUtils::offset(dashes, stroke_width / 2, join_type, mitter, end_type);
    } else {
        result = Algorithms::ClipperUtils::contour_to_polygons(
            lines_path.polygons,
            stroke_width,
            join_type,
            mitter
        ...
        Slic3r::append(
            result,
            Algorithms::ClipperUtils::offset(lines_path.polylines, stroke_width / 2, join_type, mitter, end_type)
        );
    }

    bool is_non_zero = true;
    return Algorithms::HealPolygon::heal_polygons(result, is_non_zero, param.max_heal_iteration);
}
