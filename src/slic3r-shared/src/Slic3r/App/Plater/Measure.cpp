...
#include "Slic3r/App/Plater/MeasureUtils.hpp"
#include "Slic3r/Biz/Algorithms/Geometry/Circle.hpp"
#include "Slic3r/App/Plater/SurfaceMesh.hpp"
...
static std::tuple<Domain::Vec3d, double, double> get_center_and_radius(
    const std::vector<Domain::Vec3d>& points,
    const Domain::Transform3d& trafo,
    const Domain::Transform3d& trafo_inv
)
{
    Domain::Vec2ds out;
    double z = 0.;
    for (const Domain::Vec3d& pt : points) {
        Domain::Vec3d pt_transformed = trafo * pt;
        z                            = pt_transformed.z();
    ...
    return std::make_tuple(
        trafo.inverse() * Domain::Vec3d(circle.center.x(), circle.center.y(), z),
        circle.radius,
        error
    );
}

static std::array<Domain::Vec3d, 3> orthonormal_basis(const Domain::Vec3d& v)
{
    std::array<Domain::Vec3d, 3> ret;
    ret[2] = v.normalized();
    int index;
    ret[2].cwiseAbs().maxCoeff(&index);
    switch (index) {
    case 0: {
        ret[0] = Domain::Vec3d(ret[2].y(), -ret[2].x(), 0.0).normalized();
        break;
    }
    case 1: {
        ret[0] = Domain::Vec3d(0.0, ret[2].z(), -ret[2].y()).normalized();
        break;
    }
    case 2: {
        ret[0] = Domain::Vec3d(-ret[2].z(), 0.0, ret[2].x()).normalized();
        break;
    }
    }
...
public:
    ...
    struct PlaneData
    {
        ...
        std::vector<std::vector<Domain::Vec3d>> borders; // FIXME: should be in fact local in update_planes()
    ...
    std::optional<SurfaceFeature> feature(size_t face_idx, const Domain::Vec3d& point);
    size_t num_of_planes() const;
    const std::vector<int>& plane_triangle_indices(int idx) const;
    const std::vector<SurfaceFeature>& plane_features(unsigned int plane_id);
...
void MeasuringImpl::update_planes()
{
    ...
    const std::vector<Domain::Vec3f> face_normals    = TriMesh::its_face_normals(m_its);
    ...
    auto is_same_normal = [](const stl_normal& a, const stl_normal& b) -> bool {
        return (
            std::abs(a(0) - b(0)) < 0.001 && std::abs(a(1) - b(1)) < 0.001 && std::abs(a(2) - b(2)) < 0.001
        );
    };
    ...
    m_planes.reserve(
        num_of_facets / 5
    ); // empty plane data object is quite lightweight, let's save the initial reallocations
    ...
    while (1) {
        ...
        while (facet_queue_cnt > 0) {
            ...
            if (is_same_normal(this_normal, *normal_ptr)) {
                // const Index3& face = m_its.indices[facet_idx];
                ...
                for (int j = 0; j < 3; ++j)
                    if (int neighbor_idx = face_neighbors[facet_idx][j];
                        neighbor_idx >= 0 && m_face_to_plane[neighbor_idx] == size_t(-1))
    ...
    DEBUG_ASSERT(std::none_of(m_face_to_plane.begin(), m_face_to_plane.end(), [](size_t val) {
        return val == size_t(-1);
    }));
...
{
    DEBUG_ASSERT(!m_planes[plane_idx].features_extracted);

    PlaneData& plane = m_planes[plane_idx];
    plane.surface_features.clear();
    const Domain::Vec3d& normal = plane.normal;

    Eigen::Quaterniond q;
    q.setFromTwoVectors(plane.normal, Domain::Vec3d::UnitZ());
    Domain::Transform3d trafo = Domain::Transform3d::Identity();
    trafo.rotate(q);
    const Domain::Transform3d trafo_inv = trafo.inverse();

    std::vector<double> angles; // placed in outer scope to prevent reallocations
    std::vector<double> lengths;

    for (const std::vector<Domain::Vec3d>& border : plane.borders) {
        if (border.size() <= 1)
            continue;
        ...
        if (!done) {
            ...
            auto are_angles_same = [](double a, double b) {
                return Domain::is_approx(a, b, 0.01);
            };
            auto are_lengths_same = [](double a, double b) {
                return Domain::is_approx(a, b, 0.01);
            };
            ...
            auto offset_to_index = [border_size = int(border.size())](int idx, int offset) -> int {
                DEBUG_ASSERT(std::abs(offset) < border_size);
                int out = idx + offset;
                if (out >= border_size)
                    out = out - border_size;
                else if (out < 0)
            ...
            for (int i = 0; i < int(border.size()); ++i) {
                const Domain::Vec3d& v2 = border[i]
                    - (i == 0 ? border[border.size() - 1] : border[i - 1]);
                const Domain::Vec3d& v1 = (i == int(border.size() - 1) ? border[0] : border[i + 1])
                    - border[i];
            ...
            DEBUG_ASSERT(border.size() == angles.size());
            DEBUG_ASSERT(border.size() == lengths.size());
            ...
            std::vector<Domain::Vec3d> single_circle; // could be in loop-scope, but reallocations
            ...
            while (i != first_pt_idx || first_iter) {
                    ...
                    && i != start_idx)
                {
                    ...
                    if (circle && single_circle.size() >= 5) { // Less than 5 vertices? Not a circle.
                        ...
                        if (accept_circle) {
                            const auto& [center, radius, err] = get_center_and_radius(
                                single_circle,
                                trafo,
                                trafo_inv
                            );
                            ...
                            if (accept_circle) {
                                ...
                                circles.emplace_back(SurfaceFeature(
                                    SurfaceFeatureType::Circle,
                                    center,
                                    plane.normal,
                                    std::nullopt,
            ...
            if (circles_idxs.empty()) {
                ...
                for (int i = 1; i < int(border.size()); ++i)
                    edges.emplace_back(
                        SurfaceFeature(SurfaceFeatureType::Edge, border[i - 1], border[i])
                    );
                edges.emplace_back(
                    SurfaceFeature(SurfaceFeatureType::Edge, border[0], border[border.size() - 1])
                );
            } else if (circles_idxs.size() > 1
                       || circles_idxs.front().first != circles_idxs.front().second)
            {
                ...
                while (true) {
                    i = offset_to_index(i, 1);
                    edges.emplace_back(SurfaceFeature(
                        SurfaceFeatureType::Edge,
                        border[offset_to_index(i, -1)],
                        border[i]
                    ));
            ...
            DEBUG_ASSERT(std::all_of(edges.begin(), edges.end(), [](const SurfaceFeature& f) {
                return f.type() == SurfaceFeatureType::Edge;
            }));
            for (int i = edges.size() - 1; i >= 0; --i) {
                const auto& [first_start, first_end] = edges[i == 0 ? edges.size() - 1 : i - 1].edge();
                const auto& [second_start, second_end] = edges[i].edge();

                if (Domain::is_approx(first_end, second_start)
                    && Domain::is_approx(
                        (first_end - first_start)
                            .normalized()
                            .dot((second_end - second_start).normalized()),
                        1.
                    ))
                {
                    // The edges have the same direction and share a point. Merge them.
                    edges[i == 0 ? edges.size() - 1 : i - 1] = SurfaceFeature(
                        SurfaceFeatureType::Edge,
                        first_start,
                        second_end
                    );
            ...
            DEBUG_ASSERT(std::all_of(circles.begin(), circles.end(), [](const SurfaceFeature& f) {
                return f.type() == SurfaceFeatureType::Circle;
            }));
            DEBUG_ASSERT(std::all_of(edges.begin(), edges.end(), [](const SurfaceFeature& f) {
                return f.type() == SurfaceFeatureType::Edge;
            }));
            plane.surface_features.insert(
                plane.surface_features.end(),
                std::make_move_iterator(circles.begin()),
    ...
    Domain::Vec3d cog = Domain::Vec3d::Zero();
    size_t counter    = 0;
    for (const std::vector<Domain::Vec3d>& b : plane.borders) {
...
std::optional<SurfaceFeature> MeasuringImpl::feature(size_t face_idx, const Domain::Vec3d& point)
{
    ...
    DEBUG_ASSERT(
        plane.surface_features.empty()
        || plane.surface_features.back().type() == SurfaceFeatureType::Plane
    );
    ...
    if (closest_feature_idx != size_t(-1)) {
        ...
        if (f.type() == SurfaceFeatureType::Edge) {
    ...
    DEBUG_ASSERT(plane.surface_features.back().type() == SurfaceFeatureType::Plane);
    return std::make_optional(plane.surface_features.back());
}

size_t MeasuringImpl::num_of_planes() const
{
    return m_planes.size();
}

const std::vector<int>& MeasuringImpl::plane_triangle_indices(int idx) const
{
    DEBUG_ASSERT(idx >= 0 && idx < int(m_planes.size()));
    return m_planes[idx].facets;
}

const std::vector<SurfaceFeature>& MeasuringImpl::plane_features(unsigned int plane_id)
{
    DEBUG_ASSERT(plane_id < m_planes.size());
    if (!m_planes[plane_id].features_extracted)
        extract_features(plane_id);
    return m_planes[plane_id].surface_features;
}

const indexed_triangle_set& MeasuringImpl::its() const
{
    return this->m_its;
}
...
std::optional<SurfaceFeature> Measuring::feature(size_t face_idx, const Domain::Vec3d& point) const
{
    return priv->feature(face_idx, point);
}

size_t Measuring::num_of_planes() const
{
    return priv->num_of_planes();
}

const std::vector<int>& Measuring::plane_triangle_indices(int idx) const
{
    return priv->plane_triangle_indices(idx);
}

const std::vector<SurfaceFeature>& Measuring::plane_features(unsigned int plane_id) const
{
    return priv->plane_features(plane_id);
}

const indexed_triangle_set& Measuring::its() const
{
    return priv->its();
}

const AngleAndEdges AngleAndEdges::Dummy = {
    0.0,
    Domain::Vec3d::Zero(),
    {Domain::Vec3d::Zero(), Domain::Vec3d::Zero()},
    {Domain::Vec3d::Zero(), Domain::Vec3d::Zero()},
    0.0,
    true
};

static AngleAndEdges angle_edge_edge(
...
)
{
    ...
    Domain::Vec3d e1_unit = edge_direction(e1.first, e1.second);
    Domain::Vec3d e2_unit = edge_direction(e2.first, e2.second);
    ...
    Domain::Vec3d e11_proj = plane.projection(e1.first);
    Domain::Vec3d e12_proj = plane.projection(e1.second);
    Domain::Vec3d e21_proj = plane.projection(e2.first);
    Domain::Vec3d e22_proj = plane.projection(e2.second);

    const bool coplanar = (e2.first - e21_proj).norm() < Domain::EPSILON
        && (e2.second - e22_proj).norm() < Domain::EPSILON;

    // rotate the plane to become the XY plane
    auto qp         = Eigen::Quaternion<double>::FromTwoVectors(normal, Domain::Vec3d::UnitZ());
    auto qp_inverse = qp.inverse();
    const Domain::Vec3d e11_rot = qp * e11_proj;
    const Domain::Vec3d e12_rot = qp * e12_proj;
    const Domain::Vec3d e21_rot = qp * e21_proj;
    const Domain::Vec3d e22_rot = qp * e22_proj;

    // discard Z
    const Domain::Vec2d e11_rot_2d = Domain::Vec2d(e11_rot.x(), e11_rot.y());
    const Domain::Vec2d e12_rot_2d = Domain::Vec2d(e12_rot.x(), e12_rot.y());
    const Domain::Vec2d e21_rot_2d = Domain::Vec2d(e21_rot.x(), e21_rot.y());
    const Domain::Vec2d e22_rot_2d = Domain::Vec2d(e22_rot.x(), e22_rot.y());

    // find intersection (arc center) of edges in XY plane
    const Eigen::Hyperplane<double, 2> e1_rot_2d_line = Eigen::Hyperplane<double, 2>::Through(
        e11_rot_2d,
        e12_rot_2d
    );
    const Eigen::Hyperplane<double, 2> e2_rot_2d_line = Eigen::Hyperplane<double, 2>::Through(
        e21_rot_2d,
        e22_rot_2d
    );
    const Domain::Vec2d center_rot_2d = e1_rot_2d_line.intersection(e2_rot_2d_line);

    // arc center in original coordinate
    const Domain::Vec3d center = qp_inverse
        * Domain::Vec3d(center_rot_2d.x(), center_rot_2d.y(), e11_rot.z());

    // ensure the edges are pointing away from the center
    std::pair<Domain::Vec3d, Domain::Vec3d> out_e1 = e1;
    std::pair<Domain::Vec3d, Domain::Vec3d> out_e2 = e2;
    ...
    // arc radius
    const Domain::Vec3d e1_proj_mid = 0.5 * (e11_proj + e12_proj);
    const Domain::Vec3d e2_proj_mid = 0.5 * (e21_proj + e22_proj);
...
static AngleAndEdges angle_edge_plane(
    const std::pair<Domain::Vec3d, Domain::Vec3d>& e,
    const std::tuple<int, Domain::Vec3d, Domain::Vec3d>& p
)
{
    const auto& [idx, normal, origin] = p;
    Domain::Vec3d e1e2_unit           = edge_direction(e);
    ...
    const Domain::Vec3d inters = line.intersectionPoint(plane);

    // then verify edge direction and revert it, if needed
    Domain::Vec3d e1 = e.first;
    Domain::Vec3d e2 = e.second;
    ...
    if (are_parallel(e1e2_unit, normal)) {
        const std::array<Domain::Vec3d, 3> basis = orthonormal_basis(e1e2_unit);
        const double radius                      = (0.5 * (e1 + e2) - inters).norm();
        const Domain::Vec3d edge_on_plane_dir = (basis[1].dot(origin - inters) >= 0.0) ? basis[1] :
                                                                                         -basis[1];
        std::pair<Domain::Vec3d, Domain::Vec3d> edge_on_plane = std::make_pair(
            inters,
            inters + radius * edge_on_plane_dir
        );
        if (!inters.isApprox(e1)) {
            edge_on_plane.first += radius * edge_on_plane_dir;
            edge_on_plane.second += radius * edge_on_plane_dir;
        }
        return AngleAndEdges(
            0.5 * double(M_PI),
            inters,
            std::make_pair(e1, e2),
            edge_on_plane,
    ...
    const Domain::Vec3d e1e2 = e2 - e1;
    const double e1e2_len    = e1e2.norm();

    // calculate 2nd edge (on the plane)
    const Domain::Vec3d temp                              = normal.cross(e1e2);
    const Domain::Vec3d edge_on_plane_unit                = normal.cross(temp).normalized();
    std::pair<Domain::Vec3d, Domain::Vec3d> edge_on_plane = {
        origin,
        origin + e1e2_len * edge_on_plane_unit
    };

    // ensure the 2nd edge is pointing in the correct direction
    const Domain::Vec3d test_edge = (edge_on_plane.second - edge_on_plane.first).cross(e1e2);
...
static AngleAndEdges angle_plane_plane(
    const std::tuple<int, Domain::Vec3d, Domain::Vec3d>& p1,
    const std::tuple<int, Domain::Vec3d, Domain::Vec3d>& p2
)
{
    ...
    auto intersection_plane_plane = [](const Domain::Vec3d& n1,
                                       const Domain::Vec3d& o1,
                                       const Domain::Vec3d& n2,
                                       const Domain::Vec3d& o2) {
        Eigen::MatrixXd m(2, 3);
        m << n1.x(), n1.y(), n1.z(), n2.x(), n2.y(), n2.z();
        Eigen::VectorXd b(2);
        b << o1.dot(n1), o2.dot(n2);
        Eigen::VectorXd x = m.colPivHouseholderQr().solve(b);
        return std::make_pair(n1.cross(n2).normalized(), Domain::Vec3d(x(0), x(1), x(2)));
    };

    // Calculate intersection line between planes
    const auto [intersection_line_direction, intersection_line_origin] = intersection_plane_plane(
        normal1,
        origin1,
        normal2,
        origin2
    ...
    const Eigen::ParametrizedLine<double, 3> intersection_line = Eigen::ParametrizedLine<double, 3>(
        intersection_line_origin,
        intersection_line_direction
    );
    const Domain::Vec3d origin1_proj = intersection_line.projection(origin1);
    const Domain::Vec3d origin2_proj = intersection_line.projection(origin2);

    // Calculate edges on planes
    const Domain::Vec3d edge_on_plane1_unit = (origin1 - origin1_proj).normalized();
    const Domain::Vec3d edge_on_plane2_unit = (origin2 - origin2_proj).normalized();
    const double radius                     = std::max(
        10.0,
        std::max((origin1 - origin1_proj).norm(), (origin2 - origin2_proj).norm())
    );
    const std::pair<Domain::Vec3d, Domain::Vec3d> edge_on_plane1 = {
        origin1_proj + radius * edge_on_plane1_unit,
        origin1_proj + 2.0 * radius * edge_on_plane1_unit
    };
    const std::pair<Domain::Vec3d, Domain::Vec3d> edge_on_plane2 = {
...
MeasurementResult measurement(const SurfaceFeature& a, const SurfaceFeature& b, const Measuring* measuring)
{
    ...
    ///////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////////////////////////////////////////
    if (f1.type() == SurfaceFeatureType::Point) {
        if (f2.type() == SurfaceFeatureType::Point) {
            Domain::Vec3d diff     = (f2.point() - f1.point());
            result.distance_strict = std::make_optional(
                DistAndPoints{diff.norm(), f1.point(), f2.point()}
            );
            result.distance_xyz = diff.cwiseAbs();

            ///////////////////////////////////////////////////////////////////////////
        } else if (f2.type() == SurfaceFeatureType::Edge) {
            const auto [s, e] = f2.edge();
            const Eigen::ParametrizedLine<double, 3> line(s, (e - s).normalized());
            const double dist_inf      = line.distance(f1.point());
            const Domain::Vec3d proj   = line.projection(f1.point());
            ...
            if (dist_start_sq < len_sq && dist_end_sq < len_sq) {
                ...
                result.distance_strict = std::make_optional(DistAndPoints{dist_inf, f1.point(), proj});
            } else { // the result is the closer of the endpoints
                const bool s_is_closer = dist_start_sq < dist_end_sq;
                result.distance_strict = std::make_optional(
                    DistAndPoints{
                        std::sqrt(std::min(dist_start_sq, dist_end_sq) + sqr(dist_inf)),
                        f1.point(),
                        s_is_closer ? s : e
                    }
                );
            }
            result.distance_infinite = std::make_optional(DistAndPoints{dist_inf, f1.point(), proj});
            ///////////////////////////////////////////////////////////////////////////
        } else if (f2.type() == SurfaceFeatureType::Circle) {
            // Find a plane containing normal, center and the point.
            const auto [c, radius, n] = f2.circle();
            const Eigen::Hyperplane<double, 3> circle_plane(n, c);
            const Domain::Vec3d proj = circle_plane.projection(f1.point());
            if (proj.isApprox(c)) {
                const Domain::Vec3d p_on_circle = c + radius * orthogonal(n, true);
                result.distance_strict = std::make_optional(DistAndPoints{radius, c, p_on_circle});
            } else {
                const Eigen::Hyperplane<double, 3> circle_plane(n, c);
                const Domain::Vec3d proj = circle_plane.projection(f1.point());
                const double dist        = std::sqrt(
                    std::pow((proj - c).norm() - radius, 2.) + (f1.point() - proj).squaredNorm()
                );

                const Domain::Vec3d p_on_circle = c + radius * (proj - c).normalized();
                result.distance_strict          = std::make_optional(
                    DistAndPoints{dist, f1.point(), p_on_circle}
                ); // TODO
            }
            ///////////////////////////////////////////////////////////////////////////
        } else if (f2.type() == SurfaceFeatureType::Plane) {
            const auto [idx, normal, pt] = f2.plane();
            Eigen::Hyperplane<double, 3> plane(normal, pt);
            result.distance_infinite = std::make_optional(
                DistAndPoints{plane.absDistance(f1.point()), f1.point(), plane.projection(f1.point())}
            ); // TODO
        ...
        ///////////////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////
    } else if (f1.type() == SurfaceFeatureType::Edge) {
        if (f2.type() == SurfaceFeatureType::Edge) {
            ...
            result.angle = angle_edge_edge(f1.edge(), f2.edge());
            ///////////////////////////////////////////////////////////////////////////
        } else if (f2.type() == SurfaceFeatureType::Circle) {
            const std::pair<Domain::Vec3d, Domain::Vec3d> e = f1.edge();
            const auto& [center, radius, normal]            = f2.circle();
            const Domain::Vec3d e1e2                        = (e.second - e.first);
            ...
            DEBUG_ASSERT(measuring != nullptr);

            const auto [from, to]            = f1.edge();
            const auto [idx, normal, origin] = f2.plane();

            const Domain::Vec3d edge_unit = (to - from).normalized();
            if (are_perpendicular(edge_unit, normal)) {
                std::vector<DistAndPoints> distances;
                const Eigen::Hyperplane<double, 3> plane(normal, origin);
                distances.push_back(
                    DistAndPoints{plane.absDistance(from), from, plane.projection(from)}
                );
                ...
                const std::vector<SurfaceFeature>& plane_features = measuring->plane_features(idx);
                std::vector<DistAndPoints> distances;
            ...
            result.angle = angle_edge_plane(f1.edge(), f2.plane());
        }
        ///////////////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////
    } else if (f1.type() == SurfaceFeatureType::Circle) {
        if (f2.type() == SurfaceFeatureType::Circle) {
            const auto [c0, r0, n0] = f1.circle();
            const auto [c1, r1, n1] = f2.circle();
            ...
            struct ClosestInfo
            {
                ...
                Domain::Vec3d circle0Closest{Domain::Vec3d::Zero()};
                Domain::Vec3d circle1Closest{Domain::Vec3d::Zero()};

                inline bool operator<(const ClosestInfo& other) const
                {
            ...
            if (!are_parallel(n0, n1)) {
                ...
                const std::array<Domain::Vec3d, 3> basis = orthonormal_basis(n1);
                const Domain::Vec3d U1                   = basis[0];
                const Domain::Vec3d V1                   = basis[1];

                // Construct the polynomial phi(cos(theta)).
                const Domain::Vec3d N0xD  = n0.cross(D);
                const Domain::Vec3d N0xU1 = n0.cross(U1);
                const Domain::Vec3d N0xV1 = n0.cross(V1);
                ...
                for (size_t i = 0; i < numPairs; ++i) {
                    ...
                    Domain::Vec3d delta   = D + r1 * (pairs[i].first * U1 + pairs[i].second * V1);
                    ...
                    if (lenN0xDelta > 0.0) {
                        ...
                        const Domain::Vec3d r0U0 = r0 * orthogonal(n0, true);
                        const Domain::Vec3d diff = delta - r0U0;
                ...
                const Domain::Vec3d normProj = N0dD * n0;
                const Domain::Vec3d compProj = D - normProj;
                Domain::Vec3d U              = compProj;
            ...
            result.distance_infinite = std::make_optional(
                DistAndPoints{
                    std::sqrt(candidates[0].sqrDistance),
                    candidates[0].circle0Closest,
                    candidates[0].circle1Closest
                }
            ); // TODO
            ///////////////////////////////////////////////////////////////////////////
        } else if (f2.type() == SurfaceFeatureType::Plane) {
            ...
            const auto [center, radius, normal1] = f1.circle();
            const auto [idx2, normal2, origin2]  = f2.plane();

            const bool coplanar = are_parallel(normal1, normal2)
                && Eigen::Hyperplane<double, 3>(normal1, center).absDistance(origin2)
                    < Domain::EPSILON;
            if (!coplanar) {
                const std::vector<SurfaceFeature>& plane_features = measuring->plane_features(idx2);
                std::vector<DistAndPoints> distances;
                for (const SurfaceFeature& sf : plane_features) {
                    if (sf.type() == SurfaceFeatureType::Edge) {
                        const auto m = measurement(sf, f1);
        ...
        ///////////////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////
    } else if (f1.type() == SurfaceFeatureType::Plane) {
        const auto [idx1, normal1, pt1] = f1.plane();
        const auto [idx2, normal2, pt2] = f2.plane();

        if (are_parallel(normal1, normal2)) {
            // The planes are parallel, calculate distance.
            const Eigen::Hyperplane<double, 3> plane(normal1, pt1);
            result.distance_infinite = std::make_optional(
                DistAndPoints{plane.absDistance(pt2), pt2, plane.projection(pt2)}
            ); // TODO
        } else
            result.angle = angle_plane_plane(f1.plane(), f2.plane());
    }
