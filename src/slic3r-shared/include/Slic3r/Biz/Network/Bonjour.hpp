...
public:
	...
	/**
	 * @brief Sets the requested service protocol, "tcp" by default.
	 */
	Bonjour& set_protocol(std::string protocol);
	/**
	 * @brief Sets which TXT key-values should be collected. Note that "path" is always collected.
	 */
	...
	/**
	 * @brief Sets the hostname queried by resolve().
	 */
	...
	/**
	 * @brief Looks up all devices by the given TxtKeys.
	 * Each correct reply is passed back in ReplyFn, finishes with CompleteFn.
	 */
	Ptr lookup();

	/**
	 * @brief Performs resolving of hostname into a vector of IP addresses passed back by ResolveFn.
	 * Needs set_hostname and on_resolve to be called before.
	 */
	Ptr resolve();

	/**
	 * @brief Resolves on the current thread.
	 */
	void resolve_sync();
private:
...
protected:
	/**
	 * @brief Const pointer to socket to get needed data such as txt_keys, etc.
	 */
...
public:
	/**
	 * @brief Two constructors: 1st is with interface which must be resolved before calling this.
	 */
