...
public:
    ...
    bool operator==(const Halfedge_index& rhs) const
    {
        return m_face == rhs.m_face && m_side == rhs.m_side;
    }
...
public:
    ...
    {
        return Halfedge_index(v.m_face, (v.m_vertex_idx == 0 ? 2 : v.m_vertex_idx - 1));
    }

    Halfedge_index halfedge(Face_index f) const
    {
        return Halfedge_index(f, 0);
    }
    ...
    const stl_vertex& point(Vertex_index v) const
    {
        return m_its.vertices[m_its.indices[v.m_face][v.m_vertex_idx]];
    }
    ...
    {
        return m_face_neighbors[h.m_face][h.m_side] == -1;
    }

    bool is_same_vertex(const Vertex_index& a, const Vertex_index& b) const
    {
        return m_its.indices[a.m_face][a.m_vertex_idx] == m_its.indices[b.m_face][b.m_vertex_idx];
    }

    Domain::Index3 get_face_neighbors(Face_index face_id) const
    {
...
private:
    const std::vector<Domain::Index3> m_face_neighbors;
