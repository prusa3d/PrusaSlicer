...
inline const Domain::Point& contour_point(const VD::cell_type &cell, const Domain::Line &line)
	{ return ((cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line.a : line.b); }
inline Domain::Point&       contour_point(const VD::cell_type &cell, Domain::Line &line)
	{ return ((cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line.a : line.b); }

inline const Domain::Point& contour_point(const VD::cell_type &cell, const Domain::Lines &lines)
	{ return contour_point(cell, lines[cell.source_index()]); }
inline Domain::Point&       contour_point(const VD::cell_type &cell, Domain::Lines &lines)
	{ return contour_point(cell, lines[cell.source_index()]); }

inline Domain::Vec2d 		vertex_point(const VD::vertex_type &v) { return Domain::Vec2d(v.x(), v.y()); }
inline Domain::Vec2d 		vertex_point(const VD::vertex_type *v) { return Domain::Vec2d(v->x(), v->y()); }
...
static inline bool edge_offset_no_intersection(const Domain::Vec2d &intersection_point)
	{ return std::isnan(intersection_point.x()); }
static inline bool edge_offset_has_intersection(const Domain::Vec2d &intersection_point)
	{ return ! edge_offset_no_intersection(intersection_point); }
std::vector<Domain::Vec2d> edge_offset_contour_intersections(
	const VD &vd, const Domain::Lines &lines, const std::vector<double> &distances,
	double offset_distance);

std::vector<Domain::Vec2d> skeleton_edges_rough(
    const VD                    &vd,
