...
namespace Slic3r::Biz::CGAL::Algorithms::VoronoiUtils
{
...
namespace Voronoi { namespace Internal {
    ...
    inline point_type retrieve_point(const Domain::Points &points, const std::vector<segment_type> &segments, const cell_type& cell)
    {
    ...
    inline void clip_infinite_edge(const Domain::Points &points, const std::vector<segment_type> &segments, const edge_type& edge, coordinate_type bbox_max_size, std::vector<point_type>* clipped_edge)
    {
    ...
    inline void sample_curved_edge(const Domain::Points &points, const std::vector<segment_type> &segments, const edge_type& edge, std::vector<point_type> &sampled_edge, coordinate_type max_dist)
    {
...
static inline void dump_voronoi_to_svg(
    ...
    const Domain::Polygons      &offset_curves = Domain::Polygons(),
    const Domain::Lines         &helper_lines = Domain::Lines(),
    double               scale = 0)
{
    ...
    bbox = BB::merge(bbox, BB::construct(points));
    bbox = BB::merge(bbox, Biz::Algorithms::Line::get_extents(lines));
    bbox = BB::merge(bbox, Biz::Algorithms::Polygon::get_extents(offset_curves));
    bbox = BB::merge(bbox, Biz::Algorithms::Line::get_extents(helper_lines));
    ...
    for (boost::polygon::voronoi_diagram<double>::const_vertex_iterator it = vd.vertices().begin(); it != vd.vertices().end(); ++it)
        if (! internalEdgesOnly || it->color() != Voronoi::Internal::EXTERNAL_COLOR)
            bbox = BB::merge(bbox, Domain::Point(round(Domain::Vec2d{it->x(), it->y()}).cast<Domain::coord_t>()));
    bbox.min -= (0.01 * BB::sizes(bbox).cast<double>()).cast<Domain::coord_t>();
    bbox.max += (0.01 * BB::sizes(bbox).cast<double>()).cast<Domain::coord_t>();
...
//                0.1
        ...
        scale *= Domain::SCALING_FACTOR;

    const std::string     inputSegmentPointColor      = "lightseagreen";
    const Domain::coord_t inputSegmentPointRadius     = std::max<Domain::coord_t>(1, Domain::coord_t(0.09 * scale));
    const std::string     inputSegmentColor           = "lightseagreen";
    const Domain::coord_t inputSegmentLineWidth       = Domain::coord_t(0.03 * scale);
    ...
    const Domain::coord_t voronoiPointRadius          = std::max<Domain::coord_t>(1, Domain::coord_t(0.06 * scale));
    ...
    const Domain::coord_t voronoiLineWidth            = Domain::coord_t(0.02 * scale);

    const std::string     offsetCurveColor            = "magenta";
    const Domain::coord_t offsetCurveLineWidth        = Domain::coord_t(0.02 * scale);

    const std::string     helperLineColor             = "orange";
    const Domain::coord_t helperLineWidth             = Domain::coord_t(0.04 * scale);
    ...
    for (Domain::Lines::const_iterator it = lines.begin(); it != lines.end(); ++ it)
    ...
    for (Domain::Lines::const_iterator it = lines.begin(); it != lines.end(); ++it)
        svg.draw(Domain::Line(Domain::Point(Domain::coord_t(it->a(0)), Domain::coord_t(it->a(1))), Domain::Point(Domain::coord_t(it->b(0)), Domain::coord_t(it->b(1)))), inputSegmentColor, inputSegmentLineWidth);
    ...
    for (boost::polygon::voronoi_diagram<double>::const_vertex_iterator it = vd.vertices().begin(); it != vd.vertices().end(); ++it)
        if (! internalEdgesOnly || it->color() != Voronoi::Internal::EXTERNAL_COLOR) {
            ...
            switch (Algorithms::Voronoi::vertex_category(*it)) {
            case Algorithms::Voronoi::VertexCategory::OnContour:    color = &voronoiPointColor;         break;
            case Algorithms::Voronoi::VertexCategory::Outside:      color = &voronoiPointColorOutside;  break;
            case Algorithms::Voronoi::VertexCategory::Inside:       color = &voronoiPointColorInside;   break;
            default: color = &voronoiPointColor; // assert(false);
            }
            Domain::Point pt(Domain::coord_t(it->x()), Domain::coord_t(it->y()));
            if (it->x() * pt.x() >= 0. && it->y() * pt.y() >= 0.)
                // Conversion to coord_t is valid.
                svg.draw(Domain::Point(Domain::coord_t(it->x()), Domain::coord_t(it->y())), *color, voronoiPointRadius);
        }
    ...
    for (boost::polygon::voronoi_diagram<double>::const_edge_iterator it = vd.edges().begin(); it != vd.edges().end(); ++it) {
        ...
        for (std::size_t i = 0; i + 1 < samples.size(); ++ i) {
            Domain::Vec2d a(samples[i].x(), samples[i].y());
            Domain::Vec2d b(samples[i+1].x(), samples[i+1].y());
            // Convert to coord_t.
            Domain::Point ia = a.cast<Domain::coord_t>();
            Domain::Point ib = b.cast<Domain::coord_t>();
            // Is the conversion possible? Do the resulting points fit into int32_t?
            auto  in_range = [](const Domain::Point &ip, const Domain::Vec2d &p) { return p.x() * ip.x() >= 0. && p.y() * ip.y() >= 0.; };
            ...
            if (! a_in_range || ! b_in_range) {
                ...
                auto p = a_in_range ? Domain::Vec2d(a + v) : Domain::Vec2d(b - v);
                Domain::Point ip = p.cast<Domain::coord_t>();
                if (! in_range(ip, p))
                    continue;
                (a_in_range ? ib : ia) = ip;
            }
            svg.draw(Domain::Line(ia, ib), color, voronoiLineWidth);
        }
    }
#endif
