...
struct VoronoiGraph::Node::Neighbor
{
    ...
    struct Size{
        ...
        // maximal width
        Domain::coord_t min_width;
        // minimal widht
        Domain::coord_t max_width;

        Size(double length, Domain::coord_t min_width, Domain::coord_t max_width)
...
public:
    ...
    Domain::coord_t min_width() const { return size->min_width; }
    Domain::coord_t max_width() const { return size->max_width; }
};
...
struct VoronoiGraph::Position
{
    ...
    Domain::coord_t calc_distance() const {
        return static_cast<Domain::coord_t>(neighbor->length() * ratio);
    }

    Domain::coord_t calc_rest_distance() const
    {
        return static_cast<Domain::coord_t>(neighbor->length() * (1. - ratio));
    }
};

inline VoronoiGraph::Node::Node(const VD::vertex_type* vertex, double distance)
