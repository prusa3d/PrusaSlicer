...
{
    const Domain::Point &point = parabola.focus;
    const Domain::Line & line  = parabola.directrix;
    Domain::Line norm_line(point, point + line.normal());
...
{
    Domain::Vec2big line_direction = (parabola.directrix.b - parabola.directrix.a).cast<int64_t>();
    Domain::Vec2big focus_from     = (parabola.focus - parabola.from).cast<int64_t>();
    Domain::Vec2big focus_to       = (parabola.focus - parabola.to).cast<int64_t>();;
                         ...
                         double                 discretization_step)
{
    ...
    for (size_t index = 1; index < parabola_samples.size(); ++index) {
        ...
        Domain::Line l(
            Domain::Point(round(Domain::Vec2d{s0.x(), s0.y()}).cast<Domain::coord_t>()),
            Domain::Point(round(Domain::Vec2d{s1.x(), s1.y()}).cast<Domain::coord_t>())
        );
