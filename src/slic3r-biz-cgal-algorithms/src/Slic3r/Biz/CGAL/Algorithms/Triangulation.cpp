    ...
    const Triangulation::HalfEdges &constrained_half_edges)
{
    ...
    return !Biz::Algorithms::get_intersections(lines).empty();
}
...
{
    size_t count = Biz::Algorithms::Polygon::count_points(polygons);
    Points points;
...
Triangulation::Indices Triangulation::triangulate(const ExPolygons &expolygons){
    Points pts = Biz::Algorithms::ExPolygon::to_points(expolygons);
    Points d_pts = Biz::Algorithms::Point::collect_duplicates(pts);
...
{
    assert(Biz::Algorithms::ExPolygon::count_points(expolygons) == points.size());
    // when contain duplicit coordinate in points will not work properly
    assert(Biz::Algorithms::Point::collect_duplicates(points).empty());
