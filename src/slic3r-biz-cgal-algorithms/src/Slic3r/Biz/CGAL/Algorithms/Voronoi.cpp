...
using Domain::Lines;
using Biz::Algorithms::PolygonsSegmentIndex;
using Biz::Algorithms::ColoredLines;
using Slic3r::Biz::Algorithms::Geometry::PI;

using PolygonsSegmentIndexConstIt = std::vector<PolygonsSegmentIndex>::const_iterator;
    ...
    if (try_to_repair_if_needed) {
        if (m_issue_type = detect_known_issues(*this, segment_begin, segment_end); m_issue_type != IssueType::NO_ISSUE_DETECTED) {
            if (m_issue_type == IssueType::MISSING_VORONOI_VERTEX) {
                SPDLOG_WARN("Detected missing Voronoi vertex, input polygons will be rotated back and forth.");
            } else if (m_issue_type == IssueType::NON_PLANAR_VORONOI_DIAGRAM) {
                SPDLOG_WARN("Detected non-planar Voronoi diagram, input polygons will be rotated back and forth.");
            } else if (m_issue_type == IssueType::VORONOI_EDGE_INTERSECTING_INPUT_SEGMENT) {
                SPDLOG_WARN("Detected Voronoi edge intersecting input segment, input polygons will be rotated back and forth.");
            } else if (m_issue_type == IssueType::FINITE_EDGE_WITH_NON_FINITE_VERTEX) {
                SPDLOG_WARN("Detected finite Voronoi vertex with non finite vertex, input polygons will be rotated back and forth.");
            } else if (m_issue_type == IssueType::PARABOLIC_VORONOI_EDGE_WITHOUT_FOCUS_POINT) {
                SPDLOG_WARN("Detected parabolic Voronoi edges without focus point, input polygons will be rotated back and forth.");
            } else {
                SPDLOG_ERROR("Detected unknown Voronoi diagram issue, input polygons will be rotated back and forth.");
            }
            ...
            if (m_issue_type = try_to_repair_degenerated_voronoi_diagram(segment_begin, segment_end); m_issue_type != IssueType::NO_ISSUE_DETECTED) {
                if (m_issue_type == IssueType::MISSING_VORONOI_VERTEX) {
                    SPDLOG_ERROR("Detected missing Voronoi vertex even after the rotation of input.");
                } else if (m_issue_type == IssueType::NON_PLANAR_VORONOI_DIAGRAM) {
                    SPDLOG_ERROR("Detected non-planar Voronoi diagram even after the rotation of input.");
                } else if (m_issue_type == IssueType::VORONOI_EDGE_INTERSECTING_INPUT_SEGMENT) {
                    SPDLOG_ERROR("Detected Voronoi edge intersecting input segment even after the rotation of input.");
                } else if (m_issue_type == IssueType::FINITE_EDGE_WITH_NON_FINITE_VERTEX) {
                    SPDLOG_ERROR("Detected finite Voronoi vertex with non finite vertex even after the rotation of input.");
                } else if (m_issue_type == IssueType::PARABOLIC_VORONOI_EDGE_WITHOUT_FOCUS_POINT) {
                    SPDLOG_ERROR("Detected parabolic Voronoi edges without focus point even after the rotation of input.");
                } else {
                    SPDLOG_ERROR("Detected unknown Voronoi diagram issue even after the rotation of input.");
                }
                                                 ...
                                                 const SegmentIterator segment_end)
{
    ...
    for (VD::cell_type cell : voronoi_diagram.cells()) {
        ...
        if (const SegmentCellRange cell_range = VoronoiUtils::compute_segment_cell_range(cell, segment_begin, segment_end); cell_range.is_valid()) {
            ...
            const Segment &source_segment      = VoronoiUtils::get_source_segment(cell, segment_begin, segment_end);
            const Domain::Vec2d    source_segment_from = boost::polygon::segment_traits<Segment>::get(source_segment, boost::polygon::LOW).template cast<double>();
            const Domain::Vec2d    source_segment_to   = boost::polygon::segment_traits<Segment>::get(source_segment, boost::polygon::HIGH).template cast<double>();
            const Domain::Vec2d    source_segment_vec  = source_segment_to - source_segment_from;
            ...
            for (const VD::edge_type *edge = cell_range.edge_begin; edge != cell_range.edge_end; edge = edge->next()) {
                ...
                } else if (const Domain::Vec2d edge_v1(edge->vertex1()->x(), edge->vertex1()->y()); Slic3r::cross2(source_segment_vec, edge_v1 - source_segment_from) < 0) {
