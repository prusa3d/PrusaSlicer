...
using Domain::ExPolygonsIndex;
using Domain::ExPolygonsIndices;
using Domain::Point;
using Domain::Points;
...
using Domain::Vec3d;
using Domain::Vec3f;
using Linef = Domain::Line2d;
using Biz::Algorithms::ClipperUtils::ApplySafetyOffset;
using Biz::Algorithms::ClipperUtils::intersection_ex;
using Biz::Algorithms::ClipperUtils::offset_ex;
using Biz::Algorithms::ClipperUtils::union_ex;
using Biz::Algorithms::ExPolygon::count_points;
using Domain::coord_t;

namespace priv {

using Project   = Slic3r::Biz::Algorithms::IProjection;
using Project3d = Slic3r::Biz::Algorithms::IProject3d;
...
void set_skip_for_out_of_aoi(
    std::vector<bool>& skip_indicies,
    const indexed_triangle_set& its,
    const Project& projection,
    const BoundingBox2crd& shapes_bb
);
...
inline Vec3d to_vec3d(const P3& p)
{
...
struct IntersectingElement
{
    ...
    Type get_type() const
    {
        return static_cast<Type>(attr % 8);
    }

    bool is_first() const
    {
        return 8 <= attr && attr < 16;
    }
...
std::vector<bool> select_patches(
...
void store(
    const ExPolygons& shapes,
    const std::vector<bool>& mask_distances,
...
)
{
    ...
    for (priv::CutMesh& cgal_model : cgal_models) {
        priv::CutAOIs
            cutAOIs = priv::cut_from_model(cgal_model, shapes, cgal_shape, projection_ratio, s2i);
#ifdef DEBUG_OUTPUT_DIR
        size_t index = &cgal_model - &cgal_models.front();
        priv::store(
            cutAOIs,
            cgal_model,
            DEBUG_OUTPUT_DIR + "model_AOIs/" + std::to_string(index) + "/"
        ); // only debug
#endif // DEBUG_OUTPUT_DIR
    ...
    priv::ProjectionDistances
        best_projection = priv::choose_best_distance(distances, shapes, start, s2i, patches);
    std::vector<bool> use_patch = priv::select_patches(
        best_projection,
        patches,
        shapes,
        shapes_bb,
...
indexed_triangle_set cut2model(const SurfaceCut& cut, const Biz::Algorithms::IProject3d& projection)
{
    assert(!cut.empty());
...
void priv::set_skip_for_out_of_aoi(
    std::vector<bool>& skip_indicies,
    const indexed_triangle_set& its,
    const Project& projection,
    const BoundingBox2crd& shapes_bb
)
{
    ...
    for (Point v :
         {Point{shapes_bb.min},
          Point{shapes_bb.min.x(), shapes_bb.max.y()},
          Point{shapes_bb.max},
          Point{shapes_bb.max.x(), shapes_bb.min.y()}})
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, its.vertices.size()),
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, its.indices.size()),
...
indexed_triangle_set its_cut_AoI(
    const indexed_triangle_set& its,
    const BoundingBox2crd& bb,
    const Biz::Algorithms::IProjection& projection
)
{
    std::vector<bool> skip_indicies(its.indices.size(), false);
...
priv::CutMesh priv::to_cgal(const indexed_triangle_set& its, const std::vector<bool>& skip_indicies, bool flip)
{
    ...
    for (const auto& t : indices) {
        ...
        case 3:
            break; // all edges are already counted
        case 2:
            edges_count += 2;
            break;
        case 1:
...
priv::CutMesh priv::to_cgal(const ExPolygons& shapes, const Project& projection)
{
    ...
    CutMesh result;
    EdgeShapeMap edge_shape_map = result
                                      .add_property_map<EI, IntersectingElement>(edge_shape_map_name)
                                      .first;
    FaceShapeMap face_shape_map = result
                                      .add_property_map<FI, IntersectingElement>(face_shape_map_name)
                                      .first;

    std::vector<VI> indices;
    auto insert_contour =
        [&projection, &indices, &result, &edge_shape_map, &face_shape_map](const Polygon& polygon) {
            indices.clear();
            ...
            for (int32_t i = 0; i < int32_t(indices.size()); i += 2) {
                ...
                IntersectingElement element{
                    contour_index,
                    (unsigned char) IntersectingElement::Type::undefined
                };
                if (is_first)
    ...
    result.reserve(
        result.number_of_vertices() + 2 * count_point,
...
uint32_t priv::ModelCut2index::calc_index(const ModelCutId& id) const
{
    ...
    assert(
        ((id.model_index + 1) < m_offsets.size() && res < m_offsets[id.model_index + 1])
...
CutAOIs create_cut_area_of_interests(
    const CutMesh& mesh,
    const ExPolygons& shapes,
    FaceTypeMap& face_type_map
);
...
)
{
    ...
    VertexShapeMap vert_shape_map = cgal_model
                                        .add_property_map<VI, const IntersectingElement*>(
                                            vert_shape_map_name,
                                            nullptr
                                        )
                                        .first;
    ...
    const EdgeShapeMap&
        edge_shape_map = cgal_shape.property_map<EI, IntersectingElement>(edge_shape_map_name).first;
    const FaceShapeMap&
        face_shape_map = cgal_shape.property_map<FI, IntersectingElement>(face_shape_map_name).first;
    ...
    FaceTypeMap face_type_map = cgal_model
                                    .add_property_map<FI, FaceType>(
                                        face_type_map_name,
                                        FaceType::not_constrained
                                    )
                                    .first;
...
{
    const VertexShapeMap& vert_shape_map = mesh.property_map<VI, const IntersectingElement*>(
                                                   vert_shape_map_name
    )
                                               .first;
...
priv::CutAOIs priv::create_cut_area_of_interests(
    const CutMesh& mesh,
    const ExPolygons& shapes,
    FaceTypeMap& face_type_map
)
...
)
{
    ...
    for (const SurfacePatch& patch : patches) {
        ...
        const VertexShapeMap& vert_shape_map = models[patch.model_id]
                                                   .property_map<VI, const IntersectingElement*>(
                                                       vert_shape_map_name
                                                   )
                                                   .first;
...
struct SearchData
{
    ...
    Biz::Algorithms::AABBTreeIndirect::Tree<2, double> tree;
};
...
uint32_t find_closest_point_index(
    const Point& p,
    const ExPolygons& shapes,
    const ExPolygonsIndices& s2i,
...
std::pair<uint32_t, uint32_t> find_closest_point_pair(
    const ExPolygons& shapes,
...
)
{
    ...
    [[maybe_unused]] auto is_same = [&s2i, &shapes](const Vec2d& p, size_t i) -> bool {
        ...
        const Polygon& poly    = (id.polygon_index == 0) ? shape.contour :
                                                           shape.holes[id.polygon_index - 1];
        Vec2crd p_             = p.cast<int>();
    ...
    const ExPolygon& shape      = shapes[id.expolygons_index];
    size_t count_polygon_points = (id.polygon_index == 0) ? shape.contour.size() :
...
// use AABB Tree Lines
uint32_t priv::find_closest_point_index(
    const Point& p,
    const ExPolygons& shapes,
    const ExPolygonsIndices& s2i,
    const std::vector<bool>& mask
)
{
    ...
    Vec2d hit_point;
    Vec2d p_d                           = p.cast<double>();
    [[maybe_unused]] double distance_sq = AABBTreeLines::squared_distance_to_indexed_lines(
        sd.lines,
        sd.tree,
        p_d,
        line_idx,
...
const priv::ProjectionDistance* priv::get_closest_projection(
    const ProjectionDistances& distance,
...
void priv::fill_polygon_distances(
    const ProjectionDistance& pd,
    uint32_t index,
    const ExPolygonsIndex& id,
    ProjectionDistances& result,
...
{
    const Points& points = (id.polygon_index == 0) ? shape.contour.points :
...
void priv::fill_shape_distances(
    uint32_t start_index,
    const ProjectionDistance* start_pd,
    ProjectionDistances& result,
    const ExPolygonsIndices& s2i,
...
)
{
    ...
    do {
        ...
        auto find_close_finished_point =
            [&first_shape_index, &shape, &result](const Point& p) -> ClosePoint {
        ...
        uint32_t index               = first_shape_index;
        auto check_unfinished_points = [&index,
                                        &result,
                                        &distances,
                                        &find_close_finished_point,
                                        &dist_sq,
                                        &start_index,
...
{
    assert(distances.size() == Biz::Algorithms::ExPolygon::count_points(shapes));
    ...
    do {
        const ProjectionDistance* pd = get_closest_projection(
            distances[unfinished_index],
            wanted_distance
        );
...
SurfacePatch create_surface_patch(
    const std::vector<FI>& fis,
    /*const*/ CutMesh& mesh,
...
bool priv::clip_cut(SurfacePatch& cut, CutMesh clipper)
{
    ...
    class ExistIntersectionClipVisitor :
        public ::CGAL::Polygon_mesh_processing::Corefinement::Default_visitor<CutMesh>
    {
        bool* exist_intersection;

    public:
        ExistIntersectionClipVisitor(bool* exist_intersection) :
...
priv::SurfacePatch priv::create_surface_patch(
    const std::vector<FI>& fis,
    /* const */ CutMesh& mesh,
...
bool priv::is_patch_inside_of_model(const SurfacePatch& patch, const Tree& tree, const Project3d& projection)
{
    ...
    assert(
        ((tree.number_of_intersected_primitives(
              Ray(a, P3(2 * a.x() - b.x(), 2 * a.y() - b.y(), 2 * a.z() - b.z()))
          )
          % 2)
         == 1)
        == is_in
...
{
    // map is created during intersection by corefine visitor
    const VertexShapeMap&
        vert_shape_map = model.property_map<VI, const IntersectingElement*>(vert_shape_map_name).first;
...
priv::SurfacePatch priv::separate_patch(
    const std::vector<FI>& fis,
    SurfacePatch& patch,
    const CvtVI2VI& cvt_from
)
...
void priv::divide_patch(size_t i, SurfacePatchesEx& patches)
{
    ...
    CutMesh::Property_map<FI, bool>
        is_processed = cm.add_property_map<FI, bool>(patch_number_name, false).first;
...
priv::SurfacePatches priv::diff_models(
    VCutAOIs& cuts,
    /*const*/ CutMeshes& cut_models,
    /*const*/ CutMeshes& models,
    const Project3d& projection
)
{
    ...
    for (size_t model_index = 0; model_index < models.size(); ++model_index) {
        ...
        ReductionMap vertex_reduction_map = cut_model_
                                                .add_property_map<VI, VI>(vertex_reduction_map_name)
                                                .first;
...
bool priv::is_over_whole_expoly(
    const SurfacePatch& patch,
    const ExPolygons& shapes,
    const VCutAOIs& cutAOIs,
    const CutMeshes& meshes
)
{
    if (!patch.is_whole_aoi)
        return false;
    return is_over_whole_expoly(
        cutAOIs[patch.model_id][patch.aoi_id],
...
)
{
    ...
    for (size_t shape_index = 0; shape_index < shapes.size(); shape_index++) {
        ...
        for (uint32_t patch_index : shapes_patches[shape_index]) {
            ...
            patch_shapes.push_back(
                {patch_index, patch_shape, patch_intersection, depth_range_center_distance}
            );
        }
...
ExPolygon priv::to_expoly(const SurfacePatch& patch, const Project& projection, Vec2d& depth_range)
{
    ...
    ExPolygons expolys                 = union_ex(polys, fill_type);
    if (expolys.size() == 1)
...
SurfaceCut priv::patch2cut(SurfacePatch& patch)
{
    ...
    CutMesh::Property_map<VI, SurfaceCut::Index>
        convert_map = mesh.add_property_map<VI, SurfaceCut::Index>(convert_map_name).first;
...
/// <returns></returns>
indexed_triangle_set create_contour_its(
    const indexed_triangle_set& its,
    const std::vector<unsigned int>& contour
);
