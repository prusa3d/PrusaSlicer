...
namespace Geometry = Biz::Algorithms::Geometry;

namespace{
// conversion from double to coor_t
Point to_point(const Site_2 &s) { return Point(s.x(), s.y()); }
...
Line create_line_between_points(
...
/// <returns></returns>
std::optional<Line> crop_ray(const Point_2 &ray_point, const Point &v1, const Point &v2, double maximal_distance) {
...
Polygons create_voronoi_cells_cgal(const Points &points, coord_t max_distance) {
    ...
    for (VD::Face_iterator fit = vd.faces_begin(); fit != vd.faces_end(); ++fit, ++fit_index) {
        ...
        Point source_pt(source_point.x(), source_point.y());
        ...
        do {
            assert(ec->is_valid());
            std::optional<Line> line_opt = to_line(ec, max_distance);
        ...
        assert(!lines.empty());
        if (lines.size() > 1)
            Biz::Algorithms::LineUtils::sort_CCW(lines, origin);
