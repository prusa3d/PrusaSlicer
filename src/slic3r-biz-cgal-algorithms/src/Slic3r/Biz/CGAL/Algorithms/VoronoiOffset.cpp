...
#include <Slic3r/Biz/Algorithms/Geometry/VoronoiVisualUtils.hpp>
#endif

namespace Slic3r::Biz::CGAL::Algorithms {

namespace Geometry = Slic3r::Biz::Algorithms::Geometry;

template <typename T, typename Number>
constexpr inline T lerp(const T& a, const T& b, Number t)
{
    assert((t >= Number(-Domain::EPSILON)) && (t <= Number(1) + Number(Domain::EPSILON)));
...
namespace detail {
    ...
    Intersections point_point_equal_distance_points(const Domain::Point &pt1, const Domain::Point &pt2, const double d)
    {
    ...
    Intersections line_point_equal_distance_points(const Domain::Line &line, const Domain::Point &ipt, const double d)
    {   
    ...
    inline bool vertex_equal_to_point(const VertexType &vertex, const Domain::Point &ipt)
    {
    ...
    bool vertex_equal_to_point(const VD::vertex_type *vertex, const Domain::Point &ipt)
        { return vertex_equal_to_point(*vertex, ipt); }

    double dist_to_site(const Domain::Lines &lines, const VD::cell_type &cell, const Vec2d &point)
    {
    ...
    bool on_site(const Domain::Lines &lines, const VD::cell_type &cell, const Vec2d &pt)
    {
        const Domain::Line &line = lines[cell.source_index()];
        auto on_contour = [&pt](const Domain::Point &ipt) { return detail::vertex_equal_to_point(pt, ipt); };
    ...
    std::pair<Vec2d, Vec2d> point_point_dr_dl_thresholds(
        // Two Voronoi sites
        const Domain::Point &pt1_site, const Domain::Point &pt2_site,
    ...
    std::pair<Vec2d, Vec2d> point_segment_dr_dl_thresholds(
        // Two Voronoi sites
        const Domain::Point &pt_site, const Domain::Line &line_site,
        // End points of a Voronoi segment
        const Vec2d &voronoi_point1, const Vec2d &voronoi_point2,
        // Threshold of the skeleton function, where alpha is an angle of a sharp convex corner with the same dr/dl.
        const double threshold_tan_alpha_half)
...
namespace debug
{
    ...
    bool verify_twin_halfedges_successive(const VD &vd, const Domain::Lines &lines)
    {
        for (size_t i = 0; i < vd.num_edges(); i += 2) {
            ...
            if (e.is_secondary()) {
                ...
                const Domain::Line  &line0 = lines[ex.cell()->source_index()];
                const Domain::Line  &line1 = lines[ex.twin()->cell()->source_index()];
                const Domain::Point &pt    = (ex.cell()->source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line0.a : line0.b;
...
//                        assert(edge->vertex0() != nullptr);
    ...
    bool verify_vertices_on_contour(const VD &vd, const Domain::Lines &lines)
    {
    ...
    bool verify_signed_distances(const VD &vd, const Domain::Lines &lines, const std::vector<double> &signed_distances)
    {
        for (const VD::edge_type &edge : vd.edges()) {
            const VD::vertex_type *v = edge.vertex0();
    ...
    bool verify_offset_intersection_points(const VD &vd, const Domain::Lines &lines, const double offset_distance, const std::vector<Vec2d> &offset_intersection_points)
    {
...
void annotate_inside_outside(VD &vd, const Domain::Lines &lines)
{
    ...
    for (const VD::edge_type &edge : vd.edges()) {
        if (edge.is_secondary() && edge.vertex0() != nullptr) {
            ...
            const Domain::Point &pt_on_contour = edge.cell()->contains_point() ? contour_point(*edge.cell(), lines) : contour_point(*edge.twin()->cell(), lines);
    ...
    for (const VD::edge_type &edge : vd.edges())
        if (edge.vertex1() == nullptr) {
            ...
            const VD::cell_type *cell = edge.cell();
            const Domain::Line          *line = cell->contains_segment() ? &lines[cell->source_index()] : nullptr;
            if (line == nullptr) {
                cell = edge.twin()->cell();
            ...
            if (line) {
                ...
                if (! on_contour && cell == edge.cell() && edge.twin()->cell()->contains_segment()) {
                    ...
                    const Domain::Point *pt_on_contour = nullptr;
                    ...
                    const Domain::Line &line2 = lines[cell2->source_index()];
                    if (line->a == line2.b)
...
std::vector<double> signed_vertex_distances(const VD &vd, const Domain::Lines &lines)
{
    ...
    double                       offset_distance)
{
    ...
    for (const VD::edge_type &edge : vd.edges()) {
        ...
        const VD::cell_type   *cell  = edge.cell();
        const VD::cell_type   *cell2 = edge.twin()->cell();
        const Domain::Line            &line0 = lines[cell->source_index()];
        const Domain::Line            &line1 = lines[cell2->source_index()];
        ...
        if (v1 == nullptr) {
            ...
            if (offset_distance > dmin) {
                ...
                if (cell->contains_point() && cell2->contains_point()) {
                    ...
                    const Domain::Point &pt0 = contour_point(*cell, line0);
                    const Domain::Point &pt1 = contour_point(*cell2, line1);
                    ...
                    assert(edge.is_secondary());
                    const Domain::Point &ipt = cell->contains_segment() ? contour_point(*cell2, line1) : contour_point(*cell, line0);
    #ifndef NDEBUG
                    if (cell->contains_segment()) {
                        const Domain::Point &pt1 = contour_point(*cell2, line1);
                    ...
                    const Domain::Line &line = cell->contains_segment() ? line0 : line1;
                    assert(line.a == ipt || line.b == ipt);
            ...
            if (bisector || edge.is_secondary()) {
                ...
                if (! bisector || (dmin != dmax && offset_distance >= dmin)) {
                    ...
                    t = std::clamp(t, 0., 1.);
                    if (d1 < d0) {
                        out[edge_idx2] = lerp(vertex_point(v1), vertex_point(v0), t);
                        // mark visited
                        out[edge_idx].y() = 1.;
                    } else {
                        out[edge_idx] = lerp(vertex_point(v0), vertex_point(v1), t);
                        // mark visited
                        out[edge_idx2].y() = 1.;
                    }
                    done = true;
                ...
                const Domain::Point &pt0                = cell->contains_point() ? contour_point(*cell, line0) : contour_point(*cell2, line1);
                // Project p0 to line segment <v0, v1>.
                Vec2d p0(v0->x(), v0->y());
                Vec2d p1(v1->x(), v1->y());
                Vec2d px(pt0.x(), pt0.y());
                const Domain::Point *pt1 = nullptr;
                Vec2d dir;
                if (point_vs_segment) {
                    const Domain::Line &line = cell->contains_segment() ? line0 : line1;
				...
				if (cell->contains_point()) {
                    ...
                    if (cos_a < cos_threshold * norm) {
						...
						for (size_t i = 1; i < n_steps; ++ i, a += astep) {
                            ...
                            poly.points.emplace_back(Domain::Point(Domain::coord_t(p.x()), Domain::coord_t(p.y())));
						}
                    }
				}
                {
                    Domain::Point pt_last(Domain::coord_t(p2.x()), Domain::coord_t(p2.y()));
