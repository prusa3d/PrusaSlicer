...
using Domain::Vec2d;
using Biz::Algorithms::PointUtils;
using Biz::Algorithms::LineUtils;
using Biz::Algorithms::PolygonUtils;
using Biz::Algorithms::Parabola;
using Biz::Algorithms::ParabolaSegment;
using Biz::Algorithms::CallStack;
using Biz::Algorithms::IStackFunction;
...
Domain::Point VoronoiGraphUtils::to_point(const VD::vertex_type *vertex)
{
    return Domain::Point(to_coord(vertex->x()), to_coord(vertex->y()));
}

VoronoiGraphUtils::VD::point_type VoronoiGraphUtils::to_point(const Domain::Point &point)
{
    return VD::point_type(point.x(), point.y());
}

Vec2d VoronoiGraphUtils::to_point_d(const VD::vertex_type *vertex)
{
    return Vec2d(vertex->x(), vertex->y());
}

VoronoiGraphUtils::VD::segment_type VoronoiGraphUtils::to_segment(const Domain::Line &line)
{
    return VD::segment_type(to_point(line.a), to_point(line.b));
}

Domain::Point VoronoiGraphUtils::to_direction(const VD::edge_type *edge)
{
    return to_direction_d(edge).cast<coord_t>();
}

Vec2d VoronoiGraphUtils::to_direction_d(const VD::edge_type *edge) 
{
...
Domain::Line VoronoiGraphUtils::create_line_between_source_points(
    const Domain::Point &point1, const Domain::Point &point2, double maximal_distance)
{
    Domain::Point middle = (point1 + point2) / 2;
    Domain::Point diff = point1 - point2;
    ...
    Domain::Point  side_dir(round(Vec2d{-diff.y() * scale, diff.x() * scale}).cast<coord_t>());
    return Domain::Line(middle - side_dir, middle + side_dir);
}

std::optional<Domain::Line> VoronoiGraphUtils::to_line(
    const VD::edge_type &edge, const Domain::Points &points, double maximal_distance)
{
    assert(edge.is_linear());
    assert(edge.is_primary());
    const Domain::Point &p1 = retrieve_point(points, *edge.cell());
    const Domain::Point &p2 = retrieve_point(points, *edge.twin()->cell());
    const VD::vertex_type *v0 = edge.vertex0();
    const VD::vertex_type *v1 = edge.vertex1();
    ...
    if (edge.is_finite()) {
        ...
        if (!is_v0_in_limit) { 
            ...
            return Domain::Line(to_point(v0), to_point(v1));
        }
    } else if (v0 == nullptr) {
        if (v1 == nullptr) 
    ...
    Domain::Point direction  = (use_v1) ? 
        Domain::Point(p2.y() - p1.y(), p1.x() - p2.x()) :
        Domain::Point(p1.y() - p2.y(), p2.x() - p1.x());
    const VD::vertex_type* edge_vertex = (use_v1) ? v1 : v0;
    // koeficient for crop line
    if (!use_double_precision) {
        Domain::Point ray_point = to_point(edge_vertex);
        Domain::Line ray(ray_point, ray_point + direction);
        return LineUtils::crop_half_ray(ray, p1, maximal_distance);
    }
    std::optional<Domain::Line2d> segment;
    if (use_both) { 
        Domain::Line2d edge_segment(to_point_d(v0), to_point_d(v1));
        ...
        Domain::Line2d  line_short(start_point, start_point + direction.cast<double>());
        segment = LineUtils::crop_half_ray(line_short, p1, maximal_distance);
    }
    if (!segment.has_value()) return {};
    return Domain::Line(segment->a.cast<coord_t>(), segment->b.cast<coord_t>());
}


Domain::Polygon VoronoiGraphUtils::to_polygon(const Domain::Lines &lines,
                                              ...
                                              size_t       count_points)
{
    ...
    const Domain::Line *prev_line = &lines.back();
    double max_angle = 2 * M_PI / count_points;
    for (const Domain::Line &line : lines) {
        const Domain::Point &p1   = prev_line->b;
        const Domain::Point &p2   = line.a;
        prev_line         = &line;
        Domain::Point diff = p1-p2;
        if (abs(diff.x()) < minimal_distance &&
            abs(diff.y()) < minimal_distance) {
            Domain::Point avg   = (p1 + p2) / 2;
            points.push_back(avg);
            continue;
        } 
        Domain::Point  v1 = p1 - center;
        Domain::Point  v2 = p2 - center;
    ...
    Domain::Polygon polygon(points);
    if (!Biz::Algorithms::Polygon::contains(polygon, center)) {
        draw(polygon, lines, center); 
    }
    assert(polygon.is_valid());
    assert(Biz::Algorithms::Polygon::contains(polygon, center));
    assert(PolygonUtils::is_not_self_intersect(polygon, center));
    return polygon;
}
    
Domain::Polygon VoronoiGraphUtils::to_polygon(const VD::cell_type & cell,
                                              const Domain::Points &points,
                                              double maximal_distance)
{
    using Biz::Algorithms::Geometry::Orientation;
    using Biz::Algorithms::Geometry::orient;

    Domain::Lines lines;
    Domain::Point center = points[cell.source_index()];
    // Convenient way to iterate edges around Voronoi cell.
    const VD::edge_type *edge = cell.incident_edge();
    do {
        assert(edge->is_linear());
        if (!edge->is_primary()) continue;
        std::optional<Domain::Line> line = to_line(*edge, points, maximal_distance);
        if (!line.has_value()) continue;
        Orientation orientation = orient(center, line->a, line->b);
        // Can be rich on circle over source point edge
        if (orientation == Orientation::ORIENTATION_COLINEAR)
            continue;
        if (orientation == Orientation::ORIENTATION_CW)
    ...
    Domain::Polygon polygon = to_polygon(lines, center, maximal_distance, min_distance, count_point);
                                               ...
                                               const Domain::Lines &          lines)
{
    ...
    const VD::cell_type *cell = edge->cell(); // const VD::cell_type *  cell2     = edge.twin()->cell();
    const Domain::Line &line = lines[cell->source_index()];
    // const Line &           line1     = lines[cell2->source_index()];
    Domain::Point  point = to_point(vertex);
    double distance = Biz::Algorithms::Line::distance_to(line, point);
...
Domain::Point VoronoiGraphUtils::retrieve_point(const Domain::Lines &        lines,
...
const Domain::Point &VoronoiGraphUtils::retrieve_point(
    const Domain::Points &points, const VD::cell_type &cell)
{
...
Domain::Point VoronoiGraphUtils::get_parabola_point(
    const VD::edge_type &parabola, const Domain::Lines &lines)
{
    using namespace boost::polygon;
...
Domain::Line VoronoiGraphUtils::get_parabola_line(
    const VD::edge_type &parabola, const Domain::Lines &lines)
{
    assert(parabola.is_curved());
...
Parabola VoronoiGraphUtils::get_parabola(
    const VD::edge_type &edge, const Domain::Lines &lines)
{
    Domain::Point    point = get_parabola_point(edge, lines);
    Domain::Line     line  = get_parabola_line(edge, lines);
...
double VoronoiGraphUtils::calculate_length_of_parabola(
    ...
    const Domain::Lines &                                       lines)
{
    Domain::Point v0 = to_point(edge.vertex0());
    Domain::Point v1 = to_point(edge.vertex1());
    ...
    const VD::edge_type &edge, const Domain::Lines &lines)
{
    auto get_squared_distance = [&](const VD::vertex_type *vertex,
                                    const Domain::Point &point) -> double {
        Domain::Point point_v = to_point(vertex);
        Vec2d vector  = (point - point_v).cast<double>();
        return vector.x() * vector.x() + vector.y() * vector.y();
    };
    auto max_width = [&](const Domain::Point& point)->double{
        return 2. *
    ...
    if (edge.is_linear()) {
        ...
        if (edge.cell()->contains_point()) {
            const Domain::Line &source_line = lines[edge.cell()->source_index()];
            Domain::Point source_point;
        ...
        const Domain::Line &line = lines[edge.cell()->source_index()];
...
std::pair<coord_t, coord_t> VoronoiGraphUtils::calculate_width_for_line(
    const VD::edge_type &line_edge, const Domain::Lines &lines)
{
    assert(line_edge.is_linear());
    // edge line could be initialized by 2 points
    if (line_edge.cell()->contains_point()) {
        const Domain::Line &source_line = lines[line_edge.cell()->source_index()];
        Domain::Point       source_point;
    ...
    const Domain::Line &                line = lines[line_edge.cell()->source_index()];
    Domain::Point                       v0   = to_point(line_edge.vertex0());
    Domain::Point                       v1   = to_point(line_edge.vertex1());
    double                      distance0 = line.perp_distance_to(v0);
    double                      distance1 = line.perp_distance_to(v1);
...
std::pair<coord_t, coord_t> VoronoiGraphUtils::calculate_width_for_parabola(
    const VD::edge_type &parabola_edge, const Domain::Lines &lines)
{
    ...
    Domain::Point                       v0       = to_point(parabola_edge.vertex0());
    Domain::Point                       v1       = to_point(parabola_edge.vertex1());
...
std::pair<coord_t, coord_t> VoronoiGraphUtils::min_max_width(
    const VD::edge_type &edge, const Domain::Point &point)
{
    auto distance = [](const VD::vertex_type *vertex,
                       const Domain::Point &          point) -> coord_t {
...
VoronoiGraph VoronoiGraphUtils::create_skeleton(const VD &vd, const Domain::Lines &lines)
{
...
bool VoronoiGraphUtils::is_opposit_direction(const VD::edge_type *edge, const Domain::Line &line)
{
    Domain::Point dir_line = LineUtils::direction(line);
    Domain::Point dir_edge = VoronoiGraphUtils::to_direction(edge);
    return !PointUtils::is_same_direction(dir_line, dir_edge);
}

Domain::Point VoronoiGraphUtils::create_edge_point(
    const VoronoiGraph::Position &position)
{
    return create_edge_point(position.neighbor->edge, position.ratio);
}
    
Domain::Point VoronoiGraphUtils::create_edge_point(const VD::edge_type *edge,
                                                   double               ratio)
{
    const Domain::Point v0 = round(Vec2d{edge->vertex0()->x(), edge->vertex0()->y()}).cast<coord_t>();
    const Domain::Point v1 = round(Vec2d{edge->vertex1()->x(), edge->vertex1()->y()}).cast<coord_t>();
...
VoronoiGraph::Position VoronoiGraphUtils::get_position_with_width(
    const VoronoiGraph::Node::Neighbor *neighbor, coord_t width, const Domain::Lines &lines)
{
    ...
    Domain::Line edge_line(to_point(edge->vertex0()), to_point(edge->vertex1()));
    const Domain::Line &source_line = lines[edge->cell()->source_index()];
...
std::pair<Domain::Point, Domain::Point> VoronoiGraphUtils::point_on_lines(
    const VoronoiGraph::Position &position, const Domain::Lines &lines)
{
    ...
    Domain::Point edge_point = create_edge_point(position);
    auto point_on_line = [&](const VD::edge_type *edge) -> Domain::Point {
        assert(edge->is_finite());
        const VD::cell_type *cell = edge->cell();
        size_t line_index = cell->source_index();
        const Domain::Line &line = lines[line_index];
        using namespace boost::polygon;
        ...
        Domain::Line intersecting_line(edge_point, edge_point + PointUtils::perp(dir));
        std::optional<Vec2d> intersection = LineUtils::intersection(line, intersecting_line);
        assert(intersection.has_value());
        Domain::Point result = intersection->cast<coord_t>();
...
double get_distance_sq(const VD::edge_type &edge, const Domain::Point &point, double &edge_ratio) {
...
VoronoiGraph::Position VoronoiGraphUtils::align(
    const VoronoiGraph::Position &position, const Domain::Point &to, double max_distance)
{
...
double VoronoiGraphUtils::outline_angle(const VoronoiGraph::Node::Neighbor &neighbor, const Domain::Lines& lines)
{
    ...
    const Domain::Line &l1 = lines[c1->source_index()];
    const Domain::Line &l2 = lines[c2->source_index()];
...
void VoronoiGraphUtils::draw(Biz::Algorithms::SVG::SVG &                svg,
                             ...
                             coord_t              width)
{
    Domain::Point from = to_point(edge.vertex0());
    Domain::Point to   = to_point(edge.vertex1());
    if (edge.is_curved()) { 
    ...
    svg.draw(Domain::Line(from, to), color, width);
}
                            ...
                            bool caption)
{
    ...
    for (auto &node : path) {
        ...
        Domain::Point from = to_point(prev_node->vertex);
        Domain::Point to   = to_point(node->vertex);
        svg.draw(Domain::Line(from, to), color, width);
        if (caption) {
...
void VoronoiGraphUtils::draw(const Domain::Polygon &polygon,
                             const Domain::Lines &  lines,
                             const Domain::Point &  center)
{
