...
template Points VoronoiUtils::discretize_parabola(const Point &, const PolygonsSegmentIndex &, const Point &, const Point &, Domain::coord_t, float);
template PolygonsPointIndex VoronoiUtils::get_source_point_index(const VoronoiDiagram::cell_type &, PolygonsSegmentIndexConstIt, PolygonsSegmentIndexConstIt);
...
VoronoiUtils::discretize_parabola(const Point &source_point, const Segment &source_segment, const Point &start, const Point &end, const coord_t approximate_step_size, float transitioning_angle)
{
    ...
    Point pxx;
    Biz::Algorithms::Line::distance_to_infinite_squared(Line(a, b), source_point, pxx);
...
VoronoiUtils::compute_segment_cell_range(const VD::cell_type &cell, const SegmentIterator segment_begin, const SegmentIterator segment_end)
{
    ...
    do {
        ...
        Domain::Vec2big v0 = VoronoiUtils::to_point(edge->vertex0());
        Domain::Vec2big v1 = VoronoiUtils::to_point(edge->vertex1());
...
VoronoiUtils::compute_point_cell_range(const VD::cell_type &cell, const SegmentIterator segment_begin, const SegmentIterator segment_end)
{
    ...
    const Point source_point = VoronoiUtils::get_source_point(cell, segment_begin, segment_end);
    ...
    const PolygonsPointIndex source_point_idx = VoronoiUtils::get_source_point_index(cell, segment_begin, segment_end);
    const Point                       edge_v0          = VoronoiUtils::to_point(edge->vertex0()).template cast<CoordType>();
    const Point                       edge_v1          = VoronoiUtils::to_point(edge->vertex1()).template cast<CoordType>();
    ...
    if (!Biz::Algorithms::Geometry::is_point_inside_polygon_corner(source_point_idx.prev().p(), source_point_idx.p(), source_point_idx.next().p(), edge_query_point)) {
    ...
    do {
        ...
        if (Domain::Vec2big v1 = VoronoiUtils::to_point(edge->vertex1()); v1 == source_point_i64) {
            ...
            assert((VoronoiUtils::to_point(edge->vertex0()) == source_point_i64 || edge->is_primary()) && "Point cells must end in the point! They cannot cross the point with an edge, because collinear edges are not allowed in the input.");
        }
    } while (edge = edge->next(), edge != cell.incident_edge());
