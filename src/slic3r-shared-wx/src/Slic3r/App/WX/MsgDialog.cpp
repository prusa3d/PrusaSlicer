...
MsgDialog::MsgDialog(wxWindow *parent, const wxString &title, const wxString &headline, long style, wxBitmap bitmap)
	...
	, boldfont(w_config()->normal_font())
	, content_sizer(new wxBoxSizer(wxVERTICAL))
	, btn_sizer(new wxBoxSizer(wxHORIZONTAL))
{
#ifdef __APPLE__
    this->SetBackgroundColour(w_config()->get_window_default_clr());
#endif
    ...
    headtext->Wrap(CONTENT_WIDTH * w_config()->em_unit());
...
wxButton* MsgDialog::add_button(wxWindowID btn_id, bool set_focus /*= false*/, const wxString& label/* = wxString()*/)
{
    ...
    w_config()->SetWindowVariantForButton(btn);
    if (set_focus) {
        btn->SetFocus();
...
static void add_msg_content(MsgDialog* parent, wxBoxSizer* content_sizer, const HtmlContent& content)
{
    ...
    wxFont      font = w_config()->normal_font();//wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT);
    wxFont      monospace = w_config()->code_font();
    wxColour    text_clr = w_config()->get_label_clr_default();
    auto        text_clr_str = encode_color(ColorRGB(text_clr.Red(), text_clr.Green(), text_clr.Blue()));
    auto        bgr_clr_str = w_config()->get_html_bg_color(parent);
...
#else
        double scale_factor = parent->GetDPIScaleFactor();//! (double)get_dpi_for_window(parent) / (double)DPI_DEFAULT;
...
void ErrorDialog::create(const HtmlContent& content, int icon_width)
{
    ...
    SetMaxSize(wxSize(-1, CONTENT_MAX_HEIGHT*w_config()->em_unit()));
    ...
    : MsgDialog(parent, caption.IsEmpty() ? wxString::Format(_L("%s info"), from_u8(SLIC3R_APP_NAME)) : caption, wxEmptyString, style)
{
    ...
    m_checkBox = new Widgets::CheckBox(this, m_checkBoxText);
#else
