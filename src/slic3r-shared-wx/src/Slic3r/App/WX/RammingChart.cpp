...
void Chart::draw()
{
    ...
    dc.SetBrush(wxBrush(w_config()->get_highlight_default_clr()));
#else
    ...
    if (visible_area.m_width < 0.499) {
        dc.DrawText(
            _L("NO RAMMING AT ALL"),
            wxPoint(
                m_rect.GetLeft() + m_rect.GetWidth() / 2 - legend_side,
    ...
    if (!m_line_to_draw.empty()) {
        for (unsigned int i = 0; i < m_line_to_draw.size() - 2; ++i) {
            int color =
                510 * ((m_rect.GetBottom() - (m_line_to_draw)[i]) / double(m_rect.GetHeight()));
    ...
    // draw x-axis:
    float last_mark = -10'000;
    for (float math_x = int(visible_area.m_x * 10) / 10;
         math_x < (visible_area.m_x + visible_area.m_width);
         math_x += 0.1f)
    {
        ...
        dc.DrawText(
            wxString().Format(wxT("%.1f"), math_x),
            wxPoint(x - scale_unit, y + 0.5 * scale_unit)
        );
        last_mark = x;
    }
    ...
    // draw y-axis:
    last_mark = 10'000;
    for (int math_y = visible_area.m_y; math_y < (visible_area.m_y + visible_area.m_height);
         math_y += 1)
    {
    ...
    // axis labels:
    wxString label  = from_u8(fmt::format("{} ({})", Biz::_u8L("Time"), Biz::_u8L("s")));
    int text_width  = 0;
    int text_height = 0;
    ...
    dc.DrawText(
        label,
        wxPoint(
            0.5 * (m_rect.GetRight() + m_rect.GetLeft()) - text_width / 2.f,
            m_rect.GetBottom() + 0.5 * legend_side
        )
    );
    label = from_u8(fmt::format("{} ({})", Biz::_u8L("Volumetric speed"), Biz::_u8L("mm³/s")));
    dc.GetTextExtent(label, &text_width, &text_height);
    dc.DrawRotatedText(
        label,
        wxPoint(0, 0.5 * (m_rect.GetBottom() + m_rect.GetTop()) + text_width / 2.f),
        90
    );
}
...
void Chart::mouse_moved(wxMouseEvent& event)
{
    ...
    double new_y =
        m_dragged->get_pos().m_y - double(delta_y) / m_rect.GetHeight() * visible_area.m_height;

    if (m_uniform)
        for (ButtonToDrag& b : m_buttons)
            b.move(
                fixed_x ? 0 : double(delta_x) / m_rect.GetWidth() * visible_area.m_width,
                new_y - b.get_pos().m_y
            );
    else
        m_dragged->move(
...
void Chart::recalculate_line()
{
    ...
    if (points.size() > 1) {
        ...
        const bool boundary_first_derivative =
            true; // true - first derivative is 0 at the leftmost and rightmost point
        // false - second ---- || -------
        const int N =
            points.size() - 1; // last point can be accessed as N, we have N+1 total points
        ...
        for (int i = 1; i <= N - 1; ++i) {
            ...
            lambda[i] = 1.f - mu[i];
            rhs[i]    = 6
                * (float(points[i + 1].y - points[i].y)
                       / (h[i + 1] * (points[i + 1].x - points[i - 1].x))
                   - float(points[i].y - points[i - 1].y)
                       / (h[i] * (points[i + 1].x - points[i - 1].x)));
        }
        ...
        if (boundary_first_derivative) {
            ...
            mu[N]                            = 1;
            rhs[0]                           = (6.f / h[1])
                * (float(points[0].y - points[1].y) / (points[0].x - points[1].x)
                   - endpoints_derivative);
            rhs[N] = (6.f / h[N])
                * (endpoints_derivative
                   - float(points[N - 1].y - points[N].y) / (points[N - 1].x - points[N].x));
        } else {
            lambda[0] = 0;
            mu[N]     = 0;
            rhs[0]    = 0;
        ...
        for (int i = 1; i <= N; ++i) {
            float multiple =
                mu[i] / diag[i - 1]; // let's subtract proper multiple of above equation
        ...
        for (int x = m_rect.GetLeft(); x <= m_rect.GetRight(); ++x) {
            if (splines) {
                ...
                if (points[0].x > x)
                    ...
                    y = (rhs[i - 1] * pow(points[i].x - x, 3)
                         + rhs[i] * pow(x - points[i - 1].x, 3))
                            / (6 * h[i])
                        + (points[i - 1].y - rhs[i - 1] * h[i] * h[i] / 6.f)
                            * (points[i].x - x)
                            / h[i]
                ...
                if (i + 2 <= points.size() && m_buttons[i + 1].get_pos().m_x - 0.125 < x_math)
                    ++i;
                m_line_to_draw.push_back(
                    math_to_screen(wxPoint2DDouble(x_math, m_buttons[i].get_pos().m_y)).y
                );
            }
            ...
            m_total_volume += (m_rect.GetBottom() - m_line_to_draw.back())
                * (visible_area.m_width / m_rect.GetWidth())
...
std::vector<std::pair<float, float>> Chart::get_buttons() const
{
    ...
    for (const auto& button : m_buttons)
        buttons_out.push_back(
            std::make_pair(float(button.get_pos().m_x), float(button.get_pos().m_y))
        );
    return buttons_out;
}
