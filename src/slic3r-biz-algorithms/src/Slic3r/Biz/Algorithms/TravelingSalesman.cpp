...
template<typename PointType, ReverseCouldFail REVERSE_COULD_FAIL = ReverseCouldFail::False>
std::vector<std::pair<size_t, bool>> chain_segments(
    const std::function<const PointType&(std::size_t, bool)>& end_point_func,
    const size_t num_segments,
    const std::optional<PointType>& start_near,
    const std::function<bool(std::size_t)> could_reverse_func = [](size_t) -> bool { return true; }
)
{
	std::vector<std::pair<size_t, bool>> out;
	...
	else if (num_segments == 1)
	{
		...
		out.emplace_back(0, could_reverse_func(0) && start_near &&
	...
	else
	{
		...
		struct EndPoint {
			EndPoint(const Domain::Vec2d &pos) : pos(pos) {}
            Domain::Vec2d     pos;
        ...
        namespace kd_tree = Slic3r::Biz::Algorithms::KDTreeIndirect;
        kd_tree::KDTreeIndirect<2, double, decltype(coordinate_fn)> kdtree(coordinate_fn, end_points.size());
		...
		do {
			...
			if (REVERSE_COULD_FAIL == ReverseCouldFail::True) {
		...
		if (REVERSE_COULD_FAIL == ReverseCouldFail::True) {
			...
			if (failed)
				...
				out = chain_segments_closest_point(end_points, kdtree, could_reverse_func, (initial_point != nullptr) ? *initial_point : end_points.front());
		} else {
			assert(! failed);
		}
	}
...
{
    const auto segment_end_point{[&points](size_t idx, bool) -> const Domain::Point& {
        return points[idx];
    }};

    const std::vector<std::pair<size_t, bool>> permutation{chain_segments<Domain::Point>(
        segment_end_point,
        points.size(),
        start_near
    )};
    ASSERT(permutation.size() == points.size());
    std::vector<std::size_t> result;
    result.reserve(permutation.size());
    std::transform(
        permutation.begin(), permutation.end(), std::back_inserter(result),
        [](const std::pair<size_t, bool>& index_reversed) { return index_reversed.first; }
    );
    return result;
}
