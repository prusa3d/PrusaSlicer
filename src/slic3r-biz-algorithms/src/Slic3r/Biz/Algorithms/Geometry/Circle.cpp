...
using Domain::Vec2ds;
using Domain::Vec3d;
using Scaling::scaled;
using Scaling::unscaled;
...
Point circle_center_taubin_newton(const Points::const_iterator& input_begin, const Points::const_iterator& input_end, size_t cycles)
{
    ...
    std::transform(input_begin, input_end, std::back_inserter(tmp), [] (const Point& in) { return unscaled<double>(in); } );
...
Circled circle_ransac(const Vec2ds& input, size_t iterations, double* min_error)
{
    ...
    for (size_t iter = 0; iter < iterations; ++ iter) {
        ...
        Circled c;
        c.center = circle_center(samples[0], samples[1], samples[2], Domain::EPSILON);
