...
bool LineUtils::is_parallel_y(const Line2d &line)
{
    double x_change = line.a.x() - line.b.x();
    return (fabs(x_change) < std::numeric_limits<double>::epsilon());
}

std::optional<Line> LineUtils::crop_ray(const Line & ray,
                                                const Point &center,
                                                double       radius)
{
    if (is_parallel_y(ray)) {
        ...
        int count = Geometry::ray_circle_intersections(
            radius, a, b, c, points);
        if (count != 2) return {};
...
std::optional<Line2d> LineUtils::crop_ray(const Line2d &ray,
                                                 const Point &center,
                                                 double       radius)
{
    ...
    if (is_parallel_y(ray)) {
        ...
        Vec2d   first(x, y);
        Vec2d   second(x, -y);
        return Line2d(first + center_d,
                     second + center_d);
    } else {
        Line2d moved_line(ray.a - center_d, ray.b - center_d);
        double a, b, c;
        std::tie(a, b, c) = get_param(moved_line);
        std::pair<Vec2d, Vec2d> points;
        int count = Geometry::ray_circle_intersections(radius, a, b,
                                                               c, points);
        if (count != 2) return {};
        return Line2d(points.first + center_d, points.second + center_d);
    }
}

std::optional<Line> LineUtils::crop_half_ray(const Line & half_ray,
                                                     const Point &center,
                                                     double       radius)
{
...
std::optional<Line2d> LineUtils::crop_half_ray(const Line2d & half_ray,
                                                     const Point &center,
                                                     double       radius)
{
    std::optional<Line2d> segment = crop_ray(half_ray, center, radius);
    if (!segment.has_value()) return {};
    ...
    if (!use_a && !use_b) return {};
    if (use_a && use_b) return segment;
    return Line2d(half_ray.a, (use_a) ? segment->a : segment->b);
}

std::optional<Line> LineUtils::crop_line(const Line & line,
                                                 const Point &center,
                                                 double       radius)
{
...
std::optional<Line2d> LineUtils::crop_line(const Line2d & line,
                                                 const Point &center,
                                                 double       radius)
{
    std::optional<Line2d> segment = crop_ray(line, center, radius);
    if (!segment.has_value()) return {};
    ...
    if (use_a) {
        if (same_dir)
            return Line2d(segment->a, line.b);
        else
            return Line2d(line.a, segment->a);
    } else { // use b
        if (same_dir)
            return Line2d(line.a, segment->b);
        else
            return Line2d(segment->b, line.b);
    }
}


std::tuple<double, double, double> LineUtils::get_param(const Line &line) {
    Domain::Vec2crd normal = line.normal();
    double a = normal.x();
    double b = normal.y();
...
std::tuple<double, double, double> LineUtils::get_param(const Line2d &line)
{
                     ...
                     const char *color_b)
{
    ...
    if (side_points) {
        std::string name_a = use_name ? (std::string("A_") + name): "A";
        std::string name_b = use_name ? (std::string("B_") + name) : "B";
        svg.draw_text(line.a, name_a.c_str(), color_a);
        svg.draw_text(line.b, name_b.c_str(), color_b);
    }
}

double LineUtils::perp_distance(const Line2d &line, Vec2d p)
{
...
std::optional<Vec2d> LineUtils::intersection(const Line &ray1, const Line &ray2)
{
...
Point LineUtils::direction(const Line &line)
{
    return line.b - line.a;
}

Point LineUtils::middle(const Line &line) {
