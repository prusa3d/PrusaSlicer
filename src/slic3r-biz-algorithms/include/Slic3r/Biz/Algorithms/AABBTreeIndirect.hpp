...
namespace Slic3r::Biz::Algorithms::AABBTreeIndirect {
...
public:
	...
	BoundingBoxWrapper(const size_t idx, const Domain::BoundingBox2crd &bbox) :
        ...
        m_bbox(
            bbox.min - Domain::Point(static_cast<Domain::coord_t>(Domain::SCALED_EPSILON), static_cast<Domain::coord_t>(Domain::SCALED_EPSILON)),
            bbox.max + Domain::Point(static_cast<Domain::coord_t>(Domain::SCALED_EPSILON), static_cast<Domain::coord_t>(Domain::SCALED_EPSILON))
        ) {}
        ...
        typename VectorType::Scalar &max_distance_squared)
{
    ...
    size_t hit_idx;
    VectorType hit_point = VectorType::Ones() * (std::numeric_limits<typename VectorType::Scalar>::quiet_NaN());
