...
inline const Domain::Point &make_point(const Domain::Point &p) { return p; }
...
public:
     ...
     */
    const Domain::Polygon &getPolygon() const { return (*polygons)[poly_idx]; }
...
using PolygonsPointIndex = PathsPointIndex<Domain::Polygons>;
...
{
    std::pair<Domain::Point, Domain::Point> operator()(const PolygonsPointIndex &val) const
    {
        const Domain::Polygon &poly           = (*val.polygons)[val.poly_idx];
        Domain::Point          start          = poly[val.point_idx];
        unsigned int   next_point_idx = (val.point_idx + 1) % poly.size();
        Domain::Point          end            = poly[next_point_idx];
        return std::pair<Domain::Point, Domain::Point>(start, end);
    }
};
...
{
    Domain::Point operator()(const PathsPointIndex<Paths>& val) const
    {
 ...
 */
template <>
struct hash<Slic3r::Biz::Algorithms::PolygonsPointIndex>
{
    size_t operator()(const Slic3r::Biz::Algorithms::PolygonsPointIndex& lpi) const
    {
        return Slic3r::Domain::PointHash{}(lpi.p());
    }
};
}//namespace std
