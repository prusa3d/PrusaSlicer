...
using Circlef = Circle<Domain::Vec2f>;
using Circled = Circle<Domain::Vec2d>;
using CircleSqf = CircleSq<Domain::Vec2f>;
using CircleSqd = CircleSq<Domain::Vec2d>;

/// Find the center of the circle corresponding to the vector of Points as an arc.
Domain::Point circle_center_taubin_newton(const Domain::Points::const_iterator& input_start, const Domain::Points::const_iterator& input_end, size_t cycles = 20);
inline Domain::Point circle_center_taubin_newton(const Domain::Points& input, size_t cycles = 20) { return circle_center_taubin_newton(input.cbegin(), input.cend(), cycles); }

/// Find the center of the circle corresponding to the vector of Pointfs as an arc.
Domain::Vec2d circle_center_taubin_newton(const Domain::Vec2ds::const_iterator& input_start, const Domain::Vec2ds::const_iterator& input_end, size_t cycles = 20);
inline Domain::Vec2d circle_center_taubin_newton(const Domain::Vec2ds& input, size_t cycles = 20) { return circle_center_taubin_newton(input.cbegin(), input.cend(), cycles); }
Circled circle_taubin_newton(const Domain::Vec2ds& input, size_t cycles = 20);

// Find circle using RANSAC randomized algorithm.
Circled circle_ransac(const Domain::Vec2ds& input, size_t iterations = 20, double* min_error = nullptr);
...
// to minimize the small circle bias!
    ...
    Circled circle_linear_least_squares_svd(const Domain::Vec2ds &input);
    // Linear Least squares fitting with QR decomposition. Medium accuracy, medium speed.
    Circled circle_linear_least_squares_qr(const Domain::Vec2ds &input);
    // Linear Least squares fitting solving normal equations. Low accuracy, high speed.
    Circled circle_linear_least_squares_normal(const Domain::Vec2ds &input);
...
inline Circled smallest_enclosing_circle_welzl(const Domain::Points &points)
{
    return smallest_enclosing_circle_welzl<Domain::Vec2d, Domain::Points>(points, Domain::SCALED_EPSILON);
}
...
{
    T lv2 = a * a + b * b;
    if (lv2 < T(Domain::SCALED_EPSILON * Domain::SCALED_EPSILON)) {
